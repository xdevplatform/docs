...
Each URL embeds a token that authorizes exactly one operation on exactly one
job until its expiry. Treat the URLs as secrets — anyone holding them can
perform that operation until they expire. Do not log them or embed them in
client-side code.
