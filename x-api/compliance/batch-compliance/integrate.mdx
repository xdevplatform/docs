...
up to 128 MB (about 6.5 million IDs).

<Note>
The previous Google Cloud Storage-based flow accepted uploads up to 1 GB
with an optional resumable mode. The current limit is 128 MB per upload —
for larger ID sets, split them across sequential jobs (results are
per-ID, so splitting is always safe). Support for larger single uploads
is under consideration.
</Note>

There is no resumable/chunked mode: if a transfer is
