...
<Steps>
  <Step title="Create a job" icon="plus">
    ...
    **Response:**
    ...
    Save the `upload_url` and `download_url` for the next steps. Both URLs include a signed `token` query parameter and expire at `upload_expires_at` and `download_expires_at` respectively; use them exactly as returned.

    <Warning>
      Do not include a `resumable` field in the request body. Resumable uploads are not supported, and setting `resumable` returns a 400 error.
    </Warning>
  </Step>
