...
2.  You can get information such as whether a Post was editable at the time it was created, how much time, if any, is remaining for a Post to be edited, and how many edits remain by specifying `edit_controls` in your [tweet.fields](https://developer.twitter.com/en/docs/twitter-api/fields) parameter.
3.  Finally, you can get the Post objects for each Post in a Post's edit history, by specifying the `edit_history_tweet_ids` using the [expansion](https://developer.twitter.com/en/docs/twitter-api/expansions) parameter
