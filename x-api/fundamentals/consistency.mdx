...
Here is an example response object from the [single Post lookup](https://twitter.com/en/docs/twitter-api/tweets/lookup/api-reference/get-tweets-id.html) endpoint (with the [tweet.fields](https://twitter.com/en/docs/twitter-api/data-dictionary/object-model/tweet.html) parameter set to `author_id`, `entities`):
...
All X API v2 endpoints require authentication. Many of them accept the [OAuth 2.0 Bearer Token](https://developer.twitter.com/en/docs/authentication/oauth-2-0) method, requiring a Bearer Token with the request to the endpoint to make a successful request.

For endpoints requiring authorization to create, manipulate, or retrieve data on behalf of another user, use [OAuth 1.0a User Context](https://developer.twitter.com/resources/fundamentals/authentication). This means providing your [developer App’s](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens) [API keys and tokens](https://developer.twitter.com/resources/fundamentals/authentication/api-key-and-secret) as well as a set of user [Access Tokens](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens) that you generate for the user on behalf of whom you’re making a request. The [3-legged OAuth flow](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens) can help you retrieve Access Tokens. Use a [tool or library](https://developer.twitter.com/en/docs/twitter-api/tools-and-libraries) that automatically builds this authorization header.
...
The object returned on each endpoint is condensed. To allow developers more customization in the response they get back from the API, the [`fields`](https://developer.twitter.com/en/docs/twitter-api/data-dictionary/object-model/tweet) parameter is used to request the fields desired. Fields will remain consistent across endpoints. The Post object will return the same fields across all endpoints where the Post object is returned. The same set of fields can be queried across similar endpoints.

For example, the same Post fields can be queried in the [Posts lookup](https://developer.twitter.com/en/docs/twitter-api/tweets/lookup/introduction) and for the expanded pinned Post in the [Users lookup](https://developer.twitter.com/en/docs/twitter-api/users/lookup/introduction).

---

### Expansions

Where appropriate, [expansions](https://developer.twitter.com/en/docs/twitter-api/expansions) are available for all nested id fields (e.g., all fields named `*_id`, such as `author_id`). Expansions are also available for all fields that have an id that is not the top-level identifier of the current object. For example, in the [Posts lookup](https://developer.twitter.com/en/docs/twitter-api/tweets/lookup/introduction), the Post is the current object with field `id` as the top-level identifier. The `author_id` or `referenced_tweets.id` fields are available to expand into complete user or Post objects by adding these comma-separated values to the `expansions` parameter.
