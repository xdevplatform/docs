---
title: Powerstream low-latency streaming API on the X API v2
...
description: "Powerstream is the X API v2 lowest-latency streaming endpoint for public Post data, using PowerTrack-style rules to filter by keywords, operators, and metadata."
---
...
<Card title="Powerstream operators" icon="list-check" href="/x-api/powerstream/operators">
...
- Rate limits: Rule management endpoints allow 5,000 requests per 15-minute window, per app, per endpoint. The stream endpoint (`GET /2/powerstream`) has no request rate limit. Connection limits apply instead.
