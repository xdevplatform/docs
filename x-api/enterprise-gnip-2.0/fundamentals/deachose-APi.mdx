...
2.  To help support maintaining a connection, each Decahose stream supports [Redundant Connections](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#additional-streams). The most common architecture is for a stream to have two connections, and on the client-side there are two independent consumers –ideally on different networks. With this design, there can be redundancy across the client-side networks, servers, and datastore pathways. Note that a full-copy of the data is served on each connection and the client-side must be tolerant of and manage duplicate data.
...
Since disconnects will happen, the Decahose stream has a dedicated [Recovery](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#overview) and a [Backfill](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#backfill) feature to help recover data that was missed due to disconnections and other operational issues.
...
[Methods](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#methods)

[Authentication](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#authentication)

[GET /\{stream-type}/:stream](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#get-stream-type-stream)

[Replay API](/x-api/enterprise-gnip-2.0/fundamentals/deachose-APi#replay-api)
...
**Note:** Please see [this article](https://developer.x.com/en/docs/tutorials/consuming-streaming-data) for additional details on consuming streaming data after the connection is established.
