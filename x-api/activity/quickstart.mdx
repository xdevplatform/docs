...
description: "This guide explains how to subscribe for and receive events using the X Activity API endpoints. Reference for the X API v2 standard tier covering activity."
---
...
We'll use our [bearer token](https://docs.x.com/fundamentals/authentication/oauth-2-0/overview#bearer-token-also-known-as-app-only) (from the developer portal) for authorization. The app-only bearer token works for public events like this one. Some events are private and require [user-context (OAuth 2.0)](https://docs.x.com/fundamentals/authentication/oauth-2-0/overview#oauth-2-0-authorization-code-flow-with-pkce) authentication instead — see [Event privacy and authentication](/x-api/activity/introduction#event-privacy-and-authentication) for details.
...
<CardGroup cols={2}>
  <Card title="Event payloads" icon="/icons/xds/icon-brackets.svg" href="/x-api/activity/event-payloads">
    Sample JSON for each activity event type
  </Card>
  <Card title="API Reference" icon="/icons/xds/icon-code.svg" href="/x-api/activity/activity-stream">
