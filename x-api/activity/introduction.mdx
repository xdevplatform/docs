...
Chat events are triggered when a user sends or receives a direct message. These events deliver **end-to-end encrypted** message payloads that must be decrypted using the [XChat XDK](/xchat/introduction).
...
<Tip>
See the [XChat API documentation](/xchat/getting-started) for a complete guide on handling encrypted chat events.
</Tip>
