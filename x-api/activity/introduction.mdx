...
When you create a subscription, you can pass an optional `expires_at` field with an RFC 3339 timestamp. When that time is reached, XAA automatically deletes the subscription so you do not have to clean it up yourself. Re-creating the same subscription refreshes `expires_at` to the new value.
...
For example, to create a subscription that expires on January 1, 2027:

```json
{
  "event_type": "post.create",
  "filter": { "user_id": "1830023695835787264" },
  "webhook_id": "2090847910112202752",
