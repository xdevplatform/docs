...
DM events are triggered when a user sends or receives an unencrypted direct message.
...
| `dm.indicate_typing` | Fire when a user is typing a message to the filtered user | `user_id` |

### Chat Events

Chat events are triggered when a user sends or receives an encrypted direct message.
