...
Chat events are triggered when a user sends or receives a direct message. These events deliver **end-to-end encrypted** message payloads that must be decrypted using the [Chat XDK](/xchat/introduction).
...
| `chat.conversation_join` | Fired when a user joins a group conversation | `user_id` |

<Tip>
See the [Chat API documentation](/xchat/getting-started) for a complete guide on handling encrypted chat events.
</Tip>
