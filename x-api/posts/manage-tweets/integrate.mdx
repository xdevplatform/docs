...
description: "This page covers tools and key concepts for integrating the manage Posts endpoints into. Reference for the X API v2 standard tier covering manage tweets."
...
### Post creation restrictions (self-serve)

The following restrictions apply to self-serve (Pay Per Use) API customers only. Enterprise API customers are not subject to these limits.

**Reply restrictions**

Replies via the API are only permitted if the replying account has been explicitly summoned by the original post's author. This means one of the following must be true:

- The original author @mentions the replying user/account in their post.
- The original author quotes a post from the replying user/account.

If neither condition is met, the reply request will be rejected.
