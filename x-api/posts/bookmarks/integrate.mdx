...
These specific endpoints require the use of [OAuth 2.0 Authorization Code Flow with PKCE](/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-3), which means that you must use a set of keys and user Access Tokens to make a successful request. The Access Tokens must be associated with the user that you are requesting on behalf of. If you want to generate a set of Access Tokens for another user, they must authorize or authenticate your App using an [Authorize URL](/resources/fundamentals/authentication#authorize-url).
