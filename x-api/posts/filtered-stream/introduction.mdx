...
Alternatively, you can link the Filtered Stream to a webhook for delivery as webhook events. For more details on using webhooks with Filtered Stream, see the [Filtered Stream Webhooks introduction](https://docs.x.com/x-api/webhooks/stream/introduction).
