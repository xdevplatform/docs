...
description: "This guide walks you through making your first full-archive search request to find Posts from. Reference for the X API v2 standard tier covering quickstart."
...
<Tip>
Full-archive search supports queries up to 1,024 UTF-8 bytes (4,096 for Enterprise). Multibyte characters count as more than one byte, so non-ASCII operators or terms can exhaust the budget with fewer visible characters. Over-budget queries are accepted with HTTP 200 and return `result_count: 0`.
</Tip>
...
| Query length | 512 UTF-8 bytes | 1,024 UTF-8 bytes |
...
<CardGroup cols={2}>
  ...
  <Card title="API Reference" icon="/icons/xds/icon-code.svg" href="/x-api/posts/full-archive-search">
