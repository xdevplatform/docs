...
description: "This page provides a complete list of operators available when building a query for the Search. Reference for the X API v2 standard tier covering integrate."
...
The limit is measured in UTF-8 bytes, not characters. Multibyte characters count as more than one byte each and can exhaust the budget with fewer visible characters. Queries that exceed the byte limit are accepted with HTTP 200 but return `result_count: 0`.
...
<CardGroup cols={2}>
  <Card title="Build a query" icon="/icons/xds/icon-filter.svg" href="/x-api/posts/search/integrate/build-a-query">
    Learn query syntax and best practices
  </Card>
  <Card title="Search introduction" icon="/icons/xds/icon-search.svg" href="/x-api/posts/search/introduction">
