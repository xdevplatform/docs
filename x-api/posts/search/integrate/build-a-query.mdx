...
description: "The search endpoints accept a single query with a GET request and return a set of historical. Reference for the X API v2 standard tier covering integrate."
...
The limit is measured in UTF-8 bytes, not characters. Multibyte characters (for example, non-ASCII operators or search terms) count as more than one byte each and can exhaust the budget with fewer visible characters. Queries that exceed the byte limit are accepted with HTTP 200 but return `result_count: 0`.
...
<CardGroup cols={2}>
  <Card title="Operators reference" icon="/icons/xds/icon-bulleted-list.svg" href="/x-api/posts/search/integrate/operators">
    Complete list of available operators
  </Card>
  <Card title="Search quickstart" icon="/icons/xds/icon-rocket.svg" href="/x-api/posts/search/quickstart/recent-search">
