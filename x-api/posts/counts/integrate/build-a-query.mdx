...
Your query can be up to 512 UTF-8 bytes for pay-per-use customers, or up to 4,096 UTF-8 bytes for Enterprise customers. The limit is measured in bytes, not characters, so multibyte characters count as more than one byte each. Queries that exceed the byte limit are accepted with HTTP 200 but return `result_count: 0`.
