---
title: "Compliance.Models"
sidebarTitle: "Compliance.Models"
description: "Reference for the compliance.models Python module in the X API SDK. Pydantic request and response models for the compliance endpoints of the X API v2."
---

이 모듈은 요청 및 응답 데이터 구조를 위한 Pydantic 모델을 제공합니다.
X API의 compliance 엔드포인트를 위한 것입니다. 모든 모델은
OpenAPI 명세에서 생성되며 타입 안전성과 검증을 제공합니다.

### class xdk.compliance.models.CreateJobsRequest

create_jobs에 대한 요청 모델

<ResponseField name="model_config" type="ConfigDict">
Default: `{'populate_by_name': True, 'validate_by_alias': True, 'validate_by_name': True}`

Configuration for the model, should be a dictionary conforming to [ConfigDict][pydantic.config.ConfigDict].
</ResponseField>

## CreateJobsResponse

<Badge color="blue">Class</Badge>

<Badge color="gray">기반 클래스: BaseModel</Badge>

create_jobs에 대한 응답 모델

## 메서드

### `class xdk.compliance.models.CreateJobsResponse`

create_jobs에 대한 응답 모델

### `class xdk.compliance.models.GetJobsByIdResponse`

get_jobs_by_id에 대한 응답 모델

### `class xdk.compliance.models.GetJobsResponse`

get_jobs에 대한 응답 모델

