---
title: "새 메시지를 실시간으로 받기"
sidebarTitle: 실시간 이벤트
description: "웹훅이나 스트림으로 X Chat의 새 메시지와 이벤트를 실시간으로 수신하고 Chat XDK로 복호화합니다."
keywords: ["X Chat 이벤트", "웹훅", "활동 스트림", "실시간 메시지", "chat.received", "chat.sent"]
---
...
비공개 X Chat 이벤트 타입은 모니터링하는 사용자에 대한 권한 부여가 필요합니다. 암호화된 X Chat 파일 첨부는 Post API의 `expansions=attachments.media_keys` / `media.fields=variants`가 아니라 **`media_hash_key`**와 X Chat 미디어 다운로드를 사용합니다.
...
요청 본문과 필요한 스코프는 각 경로의 OpenAPI 작업에 정의되어 있습니다. X Activity API(XAA) 구독을 생성하려면 활동을 모니터링할 사용자에 대한 **사용자 컨텍스트 권한 부여**(채팅 이벤트에 대한 `dm.read` 같은 관련 스코프를 갖춘 OAuth 2.0 사용자 컨텍스트)가 필요합니다.
...
```json title="chat.received" lines wrap icon="/icons/xds/icon-brackets.svg"
{
  "data": {
    "event_type": "chat.received",
