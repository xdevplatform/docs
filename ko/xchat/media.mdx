---
title: "X Chat에서 이미지와 파일 보내기"
sidebarTitle: 미디어
description: "암호화된 채팅에서 이미지와 파일 첨부를 공유하세요. Chat XDK로 미디어를 암호화, 업로드, 전송, 다운로드, 복호화할 수 있습니다."
keywords: ["X Chat 미디어", "암호화된 이미지", "첨부 파일", "encrypt_stream", "미디어 업로드", "미디어 다운로드"]
---
...
이 경로는 Posts 미디어 모델(`expansions=attachments.media_keys`, `media.fields=variants` 등)이 **아닙니다**. 이러한 매개변수는 **Posts**에 적용됩니다; 암호화된 X Chat blob은 **`media_hash_key`**와 X Chat 미디어 다운로드로 주소가 지정됩니다.
...
**이벤트의 키 버전으로 키를 선택하세요.** 각 복호화된 메시지 이벤트는 콘텐츠가 암호화된 `keyVersion`(JS의 경우; 다른 바인딩은 `key_version`)을 가집니다. 첨부 파일은 최신 버전이 아니라 **해당** 버전의 대화 키(`conversationKeys.keys[event.keyVersion]`)로 복호화하세요. 키 순환(예: 멤버 추가) 후에는 최신 키가 이전 메시지에 첨부된 미디어를 복호화할 수 없습니다.
