---
title: "X Chat API로 암호화된 다이렉트 메시지 보내기"
sidebarTitle: 소개
description: "X Chat API가 제공하는 것을 알아보세요: X에서 암호화된 다이렉트 메시지를 주고받으며, 어려운 부분은 Chat XDK가 처리해 줍니다."
keywords: ["X Chat API", "Chat API", "암호화 DM", "암호화된 다이렉트 메시지", "Chat XDK"]
---

**X Chat API**를 사용하면 X에서 **암호화된** 다이렉트 메시지를 주고받을 수 있습니다. 메시지 본문은 클라이언트에서 암호화되며, X는 암호문만 라우팅할 뿐 평문 내용을 읽을 수 없습니다. 메시지는 또한 **서명**되어 수신자가 발신자를 검증할 수 있습니다.
...
1. [암호화 기초](/xchat/cryptography-primer): 암호화 개념에 대한 선택적 배경 지식
2. [시작하기](/xchat/getting-started): 키 구현, 전송 및 수신
3. [Chat XDK](/xchat/xchat-xdk): 암호화 SDK 참조
4. [WASM으로 UI 앱 구축하기](/xchat/building-ui-apps-with-wasm) 및 [개인 키 다루기](/xchat/handling-private-keys): 키를 기기에 보관하고 PIN을 루트 자격 증명처럼 취급하세요
5. 필요한 주제에 따라 [실시간 이벤트](/xchat/real-time-events), [미디어](/xchat/media), 또는 [문제 해결](/xchat/troubleshooting)
