...
title: Chat API 시작하기
sidebarTitle: 시작하기
description: Chat XDK와 X API로 종단 간 암호화 X Chat 메시징 구현하기
...
| 구성 요소 | 역할 |
|:----------|:-----|
| **[Chat XDK](/xchat/xchat-xdk)** | 암호화, 복호화, 서명 및 개인 키 저장 (Juicebox 또는 키 blob) |
...
**Juicebox**(PIN + API의 `juicebox_config`) 또는 **키 blob**(`import_keys`)으로 개인 키를 로드합니다. 그런 다음 등록된 공개 키 버전(레코드의 `public_key_version`)을 설정합니다.
...
서버 및 봇 샘플에서는 종종 **키 blob**(`export_keys` / `import_keys`)을 사용합니다. 클라이언트 앱에서는 종종 **Juicebox**(PIN으로 `setup` / `unlock`)를 사용합니다. 두 경로 모두에 대해 [Chat XDK](/xchat/xchat-xdk) 참조를 확인하세요.

---

## 3. 공개 키 등록 (일회성)

키페어를 생성하고 X에 **공개** 키를 등록한 다음, **개인** 키를 저장합니다(PIN으로 `setup`, 또는 `export_keys`).
...
<Warning>
Juicebox에 강력한 PIN을 사용하세요. PIN이나 보호되지 않은 키 blob을 분실하면 이전 메시지를 복호화할 수 없게 될 수 있습니다.
</Warning>
...
- 프로덕션에서 PIN, 개인 키, 메시지 평문을 로그에 남기지 마세요
- 웹 앱에서는 OAuth 토큰(및 Juicebox realm 토큰 발급)을 서버에 유지하세요; 개인 키는 클라이언트 Chat XDK에만 보관하는 것이 좋습니다
