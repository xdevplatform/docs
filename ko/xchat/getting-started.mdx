---
title: "시작하기: 첫 암호화 메시지 보내기"
sidebarTitle: 시작하기
description: "Python, TypeScript, Go, Rust, C#, Java로 처음부터 첫 암호화 X Chat 메시지 전송까지 안내하는 실습 튜토리얼입니다."
keywords: ["X Chat 튜토리얼", "X Chat 퀵스타트", "Chat XDK", "암호화 DM", "Python", "TypeScript", "Go", "Rust", "C#", "Java"]
---

X에서 암호화된 다이렉트 메시지를 주고받기: 키를 설정하고, 대화를 초기화하며, 메시지를 보내고, 수신 트래픽을 복호화합니다.
...
<Warning>
**감싸기 전에 가져온 키를 검증하세요.** `prepare_conversation_key_change`는 전달하는 모든 공개 키에 대해 새 대화 키를 암호화합니다. 각 가져온 레코드를 먼저 `verify_key_binding(identity, signing, signature)`로 확인하세요(공개 키 API에서 얻은 레코드의 `public_key`, `signing_public_key`, `identity_public_key_signature` 필드를 전달). 그래야 대체된 아이덴티티 키가 대화 키를 받지 못합니다.
</Warning>
...
API가 요구하는 경우 URL 경로에는 **하이픈이 있는** 대화 ID를 사용하세요(`:` → `-`). SDK 자체는 유연합니다: `encrypt_message`와 `encrypt_reply`는 당신이 보유한 어떤 형태의 ID든 받아들이며(이벤트의 `A:B`, 목록이나 URL 경로의 `A-B`(순서 무관), 혹은 그저 수신자의 사용자 ID까지) 서명 전에 정규화합니다. 그룹 ID(접두사 `g`)는 변경 없이 통과합니다.
