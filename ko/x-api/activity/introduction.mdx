...
title: 소개
sidebarTitle: 소개
description: "게시물, 좋아요, 팔로우, 뮤트, 차단, DM, 멘션, 프로필 업데이트 등 X 플랫폼의 실시간 사용자 활동 이벤트를 웹훅을 통해 스트리밍하세요."
...
현재 X Activity API는 카테고리별로 분류된 다음 이벤트 유형을 지원합니다. 각 유형의 샘플 JSON은 [이벤트 페이로드](/x-api/activity/event-payloads)를 참조하세요.
...
Like 이벤트는 사용자가 Post에 좋아요를 누르거나 사용자 본인의 Post 중 하나에 좋아요가 눌릴 때 트리거됩니다.

| 이벤트 이름 | 설명 | 필터 |
| --- | --- | --- |
| `like.create` | 사용자가 Post에 좋아요를 누르거나, 사용자의 Post 중 하나에 좋아요가 눌릴 때 발생 | `user_id`, `direction` |

<Note>
**Direction 필터:** `like.create` 이벤트는 `outbound` 또는 `inbound`로 설정할 수 있는 선택적 `direction` 필터를 지원합니다. 사용자가 누른 좋아요에는 `outbound`를, 사용자의 Post가 받은 좋아요에는 `inbound`를 사용하세요. `direction`을 지정하지 않으면 인바운드와 아웃바운드 좋아요 이벤트가 모두 전달됩니다.

**비공개 이벤트:** `like.create`는 비공개 이벤트이며 사용자 컨텍스트(OAuth 2.0) 인증이 필요합니다. 아래 [이벤트 프라이버시 및 인증](#event-privacy-and-authentication)을 참조하세요.
</Note>
...
Mute 이벤트는 필터링된 사용자가 다른 사용자를 뮤트하거나 뮤트 해제할 때 트리거됩니다.

| 이벤트 이름 | 설명 | 필터 |
| --- | --- | --- |
| `mute.mute` | 사용자가 다른 사용자를 뮤트할 때 발생 | `user_id` |
| `mute.unmute` | 사용자가 다른 사용자를 뮤트 해제할 때 발생 | `user_id` |

<Note>
**비공개 이벤트:** Mute 이벤트는 비공개이며 `mute.read` 스코프가 포함된 사용자 컨텍스트(OAuth 2.0) 인증이 필요합니다. `user_id` 필터는 뮤트 또는 뮤트 해제를 수행하는 사용자와 매칭됩니다 — 어떤 사용자가 뮤트*당하는* 것에 대한 알림은 구독할 수 없습니다. 아래 [이벤트 프라이버시 및 인증](#event-privacy-and-authentication)을 참조하세요.
</Note>

### Block 이벤트

Block 이벤트는 필터링된 사용자가 다른 사용자를 차단하거나 차단 해제할 때 트리거됩니다.

| 이벤트 이름 | 설명 | 필터 |
| --- | --- | --- |
| `block.block` | 사용자가 다른 사용자를 차단할 때 발생 | `user_id` |
| `block.unblock` | 사용자가 다른 사용자를 차단 해제할 때 발생 | `user_id` |

<Note>
**비공개 이벤트:** Block 이벤트는 비공개이며 `block.read` 스코프가 포함된 사용자 컨텍스트(OAuth 2.0) 인증이 필요합니다. `user_id` 필터는 차단 또는 차단 해제를 수행하는 사용자와 매칭됩니다 — 어떤 사용자가 차단*당하는* 것에 대한 알림은 구독할 수 없습니다. 아래 [이벤트 프라이버시 및 인증](#event-privacy-and-authentication)을 참조하세요.
</Note>
...
- 뮤트 (`mute.mute`, `mute.unmute`) — `mute.read` 스코프 필요
- 차단 (`block.block`, `block.unblock`) — `block.read` 스코프 필요
