...
<Update label="2026년 6월 30일" description="X Developer Platform">
  ### X API와 X 문서를 위한 호스팅형 MCP 서버

  오늘, Grok, Cursor, Claude, VS Code와 같은 AI 도구를 X에 직접 연결할 수 있는 두 개의 호스팅형 [Model Context Protocol](/tools/mcp) 서버를 출시했습니다.

  - **X MCP** (`https://api.x.com/mcp`) — MCP 호환 클라이언트에서 X API 엔드포인트를 호출하여 Post 검색, 사용자 조회, 북마크 관리, 트렌드 및 뉴스 가져오기, Articles 초안 작성 또는 게시까지 모두 본인 X 계정의 권한으로 수행할 수 있습니다. 오픈 소스 [`xurl mcp`](https://github.com/xdevplatform/xurl) 브리지를 통해 연결하며, 이 브리지는 OAuth 2.0 PKCE 로그인을 처리하고 토큰을 자동으로 갱신합니다.
  - **Docs MCP** (`https://docs.x.com/mcp`) — AI 도구 내에서 X API 문서를 검색하고 읽을 수 있습니다.

  Grok Build, Cursor, Claude, VS Code 설정 가이드와 헤드리스/원격 로그인 방법은 [MCP 서버](/tools/mcp) 문서에서 확인할 수 있습니다.
</Update>
