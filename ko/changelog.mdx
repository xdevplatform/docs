...
<Update label="2026년 7월 21일" description="개발자 플랫폼">
  ### developer.x.com/status에 새로운 실시간 상태 페이지 공개

  X Developer Platform(X API v2, Enterprise API, Ads API, 스트리밍 엔드포인트 및 지원 서비스 포함)의 실시간 상태를 이제 [developer.x.com/status](https://developer.x.com/status)에서 확인할 수 있습니다. 새 페이지를 북마크에 추가하여 가동 시간과 현재 진행 중인 인시던트를 모니터링하세요. 과거 인시던트는 [인시던트](/incidents) 페이지에 보존되어 있습니다.
</Update>
