...
<Update label="2026년 6월 28일" description="X Ads API">
  ### 캠페인 예산 최적화가 `LINE_ITEM`으로 단순화됨

  [Campaign Management API](/x-ads-api/campaign-management/reference)의 캠페인은 이제 X Ads Manager의 동작과 동일하게 `LINE_ITEM` 예산 최적화만 지원합니다.

  - 캠페인 `POST` 및 `PUT` 요청에서 `budget_optimization`은 `LINE_ITEM`만 허용하며, 기본값이 `LINE_ITEM`으로 변경되었습니다(이전에는 `CAMPAIGN`).
  - `standard_delivery`는 캠페인 레벨에서 더 이상 사용되지 않습니다. 표준 또는 가속 게재 설정은 라인 항목의 [`standard_delivery`](/x-ads-api/campaign-management/reference) 매개변수에서 구성하세요.
</Update>
