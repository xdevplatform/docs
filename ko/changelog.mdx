...
<Update label="2026년 5월 21일" description="X API">
  ### 레거시 Basic 요금제의 Pay-Per-Use 마이그레이션

  오늘 X API의 레거시 **Basic** 요금제(월간 및 연간)가 더 이상 지원되지 않으며, **2026년 6월 1일**부터 [Pay-Per-Use(PPU)](https://developer.x.com/en/portal/products)로 자동 마이그레이션됨을 알려드립니다. 전환일(현재 청구 주기 종료일)이 지나면 Basic 구독은 종료되고, 고정 월 요금 없이 PPU로 전환됩니다.

  - **Basic 월간 구독자**: Stripe를 통해 결제 수단에서 **\$200** 초기 크레딧이 청구됩니다.
  - **Basic 연간 구독자**: 연간 요금의 남은 일할 금액이 PPU 크레딧으로 지급됩니다(전환 시 카드 추가 청구 없음).
  - **마이그레이션 후 PPU 기본 설정**: 잔액이 **\$10** 미만으로 떨어지면 **\$200** 자동 충전이 활성화되며, 청구 주기당 지출 한도는 **\$400**입니다. [Developer Portal](https://developer.x.com/en/portal/dashboard)에서 언제든지 조정할 수 있습니다.

  마이그레이션을 원하지 않는 경우 2026년 6월 1일 이전에 Developer Portal에서 옵트아웃할 수 있습니다. 옵트아웃하면 전환일 이후 API 액세스가 종료된다는 점에 유의하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)를 참조하세요.
</Update>

<Update label="2026년 5월 20일" description="X API">
  ### 수정 사항: 무료 크레딧에 대해서도 자동 충전이 작동합니다

  오늘 자동 충전 기능에 대한 수정 사항을 배포하여, 무료 크레딧(상환된 바우처에서 발생한 크레딧)이 유료 크레딧과 동일하게 처리되도록 했습니다. 이전에는 무료 크레딧 잔액이 0이 되어도 자동 충전이 활성화되어 있어도 트리거되지 않았습니다. **2026년 5월 21일**부터, 잔액이 모두 소진되고 [Developer Console](https://console.x.com)에서 자동 충전이 활성화된 경우, 저장된 결제 수단을 사용해 계정이 자동으로 충전됩니다.

  무료 크레딧 소진 후 비용 청구를 원하지 않는다면, Developer Console에서 자동 충전 설정을 확인하고 필요한 경우 비활성화하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)를 참조하세요.
</Update>
