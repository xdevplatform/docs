...
<Update label="2026년 7월 24일" description="X API v2">
  ### X Activity API 이벤트 페이로드 레퍼런스

  오늘 [X Activity API](/ko/x-api/activity/introduction)의 새로운 [이벤트 페이로드](/ko/x-api/activity/event-payloads) 레퍼런스를 게시했습니다. Post, 좋아요, 팔로우, 프로필 업데이트, 채팅, DM, 뮤트, 차단, Spaces, 뉴스, OAuth 취소 등 지원되는 모든 이벤트 유형에 대한 샘플 JSON을 제공합니다.

  구독을 연결하기 전에 실제로 캡처된 페이로드를 기준으로 웹훅 핸들러와 JSON 역직렬화를 설계하는 데 활용하세요.
</Update>
