...
<Update label="2026년 6월 18일" description="X API">
  ### 가격 페이지 업데이트: 자동 충전 보호 장치, 웹훅 이벤트 가격, 더 명확한 xAI 크레딧 등급

  오늘, [X API 가격 페이지](/x-api/getting-started/pricing)를 사용자 친화적인 여러 항목으로 개선했습니다.

  - **자동 충전 보호 장치** — 자동 충전은 이제 5분에 최대 한 번만 실행되며, 잔액이 0이거나 음수일 때는 일시 중지되어 예기치 않은 과금으로부터 보호합니다.
  - **웹훅 이벤트 가격** — [X Activity API](/x-api/activity/introduction) 이벤트(예: `post.create`, `follow.follow`, `dm.received`, `spaces.start`)에 대한 이벤트별 가격 표를 추가했습니다. 다른 리소스와 동일한 24시간 UTC 윈도우 내에서 중복 제거됩니다.
  - **xAI API 무료 크레딧 등급** — 누적 지출에 따른 보상 등급(&dollar;200 이상 10%, &dollar;500 이상 15%, &dollar;1,000 이상 20%)을 구매별로 정확히 얼마를 받게 되는지 보여주는 예시와 함께 명확히 했습니다.
  - **Enterprise 비교** — [Enterprise 가격 페이지](/enterprise-api/getting-started/pricing)에서 종량제 플랜에는 월 200만 건의 Post 읽기 상한이 포함되어 있는 반면, Enterprise 플랜은 맞춤형 또는 무제한 볼륨을 제공한다는 점을 명시했습니다.
</Update>
