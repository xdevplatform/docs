...
<Update label="2026년 6월 30일" description="개발자 도구">
  ### X API 및 문서를 위한 호스팅 MCP 서버

  오늘, Grok, Cursor, Claude Desktop, VS Code와 같은 AI 도구가 X와 직접 통신할 수 있도록 두 개의 호스팅 [Model Context Protocol 서버](/tools/mcp)를 출시했습니다:

  - **X MCP** (`https://api.x.com/mcp`) — MCP 호환 클라이언트에서 X API 엔드포인트를 호출하세요. 모델은 본인 계정의 권한으로 전체 아카이브 검색, 사용자 조회, bookmarks 관리, trends 및 뉴스 가져오기, Articles 초안 작성 또는 게시를 수행할 수 있습니다. OAuth 2.0과 토큰 갱신을 자동으로 처리하는 오픈 소스 `xurl mcp` 브리지를 통해 연결하세요.
  - **Docs MCP** (`https://docs.x.com/mcp`) — 에디터를 벗어나지 않고도 AI 어시스턴트가 X API 문서를 검색하고 읽을 수 있습니다.

  Grok Build, Cursor, Claude Desktop, VS Code 및 기타 모든 MCP 클라이언트별 설정 방법은 [MCP 설정 가이드](/tools/mcp)를 참조하세요.
</Update>
