---
title: 변경 로그
description: "업데이트를 구독하려면 @API의 \"알림 켜기\"를 활성화하세요. 오늘 X API v2 검색 엔드포인트(/2/tweets/search/recent, /2/tweets/search/all, /2/tweets/counts/recent, /2/tweets/counts/all)를 새로운 X."
sidebarTitle: 개요
...
<Update label="2026년 5월 21일" description="X API">
  ### 레거시 Basic 플랜이 Pay-Per-Use로 전환

  오늘 레거시 X API **Basic** 플랜(월간 및 연간)이 더 이상 사용되지 않으며, 새로운 주력 플랜인 **Pay-Per-Use(PPU)** 로 자동 마이그레이션됨을 발표했습니다. **2026년 6월 1일** 이후, 현재 Basic 구독은 현재 청구 주기 종료 시점(전환일)에 종료되고 PPU로 이전됩니다. PPU는 월 고정 요금이 없으며, 실제로 사용한 API 요청에 대해서만 비용을 지불합니다. 전환 시 월간 구독자에게는 Stripe를 통해 **\$200** 의 초기 크레딧이 청구됩니다. 연간 구독자에게는 연간 요금의 잔여 일할 금액이 PPU 크레딧으로 지급됩니다. 마이그레이션 후 자동 재충전은 잔액이 **\$10** 미만으로 떨어질 때 **\$200** 로 작동하도록 기본 설정되며, 청구 주기당 지출 한도는 **\$400** 로 설정됩니다. 두 설정 모두 [Developer Portal](https://developer.x.com/en/portal/dashboard) 에서 언제든지 조정할 수 있습니다. PPU 마이그레이션을 원치 않는 경우 2026년 6월 1일 이전에 Developer Portal에서 옵트아웃할 수 있지만, 전환일 이후 API 액세스가 종료됩니다. 자세한 내용은 [여기](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)를 참조하세요.
</Update>

<Update label="2026년 5월 20일" description="X API">
  ### 자동 재충전이 무료 크레딧에도 적용

  크레딧 바우처(무료 크레딧) 잔액이 0이 되었을 때 자동 재충전이 작동하지 않던 문제를 수정했습니다. **2026년 5월 21일**부터 자동 재충전에서 무료 크레딧은 유료 크레딧과 동일하게 처리됩니다. 잔액이 모두 소진되고 자동 재충전이 활성화되어 있으면, 저장된 결제 수단을 사용해 계정이 자동으로 충전됩니다. 무료 크레딧 소진 이후 청구를 원치 않는 경우 [Developer Console](https://console.x.com) 에서 설정을 확인하고 필요 시 자동 재충전을 비활성화하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)를 참조하세요.
</Update>

<Update label="2026년 5월 13일" description="X Ads API">
  ### X Ads API 피드백 프로그램

  **X Ads API** 개선을 위해 개발자와 광고주 여러분의 직접적인 피드백을 받고 있습니다. 버그 신고, 엔드포인트 관련 문제점, 원하는 기능, 문서 부족 사항 등 무엇이든 환영합니다. [피드백 양식](https://docs.google.com/forms/d/e/1FAIpQLSeDn5s1T0ze9-7K0XAw18MD3bEzGrIle3ko3uyjN8EP5dJhYQ/viewform)을 통해 또는 [공지 스레드](https://devcommunity.x.com/t/x-ads-api-share-your-feedback/265247)에 답글로 의견을 공유해 주세요.
</Update>
