...
<Update label="2026년 7월 21일" description="X API v2">
  ### Account Activity API 지원 중단

  오늘, [Account Activity API (AAA)](/x-api/account-activity/introduction)의 지원 중단을 발표했습니다. 앞으로 실시간 사용자 활동 전송에는 [X Activity API (XAA)](/x-api/activity/introduction)를 사용하세요. XAA는 동일한 이벤트 유형(Posts, Direct Messages, Likes, Follows, Spaces 등)을 더 낮은 지연 시간의 파이프라인으로 제공하며, 사용자 ID별 단일 구독 모델을 지원합니다.

  기존 AAA 통합은 지원 중단 기간 동안 계속 작동합니다. 중단을 피하려면 XAA로의 마이그레이션을 계획하세요.
</Update>
