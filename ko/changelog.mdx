...
<Update label="2026년 6월 8일" description="X API">
  ### Pay-Per-Use 가격 관리 기능 및 xAI 크레딧 등급

  오늘 [X API 가격 페이지](/x-api/getting-started/pricing)를 업데이트하여 크레딧 기반 청구에 대한 자세한 내용과 [개발자 콘솔](https://console.x.com)의 새로운 관리 기능을 추가했습니다:

  - **자동 충전** — 크레딧 잔액이 설정한 임계값 아래로 떨어지면 자동으로 충전합니다.
  - **지출 한도** — 결제 주기당 지출 한도를 설정하여 예기치 않은 청구를 방지합니다.
  - **중복 제거 명확화** — 동일한 리소스(예: Post)를 24시간(UTC) 이내에 여러 번 요청해도 한 번만 과금됩니다.
  - **무료 xAI API 크레딧 등급** — 결제 주기 내 X API 누적 지출을 기준으로 [xAI API](https://docs.x.ai) 크레딧을 최대 **20% 환급**: \$200 이상 **10%**, \$500 이상 **15%**, \$1,000 이상 **20%**. 크레딧을 받으려면 개발자 콘솔에서 xAI 팀을 연결하세요.

  엔드포인트별 읽기/쓰기 요금과 \$0.001 [Owned Reads](/x-api/getting-started/pricing#owned-reads) 가격도 단일 요금 표에 함께 정리되어 있습니다.
</Update>
