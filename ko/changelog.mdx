...
<Update label="2026년 5월 21일" description="X API">
  ### 레거시 Basic 플랜이 Pay-Per-Use로 마이그레이션

  오늘 X API의 레거시 **Basic** 플랜(월간 및 연간)이 단계적으로 종료되며, **2026년 6월 1일**부터 **Pay-Per-Use (PPU)**로 자동 마이그레이션된다고 발표했습니다. PPU에는 고정 월 요금이 없으며, 실제 요청한 만큼만 비용을 지불합니다. Basic Monthly 가입자는 전환일에 초기 크레딧 **\$200**가 청구됩니다. Basic Annual 가입자는 남은 일할 금액이 PPU 크레딧으로 지급되며, 카드에 추가 청구되지 않습니다. 마이그레이션 후 자동 충전은 잔액이 **\$10** 미만으로 떨어지면 **\$200**가 충전되도록 기본 설정되며, 주기당 지출 한도는 **\$400**입니다. 모든 항목은 [Developer Portal](https://developer.x.com/en/portal/dashboard)에서 언제든지 조정할 수 있습니다. 6월 1일 이전에 옵트아웃할 수 있지만, 그렇게 하면 전환일에 API 액세스가 종료됩니다. 전체 공지는 [여기](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)에서 확인하세요.
</Update>

<Update label="2026년 5월 20일" description="X API">
  ### 자동 충전이 이제 무료 크레딧에서도 작동합니다

  **2026년 5월 21일**부터 무료 크레딧(교환된 바우처)은 자동 충전에서 유료 크레딧과 동일하게 처리됩니다. 이전에는 자동 충전이 활성화되어 있어도 바우처 잔액이 0이 되면 자동 충전이 트리거되지 않았습니다. 이번 수정으로 자동 충전이 켜져 있고 잔액이 모두 소진되면 저장된 결제 수단으로 자동 청구되며, 이는 유료 크레딧의 기존 동작과 일치합니다. 무료 크레딧 소진 후 청구되지 않게 하려면 [Developer Console](https://console.x.com)에서 자동 충전을 검토하고 필요 시 비활성화하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)에서 확인하세요.
</Update>
