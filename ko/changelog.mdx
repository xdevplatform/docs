...
<Update label="2026년 6월 18일" description="X API v2">
  ### Filtered Stream을 위한 시맨틱 임베딩 연산자

  오늘, [Filtered Stream](/x-api/posts/filtered-stream/integrate/operators)에 새로운 `embedding:` 및 `embedding_threshold:` 연산자를 추가했습니다. 이러한 연산자는 정확한 키워드 일치를 요구하는 대신, 신경망 임베딩 모델을 사용해 자연어 쿼리와의 *시맨틱 유사도*로 Post를 매칭합니다. 키워드 기반 규칙만으로는 포착하기 어려운 개념과 주제를 추적하는 데 유용합니다.

  - `embedding:"query"` — 따옴표로 묶인 쿼리와 시맨틱적으로 유사한 Post를 매칭하는 독립형 연산자.
  - `embedding_threshold:X.XX` — 매칭에 필요한 최소 코사인 유사도를 설정합니다(0.0–1.0, 기본값 `0.3`). 다른 연산자와 함께 사용해야 합니다.

  Filtered Stream용 Embedding 티어 액세스가 프로비저닝된 Enterprise 계정에서 사용할 수 있습니다. 활성화하려면 계정 담당자에게 문의하세요.
</Update>
