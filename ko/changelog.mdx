...
<Update label="2026년 6월 19일" description="X Ads API">
  ### Ads API Analytics: 지원되지 않는 placement, 메트릭 그룹 및 엔터티 유형 제거

  오늘, 현재 프로덕션에서 지원되는 내용을 반영하도록 [Ads API Analytics](/x-ads-api/analytics) 엔드포인트를 업데이트했습니다. 통합에서 아래 값을 참조하는 경우, 그에 맞게 요청을 업데이트하세요.

  - **Placements:** `PUBLISHER_NETWORK`는 더 이상 analytics 요청에 유효한 placement가 아닙니다(Active Entities 응답에서만 계속 표시됩니다).
  - **메트릭 그룹:** `MEDIA` 메트릭 그룹이 제거되었으며, `ENGAGEMENT` 및 `FOLLOWERS`의 Media View Rate 파생 메트릭도 함께 제거되었습니다.
  - **엔터티 유형:** `MEDIA_CREATIVE` 및 `ORGANIC_TWEET`는 더 이상 analytics 또는 sync analytics에서 지원되는 엔터티 유형이 아닙니다.
  - **목표:** `APP_ENGAGEMENTS` 및 `LEAD_GENERATION` 목표 섹션(Metrics by Objective 및 Derived Metrics 모두)이 제거되었습니다.
  - **세그멘테이션:** 세그멘테이션은 `AGE`, `GENDER`, `METROS`, `PLATFORMS`에 대해서만 지원됩니다.
</Update>
