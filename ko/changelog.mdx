...
<Update label="2026년 6월 30일" description="개발자 플랫폼">
  ...
  오늘 Grok, Cursor, Claude, VS Code 같은 AI 도구가 X와 직접 연동할 수 있도록 호스팅된 [Model Context Protocol](/tools/mcp) 서버 두 개를 출시했습니다.

  - **X MCP** (`https://api.x.com/mcp`) — 본인 X 계정 권한으로 X API 엔드포인트(Post·사용자 조회, 전체 아카이브 검색, 북마크, 트렌드, 뉴스, Articles 초안 작성 및 게시)를 호출할 수 있습니다. OAuth와 토큰 갱신을 대신 처리해 주는 오픈소스 브리지 [`xurl mcp`](https://github.com/xdevplatform/xurl)를 통해 연결합니다.
  - **Docs MCP** (`https://docs.x.com/mcp`) — 인증 없이 AI 도구 안에서 X API 문서를 검색하고 읽을 수 있습니다.

  클라이언트 설정(Grok Build, Cursor, Claude, VS Code), 헤드리스 로그인 및 문제 해결은 [MCP 설정 가이드](/tools/mcp)를 참고하세요.
</Update>
