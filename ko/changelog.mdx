...
<Update label="2026년 5월 21일" description="X API">
  ### 레거시 Basic 요금제가 Pay-Per-Use로 전환

  오늘 레거시 **Basic** 요금제(월간 및 연간)의 사용 중단을 발표했습니다. **2026년 6월 1일** 이후, 모든 Basic 구독자는 현재 결제 주기가 종료될 때 [Pay-Per-Use (PPU)](https://docs.x.com/x-api/getting-started/about-x-api)로 자동 마이그레이션됩니다. 별도의 조치는 필요하지 않습니다. 월간 구독자는 **\$200**의 초기 크레딧이 결제되며, 연간 구독자는 남은 일할 계산 금액이 PPU 크레딧으로 지급됩니다. 마이그레이션 후 PPU 기본 설정: 자동 충전이 **\$200**로 활성화되며(잔액이 **\$10** 미만일 때 발동), 주기당 지출 한도는 **\$400**입니다. 모든 항목은 [Developer Portal](https://developer.x.com/en/portal/dashboard)에서 언제든 조정할 수 있습니다. 옵트아웃하려면 **2026년 6월 1일** 이전에 설정을 업데이트하세요. 옵트아웃하면 전환일에 API 액세스가 종료된다는 점에 유의하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)를 참조하세요.
</Update>

<Update label="2026년 5월 20일" description="X API">
  ### 자동 충전이 무료 크레딧에도 적용

  오늘 무료(바우처) 크레딧이 0에 도달한 경우 자동 충전이 트리거되지 않던 문제를 수정했습니다. **2026년 5월 21일**부터 무료 크레딧은 유료 크레딧과 동일하게 처리됩니다. 잔액이 완전히 소진되고 [Developer Console](https://console.x.com)에서 자동 충전이 활성화되어 있으면 저장된 결제 수단으로 계정이 자동으로 충전됩니다. 자동 충전을 활성화한 상태에서 무료 크레딧 소진 후의 청구를 원하지 않는다면 콘솔 설정에서 검토 후 비활성화하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)를 참조하세요.
</Update>
