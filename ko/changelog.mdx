...
<Update label="2026년 5월 21일" description="X API">
  ### 레거시 Basic 요금제, Pay-Per-Use로 전환

  오늘 레거시 **Basic** 요금제(월간 및 연간)의 지원 종료를 발표했습니다. **2026년 6월 1일** 이후, Basic 구독자는 다음 전환일에 [Pay-Per-Use(PPU)](https://docs.x.com/x-api/getting-started/about-x-api)로 자동 마이그레이션됩니다. 별도의 작업은 필요하지 않습니다.

  - **Basic 월간**: 전환 시점에 **$200**의 초기 크레딧이 청구되며, API 요청에 따라 차감됩니다.
  - **Basic 연간**: 남은 기간의 일할 계산 금액이 PPU 크레딧으로 지급되며, 일할 계산된 개월 수가 경과하면 만료됩니다(추가 카드 청구 없음).
  - **마이그레이션 후 PPU 기본 설정**: 잔액이 **$10** 아래로 떨어지면 **$200** 자동 충전이 활성화되며, 청구 주기당 지출 한도는 **$400**입니다. 설정은 [Developer Portal](https://developer.x.com/en/portal/dashboard)에서 언제든지 조정할 수 있습니다.

  옵트아웃하려면 2026년 6월 1일 전에 Developer Portal 설정을 업데이트하세요. 옵트아웃하면 전환일 이후 API 접근이 종료되며, 이 결정은 되돌릴 수 없습니다. 자세한 내용은 [여기](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)를 참조하세요.
</Update>

<Update label="2026년 5월 20일" description="X API">
  ### 수정: 무료 크레딧에서도 자동 충전이 작동

  오늘 바우처(무료 크레딧)에서 충전된 잔액이 0이 되었을 때 자동 충전이 트리거되지 않던 문제를 수정했습니다. **2026년 5월 21일**부터 자동 충전에서 무료 크레딧과 유료 크레딧이 동일하게 처리됩니다. 잔액이 모두 소진되고 자동 충전이 활성화되어 있다면, 저장된 결제 수단으로 계정이 자동 충전됩니다.

  무료 크레딧 소진 후 청구를 원하지 않는 경우, [Developer Console](https://console.x.com)에서 자동 충전 설정을 확인하고 비활성화하세요. 자세한 내용은 [여기](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)를 참조하세요.
</Update>
