...
## Introduction
 ...
 While the [recent search endpoint](https://developer.x.com/en/docs/x-api/posts/search/introduction) allows you to only get Posts published in the last 7 days, the user Post timeline and user mention timeline endpoints allow you to retrieve Posts and mentions that are older than the last 7 days, for an authorized user (using the user ID). Developers can use these endpoints to study topics, entities and sentiment of Posts from a user’s timeline or mentions. In this tutorial, we will show you how to explore a user’s Posts and mentions using the user Tweet timeline and user mention timeline endpoints.
 
...
*   You will also need a [Project](https://developer.x.com/en/docs/x-api/posts/search/introduction) created.
...
*   A Bearer Token from your App in the [X developer portal](https://developer-staging.x.com/en/docs/developer-portal/overview).
...
Give it a name, select the appropriate use-case, and provide a Project description. Next, you can either create a new App, or connect an existing [App](https://developer.x.com/en/docs/apps/overview) (an App is a container for your API keys that you need in order to make an HTTP request to the X API).
<Frame>
...
The user Tweet timeline and user mention timeline endpoints allow you to get Posts using the user ID. In order to get the user ID from a username, you can use the new [user lookup endpoint v2](https://developer.x.com/en/docs/x-api/users/lookup/quick-start). Replace the USER\_NAME with the username of your choice and XXXX with your own bearer token that you obtained above)
...
Once you have an understanding of how to navigate a user’s Posts, you can also use other APIs and services to do more with the Posts. Check out this tutorial that shows you [how to analyze the sentiments of your Posts](https://developer.x.com/en/docs/tutorials/how-to-analyze-the-sentiment-of-your-own-tweets). Below are some resources to keep handy when using the user Tweet timeline and user mention timeline endpoints.

## Resources

*   Learn more about the [user Tweet timeline and the user mention timeline endpoints](https://developer.x.com/en/docs/x-api/posts/timelines/introduction).
*   Check out the [API reference for the user Tweet timeline endpoint](https://developer.x.com/en/docs/x-api/posts/timelines/api-reference/get-users-id-tweets) to learn more about what’s available.
*   Check out the [API reference for the user mention timeline endpoint](https://developer.x.com/en/docs/x-api/posts/timelines/api-reference/get-users-id-mentions) to learn more about what’s available.
*   Get inspired by reading our other [tutorials](https://developer.x.com/en/docs/x-api/tutorials).
