---
    ...
    The SDK provides generic paginator utilities you can use with any list endpoint. Endpoints return plain responses; you wrap them with a paginator that:
    - accepts the server’s pagination token,
    - calls the endpoint with that token,
    - returns `{ data, meta, includes, errors }`.
    
    Key concepts
    - **Tokens**: Use `meta.next_token` (and `meta.previous_token` when available).
    - **Request options**:
      - `maxResults`, `paginationToken` (or `nextToken` for specific endpoints like `users.search`)
      - Field sets (when relevant): `userfields`, `tweetfields`, `mediafields`, `listfields`, `expansions`, `placefields`, `pollfields`
    - **Paginator types**:
      - `Paginator<T>`: generic
      - `UserPaginator`: exposes `.users`
      - `PostPaginator`: exposes `.posts`
      - `EventPaginator`: exposes `.events`
