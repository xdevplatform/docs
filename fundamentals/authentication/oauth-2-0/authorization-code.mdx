...
description: "Reference for the X OAuth 2.0 Authorization Code Flow with PKCE, covering scopes, refresh tokens, confidential clients, and access token lifetimes."
...
We support [authorization code](https://oauth.net/2/grant-types/authorization-code/) with [PKCE](https://oauth.net/2/pkce/) and [refresh token](https://oauth.net/2/grant-types/refresh-token/) as [grant types](https://oauth.net/2/grant-types/) for user authorization. Apps with existing OAuth 1.0a users can also use the [token exchange](https://datatracker.ietf.org/doc/html/rfc8693) grant to convert stored OAuth 1.0a access tokens into OAuth 2.0 tokens for the same users. See [Migrating OAuth 1.0a user tokens to OAuth 2.0](/fundamentals/authentication/oauth-2-0/oauth-1-0a-token-exchange).
