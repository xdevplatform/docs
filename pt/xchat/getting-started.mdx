...
| Componente | Papel |
|:-----------|:------|
| **[Chat XDK](/xchat/xchat-xdk)** | Criptografia, descriptografia, assinatura e armazenamento de chave privada (Juicebox ou blob de chave) |
...
## 2. Inicializar o Chat XDK

Carregue as chaves privadas com **Juicebox** (PIN + `juicebox_config` da API) ou um **blob de chave** (`import_keys`). Em seguida, defina sua versão de chave pública registrada (`public_key_version` no seu registro).
...
Amostras de servidor e bots normalmente usam um **blob de chave** (`export_keys` / `import_keys`). Apps cliente frequentemente usam **Juicebox** (`setup` / `unlock` com um PIN). Veja a referência do [Chat XDK](/xchat/xchat-xdk) para ambos os caminhos.

---

## 3. Registrar chaves públicas (uma vez)

Gere pares de chaves, registre as chaves **públicas** com o X, então armazene as chaves **privadas** (`setup` com um PIN, ou `export_keys`).
...
<Warning>
Use um PIN forte para o Juicebox. Perder o PIN ou um blob de chave desprotegido pode impedir a descriptografia de mensagens antigas.
</Warning>
...
- Não registre PINs, chaves privadas ou texto simples de mensagens em produção
- Em apps web, mantenha tokens OAuth (e a emissão do token do realm Juicebox) em um servidor; prefira manter chaves privadas apenas no Chat XDK do cliente
