...
3. **Mensagens são assinadas** para que os destinatários possam verificar quem as enviou e que nada foi alterado
...
| **Compartilhada entre participantes** | Todos os participantes que devem ler a thread têm uma cópia |
...
## Juicebox: armazenamento distribuído de chaves

Suas chaves **privadas** de identidade e de assinatura precisam ser armazenadas com cuidado. O X Chat integra o **Juicebox** para que as chaves possam ser recuperadas com um PIN entre dispositivos sem dar a nenhum servidor o segredo completo.
...
### Como o Juicebox resolve isso

O Juicebox combina **compartilhamento de segredos** com **proteção por PIN**:
...
4. A recuperação requer seu **PIN** e cooperação de **realms suficientes**
5. PINs incorretos têm **limite de taxa** para retardar tentativas
...
Você obtém recuperabilidade (novo dispositivo + PIN) sem que um único agente detenha o segredo inteiro.

<Note>
Você não configura os servidores Juicebox à mão no fluxo normal. O Chat XDK inclui o cliente Juicebox; a configuração do realm vem da X API como **`juicebox_config`** no seu registro de chave pública. O armazenamento inicial de PIN e o desbloqueio posterior são chamadas do Chat XDK — veja [Guia de introdução](/xchat/getting-started#2-initialize-the-chat-xdk) e [registrar chaves públicas](/xchat/getting-started#3-register-public-keys-one-time). Alguns apps (especialmente servidores e bots) usam um blob de chave exportado em vez do Juicebox; proteja esse material como uma senha.
</Note>
...
| **Roubo de chave em servidor único (com Juicebox)** | As partes são divididas entre realms e protegidas por PIN |
...
| **Realm** | Um servidor Juicebox independente que detém uma parte do seu material de chave |
