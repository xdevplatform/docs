...
description: Construye mensajería X Chat cifrada de extremo a extremo con el Chat XDK y la X API
...
| Componente | Función |
|:-----------|:--------|
| **[Chat XDK](/xchat/xchat-xdk)** | Cifrado, descifrado, firma y almacenamiento de claves privadas (Juicebox o un blob de claves) |
...
## 2. Inicializa el Chat XDK

Carga las claves privadas con **Juicebox** (PIN + `juicebox_config` de la API) o con un **blob de claves** (`import_keys`). Después establece la versión de tu clave pública registrada (`public_key_version` en tu registro).
...
Los ejemplos para servidor y bot suelen usar un **blob de claves** (`export_keys` / `import_keys`). Las apps cliente suelen usar **Juicebox** (`setup` / `unlock` con un PIN). Consulta la referencia del [Chat XDK](/xchat/xchat-xdk) para ambos caminos.

---

## 3. Registra las claves públicas (una sola vez)

Genera pares de claves, registra las claves **públicas** con X y luego guarda las claves **privadas** (`setup` con un PIN, o `export_keys`).
...
<Warning>
Usa un PIN fuerte para Juicebox. Perder el PIN o un blob de claves desprotegido puede impedir el descifrado de mensajes anteriores.
</Warning>
...
- No registres PINs, claves privadas ni el texto plano de mensajes en producción
- En apps web, mantén los tokens de OAuth (y la emisión de tokens de realm de Juicebox) en un servidor; procura mantener las claves privadas solo en el Chat XDK del cliente
