...
description: Comprende los conceptos de cifrado detrás de X Chat sin entrar en detalles de implementación
...
3. Los **mensajes se firman** para que los destinatarios puedan verificar quién los envió y que nada fue alterado
...
| **Compartida entre participantes** | Todos los participantes que deberían leer el hilo tienen una copia |
...
## Juicebox: almacenamiento distribuido de claves

Tus claves **privadas** de identidad y firma deben almacenarse con cuidado. X Chat integra **Juicebox** para que las claves puedan recuperarse con un PIN entre dispositivos sin darle a ningún servidor el secreto completo.
...
### Cómo lo resuelve Juicebox

Juicebox combina **secret sharing** con **protección por PIN**:
...
4. La recuperación requiere tu **PIN** y la cooperación de **suficientes realms**
5. Los PINs incorrectos están **limitados por tasa** para ralentizar los intentos
...
Obtienes recuperabilidad (nuevo dispositivo + PIN) sin que una sola parte tenga el secreto completo.

<Note>
No configuras servidores Juicebox a mano para el flujo normal. El Chat XDK incluye el cliente Juicebox; la configuración de realms proviene de la X API como **`juicebox_config`** en tu registro de clave pública. La configuración por primera vez del PIN y el desbloqueo posterior son llamadas del Chat XDK—consulta [Primeros pasos](/xchat/getting-started#2-initialize-the-chat-xdk) y [registrar claves públicas](/xchat/getting-started#3-register-public-keys-one-time). Algunas apps (especialmente servidores y bots) usan un blob de claves exportado en lugar de Juicebox; protege ese material como una contraseña.
</Note>
...
| **Robo de claves en un único servidor (con Juicebox)** | Los shares se dividen entre realms y se protegen con PIN |
...
| **Realm** | Un servidor Juicebox independiente que mantiene un share de tu material de claves |
