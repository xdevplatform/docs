...
<Update label="28 de junio de 2026" description="X Ads API">
  ### Optimización de presupuesto de campañas simplificada a `LINE_ITEM`

  Las campañas de la [Campaign Management API](/x-ads-api/campaign-management/reference) ahora solo admiten la optimización de presupuesto `LINE_ITEM`, alineándose con el comportamiento de X Ads Manager.

  - `budget_optimization` solo acepta `LINE_ITEM` en las solicitudes `POST` y `PUT` de campañas, y su valor predeterminado es `LINE_ITEM` (antes `CAMPAIGN`).
  - `standard_delivery` queda obsoleto a nivel de campaña. Configura la entrega estándar o acelerada en la línea de pedido mediante su parámetro [`standard_delivery`](/x-ads-api/campaign-management/reference).
</Update>
