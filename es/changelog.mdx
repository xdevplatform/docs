...
<Update label="21 de mayo de 2026" description="X API">
  ### Migración de los planes Basic heredados a Pay-Per-Use

  Hoy anunciamos que los planes **Basic** heredados de la X API (mensual y anual) quedan obsoletos, con migración automática a [Pay-Per-Use (PPU)](https://developer.x.com/en/portal/products) a partir del **1 de junio de 2026**. Después de tu fecha de transición (el final de tu ciclo de facturación actual), tu suscripción Basic finaliza y PPU toma el relevo sin cuota mensual fija.

  - **Suscriptores de Basic mensual**: se cargará un crédito inicial de **\$200** a tu método de pago vía Stripe.
  - **Suscriptores de Basic anual**: el valor prorrateado restante se otorga como créditos de PPU (sin cargo a la tarjeta en la transición).
  - **Ajustes predeterminados de PPU**: recarga automática habilitada en **\$200** cuando el saldo cae por debajo de **\$10**, con un límite de gasto de **\$400** por ciclo. Ajústalo en cualquier momento en el [Developer Portal](https://developer.x.com/en/portal/dashboard).

  Si no quieres migrar, puedes optar por no participar en el Developer Portal antes del 1 de junio de 2026 — ten en cuenta que optar por no participar finaliza el acceso a la API tras tu fecha de transición. Más información [aquí](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305).
</Update>

<Update label="20 de mayo de 2026" description="X API">
  ### Corrección: la recarga automática ahora se activa con los créditos gratuitos

  Hoy lanzamos una corrección para la función de recarga automática, de modo que los créditos gratuitos (de vales canjeados) se traten igual que los créditos pagados. Antes, cuando el saldo de créditos gratuitos llegaba a cero, la recarga automática no se activaba aunque estuviera habilitada. A partir del **21 de mayo de 2026**, si tu saldo se agota y la recarga automática está activada en el [Developer Console](https://console.x.com), tu cuenta se recargará automáticamente con el método de pago guardado.

  Si no quieres incurrir en cargos después de que se agoten tus créditos gratuitos, revisa el ajuste de recarga automática en el Developer Console y desactívalo si es necesario. Más información [aquí](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184).
</Update>
