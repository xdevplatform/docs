...
<Update label="30 de junio de 2026" description="X API v2">
  ### Servidores MCP para la X API y la documentación de X

  Hoy lanzamos dos servidores [Model Context Protocol](/tools/mcp) alojados para que herramientas de IA como Grok, Cursor y Claude puedan trabajar con X de forma nativa:

  - **X MCP** (`https://api.x.com/mcp`) — llama a endpoints de la X API desde cualquier cliente compatible con MCP. Sus capacidades abarcan Posts, búsqueda en archivo completo (full‑archive Search), Users, Bookmarks, News & Trends y Articles (borrador y publicación). Conéctate mediante el puente de código abierto [`xurl mcp`](/tools/mcp#x-mcp-—-x-api), que se encarga por ti del inicio de sesión OAuth 2.0 y de la renovación de tokens.
  - **Docs MCP** (`https://docs.x.com/mcp`) — permite que tu asistente de IA [busque y lea la documentación de la X API](/tools/mcp#docs-mcp-—-documentation-search) en línea mediante las herramientas `search_x` y `get_page_x`.

  Puedes ejecutar ambos servidores juntos para consultar la documentación *y* llamar a la API desde el mismo asistente. Consulta la [guía de MCP](/tools/mcp) para ver configuraciones de cliente (Grok Build, Cursor, Claude, VS Code), la configuración sin interfaz (headless) y resolución de problemas.
</Update>
