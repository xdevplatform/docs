...
<Update label="8 de junio de 2026" description="X API">
  ### Controles de precios de pago por uso y niveles de créditos de xAI

  Hoy ampliamos la [página de precios de X API](/x-api/getting-started/pricing) con más detalles sobre la facturación basada en créditos y nuevos controles en la [Consola de desarrolladores](https://console.x.com):

  - **Recarga automática**: recarga tu saldo de créditos automáticamente cuando baja del umbral que definas.
  - **Límites de gasto**: limita cuánto puedes gastar por ciclo de facturación para evitar cargos inesperados.
  - **Deduplicación aclarada**: el mismo recurso (p. ej., un Post) solicitado más de una vez en una ventana de 24 horas UTC se cobra una sola vez.
  - **Niveles de créditos gratuitos de xAI API**: obtén hasta un **20% de retorno** en créditos de [xAI API](https://docs.x.ai) según tu gasto acumulado en X API por ciclo de facturación: **10%** a partir de \$200, **15%** a partir de \$500, **20%** a partir de \$1,000. Vincula tu equipo de xAI en la Consola de desarrolladores para recibir los créditos.

  Las tarifas por endpoint de lectura y escritura, así como el precio de \$0.001 de [Owned Reads](/x-api/getting-started/pricing#owned-reads), también están ahora documentados en una única tabla de tarifas.
</Update>
