...
<Update label="18 de junio de 2026" description="X API">
  ### Actualizaciones de la página de precios: protecciones de recarga automática, precios de eventos de webhook y niveles de créditos de xAI más claros

  Hoy actualizamos la [página de precios de la X API](/x-api/getting-started/pricing) con varias aclaraciones para los usuarios:

  - **Protecciones de recarga automática**: la recarga automática ahora se activa como máximo una vez cada 5 minutos y se pausa cuando tu saldo es cero o negativo, protegiéndote de cargos descontrolados.
  - **Precios de eventos de webhook**: se añadió una tabla de precios por evento para los eventos de la [X Activity API](/x-api/activity/introduction) (p. ej., `post.create`, `follow.follow`, `dm.received`, `spaces.start`), deduplicados dentro de la misma ventana UTC de 24 horas que otros recursos.
  - **Niveles de créditos gratuitos de la xAI API**: se aclararon los niveles de recompensa por gasto acumulado (10 % a partir de &dollar;200, 15 % a partir de &dollar;500, 20 % a partir de &dollar;1,000) con un ejemplo práctico para que veas exactamente lo que recibirás por cada compra.
  - **Comparativa con Enterprise**: la [página de precios Enterprise](/enterprise-api/getting-started/pricing) ahora indica que los planes de pago por uso incluyen un límite mensual de 2 millones de lecturas de Posts, mientras que los planes Enterprise ofrecen volúmenes personalizados o ilimitados.
</Update>
