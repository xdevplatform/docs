...
<Update label="19 de junio de 2026" description="X Ads API">
  ### Ads API Analytics: eliminación de placements, grupos de métricas y tipos de entidad no admitidos

  Hoy actualizamos el endpoint de [Ads API Analytics](/x-ads-api/analytics) para reflejar lo que actualmente se admite en producción. Si tu integración hace referencia a alguno de los valores a continuación, actualiza tus solicitudes en consecuencia:

  - **Placements:** `PUBLISHER_NETWORK` ya no es un placement válido para solicitudes de analítica (solo permanece visible en las respuestas de Active Entities).
  - **Grupos de métricas:** se eliminó el grupo de métricas `MEDIA`, junto con las métricas derivadas de Media View Rate en `ENGAGEMENT` y `FOLLOWERS`.
  - **Tipos de entidad:** `MEDIA_CREATIVE` y `ORGANIC_TWEET` ya no son tipos de entidad admitidos para analytics ni sync analytics.
  - **Objetivos:** se eliminaron las secciones de los objetivos `APP_ENGAGEMENTS` y `LEAD_GENERATION` (tanto Metrics by Objective como Derived Metrics).
  - **Segmentación:** la segmentación solo está admitida para `AGE`, `GENDER`, `METROS` y `PLATFORMS`.
</Update>
