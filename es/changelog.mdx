...
<Update label="21 de julio de 2026" description="X API v2">
  ### Deprecación de la Account Activity API

  Hoy anunciamos la deprecación de la [Account Activity API (AAA)](/x-api/account-activity/introduction). A partir de ahora, usa la [X Activity API (XAA)](/x-api/activity/introduction) para la entrega de actividad de usuario en tiempo real: cubre los mismos tipos de eventos (Posts, Direct Messages, Likes, Follows, Spaces y más) a través de una canalización de menor latencia, con un único modelo de suscripción por ID de usuario.

  Las integraciones existentes de AAA seguirán funcionando durante la ventana de deprecación. Planifica la migración a XAA para evitar interrupciones.
</Update>
