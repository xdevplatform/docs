...
<Update label="30 de junio de 2026" description="Herramientas para desarrolladores">
  ### Servidores MCP alojados para la X API y la documentación

  Hoy lanzamos dos [servidores de Model Context Protocol](/tools/mcp) alojados para que herramientas de IA como Grok, Cursor, Claude Desktop y VS Code se comuniquen directamente con X:

  - **X MCP** (`https://api.x.com/mcp`): llama a endpoints de la X API desde cualquier cliente compatible con MCP. El modelo puede buscar en el archivo completo, consultar usuarios, gestionar bookmarks, obtener trends y noticias, y crear borradores o publicar Articles usando los permisos de tu propia cuenta. Conéctate mediante el puente de código abierto `xurl mcp`, que se encarga de OAuth 2.0 y de la renovación de tokens por ti.
  - **Docs MCP** (`https://docs.x.com/mcp`): permite que tu asistente de IA busque y lea la documentación de la X API sin salir de tu editor.

  Consulta la [guía de configuración de MCP](/tools/mcp) para ver instrucciones específicas para Grok Build, Cursor, Claude Desktop, VS Code y cualquier otro cliente MCP.
</Update>
