...
<Update label="30 de junio de 2026" description="Plataforma para desarrolladores">
  ...
  Hoy lanzamos dos servidores [Model Context Protocol](/tools/mcp) alojados para que herramientas de IA como Grok, Cursor, Claude y VS Code trabajen directamente con X:

  - **X MCP** en `https://api.x.com/mcp`: llama a los endpoints de la X API (búsqueda de Posts y usuarios, búsqueda en el archivo completo, bookmarks, trends, noticias, borradores y publicación de Articles) con los permisos de tu propia cuenta de X. Conéctate mediante el puente de código abierto [`xurl mcp`](https://github.com/xdevplatform/xurl), que gestiona OAuth y el refresco del token por ti.
  - **Docs MCP** en `https://docs.x.com/mcp`: busca y lee la documentación de la X API desde tu herramienta de IA, sin necesidad de autenticación.

  Consulta la [guía de configuración de MCP](/tools/mcp) para ver configuraciones de cliente (Grok Build, Cursor, Claude, VS Code), inicio de sesión sin navegador y solución de problemas.
</Update>
