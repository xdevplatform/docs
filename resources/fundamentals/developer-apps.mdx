...
If you have existing Apps, you can view, edit, or delete them via the [developer portal's App page](https://developer.twitter.com/en/portal/projects-and-apps).

Accessing the X API and X Ads API requires a set of [authentication](https://developer.twitter.com/en/docs/authentication) credentials, also known as keys and tokens, that you must pass with each request. These credentials can come in different forms depending on the type of authentication that is required by the specific endpoint that you are using.

Here are the different credentials that you can generate in your App and how to use them:

- **[API Key and Secret](https://developer.twitter.com/content/developer-twitter/resources/fundamentals/authentication/api-key-and-secret):** Essentially the username and password for your App. You will use these to authenticate requests that require [OAuth 1.0a User Context](https://developer.twitter.com/content/developer-twitter/resources/fundamentals/authentication), or to generate other tokens such as user Access Tokens or App Access Token.

- **[Access Token and Secret](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens):** In general, Access Tokens represent the user that you are making the request on behalf of. The ones that you can generate via the developer portal represent the user that owns the App. You will use these to authenticate requests that require [OAuth 1.0a User Context](https://developer.twitter.com/content/developer-twitter/resources/fundamentals/authentication). If you would like to make requests on behalf of another user, you will need to use the 3-legged OAuth flow for them to authorize you.
...
- **[App only Access Token](https://developer.twitter.com/content/developer-twitter/en/docs/authentication/oauth-2-0/bearer-tokens):** You will use this token when making requests to endpoints that responds with information publicly available on X.

In addition to generating the keys and tokens necessary to make X API requests, you will also be able to set access [permissions](https://developer.twitter.com/en/docs/apps/app-permissions.html), document the use case or purpose for the App, define a [callback URL](https://developer.twitter.com/en/docs/apps/callback-urls), and modify other settings related to your App developer environment from within the [management dashboard](https://developer.twitter.com/en/portal/projects-and-apps).

### Apps and Projects

You can use Apps and [Projects](https://developer.twitter.com/en/docs/projects) to help organize your work with the X Developer Platform by use case. Each Project can contain a single App if you have Essential access, and up to three Apps if you have Elevated or greater access.

If you would like to access the new [X API v2](https://developer.twitter.com/en/docs/twitter-api) endpoints, you will be required to use keys and tokens from an App that is associated with a Project.
...
You can [visit the dashboard](https://developer.twitter.com/content/developer-twitter/en/apps) to manage the Apps associated with your account. To learn more, please visit our documentation page on the [developer portal](https://developer.twitter.com/en/docs/developer-portal/dev-portal-enhancements.html). The dashboard allows developers to quickly and easily perform the following tasks:
...
- Review or update a specific App's settings, including updating name, description, website, callback URL, and [permissions](https://developer.twitter.com/en/docs/apps/app-permissions.html).
...
If you have existing X Apps, you can view and edit your Apps via the X [App dashboard](https://developer.twitter.com/content/developer-twitter/en/portal/projects-and-apps) if you are logged into your X account on developer.x.com. Please note you will **not** need to sign up for an account to manage any and all Apps that were previously created on developer.x.com.

If you need to create a new X App, you will need to have [signed up for a developer account](https://developer.twitter.com/en/portal/petition/essential/basic-info). If you are a member of a [team account](https://developer.twitter.com/en/docs/developer-portal/overview.html#team), you must be assigned an admin role to create a new App.
...
The [Twitter App dashboard](https://developer.twitter.com/en/portal/projects-and-apps) allows developers to quickly and easily perform the following tasks:

- View your existing Apps and [Projects](https://developer.twitter.com/en/docs/projects) and their associated App ID.
...
Here you can edit the App icon, App name, App description, your website URL, [callback URLs/redirect URIs](https://developer.twitter.com/en/docs/apps/callback-urls), terms of service URL, privacy policy URL, organization name, organization URL, and purpose or use case of the App.
...
If your application has a tag showing 'suspended' this is because your app is in violation of one or more of Twitter's [developer terms](https://developer.twitter.com/en/developer-terms.html) such as our [automation rules](https://help.twitter.com/en/rules-and-policies/twitter-automation). The developer platform policy team will communicate with developers through the email address set up on the App owner's Twitter account, to review this email address please review your [Twitter account settings](https://twitter.com/settings/your_twitter_data/account). Notification emails about suspensions will be sent to this email address with the title similar to "Application suspension notice" and will have specific information on what to do next.  To work with the Twitter team to address suspensions, please check your email for specific instructions, or use our [platform help form](https://help.twitter.com/forms/platform).

#### Keys and tokens

Inside of an App in a Project or via a standalone App you can view, regenerate, or revoke the following tokens:

- [API Key (Consumer Key) and API Secret (Consumer Secret)](https://developer.twitter.com/resources/fundamentals/authentication/api-key-and-secret)
- [App Access Token](https://developer.twitter.com/en/docs/authentication/oauth-2-0/bearer-tokens)
- [User Access Token and Access Token Secret](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens) -
The Access Token and Secret available within the developer portal relates to the user that owns the App.

**Please note** - If you would like to make requests on behalf of a different user (in other words, not the user that owns the App), you will have to use either the [OAuth 1.0a 3-legged OAuth flow](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens) or OAuth 2.0 [Authorization Code with PKCE flow](https://developer.twitter.com/content/developer-twitter/en/docs/authentication/oauth-2-0/user-access-token) to generate a set of user Access Tokens. You will then use these user-specific tokens in your request to the API.
...
App permissions describe the access level for OAuth 1.0a application-user authentication. App permissions are configured per application within your [Twitter App](https://developer.twitter.com/en/docs/apps.html) settings.
...
- [POST /2/dm_conversations/:dm_conversation_id/messages](https://developer.twitter.com/en/docs/twitter-api/direct-messages/manage/api-reference/post-dm_conversations-dm_conversation_id-messages)
- [POST /2/dm_conversations/](https://developer.twitter.com/en/docs/twitter-api/direct-messages/manage/api-reference/post-dm_conversations)
- [POST /2/dm_conversations/with/:participant_id/messages](https://developer.twitter.com/en/docs/twitter-api/direct-messages/manage/api-reference/post-dm_conversations-with-participant_id-messages)
- [GET /2/dm_conversations/with/:user_id/dm_events](https://developer.twitter.com/en/docs/twitter-api/direct-messages/lookup/api-reference/get-dm_conversations-with-participant_id-dm_events)
...
- [GET /2/dm_events](https://developer.twitter.com/x-api/direct-messages/direct-messages-lookup#get-2-dm-events)

### Additional: Request email address

This additional permission may be combined with any of the other levels. When authorizing a [Twitter App](https://developer.twitter.com/en/docs/apps.html), the user will also be informed that the app may request visibility of any email address associated with the account (via the [account/verify_credentials](https://developer.twitter.com/en/docs/twitter-api/v1/accounts-and-users/manage-account-settings/api-reference/get-account-verify_credentials.html) endpoint). To use this permission, the app settings must be configured to point to valid privacy policy and terms of service URLs via the app's settings so that users understand the terms under which their email address may be used and stored.
...
- [OAuth 1.0a 3-legged OAuth flow](https://developer.twitter.com/resources/fundamentals/authentication/obtaining-user-access-tokens) (and separately, the [Sign in with Twitter flow](https://developer.twitter.com/en/docs/authentication/guides/log-in-with-twitter))
...
When setting up these flows for their potential users to work through, developers must pass a callback URL with their requests to the authentication endpoints that make up the flows mentioned earlier. For example, developers using OAuth 1.0a User Context must pass the `callback_url` parameter when making a request to the [GET oauth/request_token](https://developer.twitter.com/en/docs/authentication/api-reference/request_token) endpoint. Similarly, developers using OAuth 2.0 Authorization Code with PKCE must pass the `redirect_uri` parameter with their request to the GET oauth2/authorize endpoint.

In addition to using these parameters, the developer must also make sure that the callback URL has also been added to their [App's](https://developer.twitter.com/en/docs/apps) callback URL allowlist, which can be found on the [developer portal's](https://developer.twitter.com/en/docs/developer-portal) App settings page.
