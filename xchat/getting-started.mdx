...
<Tabs>
  ...
  <Tab title="TypeScript">
    ```bash
    # Install the XDK and Chat XDK
    npm install @xdevplatform/xdk chat-xdk
    ```
  </Tab>
</Tabs>
...
<Tabs>
  <Tab title="Python">
    ...
    def send_message(conversation_id: str, text: str):
        ...
        # Get the conversation to retrieve key events
        conversation = x_client.chat.get_conversation(conversation_id)
        
        # Key events are in meta.conversation_key_events (list of base64 strings)
        # The most recent key event contains the current encryption key
        key_events = conversation.meta.conversation_key_events
        
        # Encrypt the message using the Chat XDK
        # Pass the latest key event — the XDK extracts the encrypted
        # conversation key for the authenticated user internally
        send_payload = chat.encrypt_message(
            text=text,
            conversation_key=key_events[-1],  # Most recent key event (base64)
        ...
        response = x_client.chat.send_message(
            ...
            body={
                ...
                "encoded_message_create_event": send_payload.encoded_message_create_event,
                "encoded_message_event_signature": send_payload.encoded_message_event_signature,
            }
        )
        
        print(f"Message sent! Encoded event: {response.data.encoded_message_event}")
        return response
  ...
  <Tab title="TypeScript">
    ...
    async function sendMessage(conversationId: string, text: string) {
      ...
      // Key events are in meta.conversation_key_events (array of base64 strings)
      // The most recent key event contains the current encryption key
      const keyEvents = conversation.meta.conversationKeyEvents;
      ...
      const sendPayload = chat.encryptMessage({
        text,
        conversationKey: keyEvents[keyEvents.length - 1], // Most recent key event (base64)
        conversationId,
      ...
      const response = await xClient.chat.sendMessage(conversationId, {
        body: {
          ...
          encodedMessageCreateEvent: sendPayload.encodedMessageCreateEvent,
          encodedMessageEventSignature: sendPayload.encodedMessageEventSignature,
        }
      });
      
      console.log('Message sent! Encoded event:', response.data.encodedMessageEvent);
      return response;
    }
...
<Tabs>
  <Tab title="Python">
    ...
    def send_reply(conversation_id: str, text: str):
        ...
        # Get the latest conversation key events
        conv = x_client.chat.get_conversation(conversation_id)
        key_events = conv.meta.conversation_key_events
