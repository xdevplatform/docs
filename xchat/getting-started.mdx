...
<Tabs>
  ...
  <Tab title="Go">
    ...
    // prepared.ConversationKey is the raw []byte key — pass to EncryptMessage
    _ = resp
    ```
  </Tab>
  <Tab title="C#">
...
Encrypt with the **raw** conversation key (raw bytes in every binding, including Go). On the send request, map:
...
<Tabs>
  ...
  <Tab title="Go">
    ...
    payload, err := chat.EncryptMessage(chatxdk.EncryptMessageParams{
        ...
        ConversationKey:        convKey, // raw []byte from prepared.ConversationKey
        Text:                   "Hello!",
