...
- Store every participant's public keys once with `set_signing_keys` (map API fields into `SigningKeyEntry`; see [Chat XDK](/xchat/xchat-xdk)) so decrypt calls verify signatures without per-call arguments
- Enable `set_cache_keys(true)` (off by default) so `decrypt_events` retains each conversation's latest **signature-verified** key, letting `decrypt_event` and the encrypt methods omit key arguments
