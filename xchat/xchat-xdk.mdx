...
For a new conversation (or rotation), **prefer** `prepare_conversation_keys`: it generates a conversation key and wraps a copy for each participant’s identity public key. Post those wraps to the initialize-keys (or key-change) API—rename SDK field `encrypted_key` to **`encrypted_conversation_key`**. Keep the **raw** key bytes (Go: often base64 of those bytes) for `encrypt_message` and media; never pass the API’s encrypted envelope into encrypt.

Use `extract_conversation_keys` on key-change event payloads to rebuild `{ keys, latest_version }`. `decrypt_conversation_key` unwraps a single ECIES blob; `encrypt_conversation_key_for_recipients` re-wraps an existing key for many users.

<Tabs>
  <Tab title="Python">
    ```python
    prepared = chat.prepare_conversation_keys(participants)
    # prepared["conversation_key"]  — raw bytes for encrypt_message
    # prepared["encrypted_keys"]    — per-user wraps; rename encrypted_key → encrypted_conversation_key on POST
    ...
    one = chat.decrypt_conversation_key(encrypted_blob, sender_identity_public_key)
    ```
  </Tab>
  <Tab title="TypeScript">
    ```typescript
    const prepared = chat.prepareConversationKeys(participants);
    // prepared.conversationKey — Uint8Array for encryptMessage
    // prepared.encryptedKeys — POST with encryptedConversationKey field names

    const extracted = chat.extractConversationKeys(keyChangeBlobs);
    const raw = extracted.keys[extracted.latestVersion!];

    const one = chat.decryptConversationKey(encryptedBlob, senderIdentityPublicKey);
    ```
  </Tab>
  <Tab title="Rust">
    ```rust
    let prepared = chat.prepare_conversation_keys(&participants)?;
    let extracted = chat.extract_conversation_keys(&key_change_blobs)?;
    let raw = &extracted.keys[&extracted.latest_version];
    let one = chat.decrypt_conversation_key(&encrypted_blob, &sender_identity_pk)?;
    ```
  </Tab>
  <Tab title="Go">
    ```go
    prepared, err := chat.PrepareConversationKeys(participants)
    // prepared.ConversationKey is typically base64 of the raw key for later EncryptMessage
    extracted, err := chat.ExtractConversationKeys(keyChangeBlobs)
    one, err := chat.DecryptConversationKey(encryptedBlob, senderIdentityPK)
    _ = prepared
    _ = extracted
    _ = one
    _ = err
  ...
  <Tab title="C#">
    ```csharp
    var prepared = chat.PrepareConversationKeys(participants);
    var extracted = chat.ExtractConversationKeys(keyChangeBlobs);
    var raw = extracted.Keys[extracted.LatestVersion];
    var one = chat.DecryptConversationKey(encryptedBlob, senderIdentityPublicKey);
    ```
  </Tab>
  <Tab title="Java">
    ```java
    PreparedConversationKeys prepared = chat.prepareConversationKeys(participants);
    ExtractedConversationKeys extracted = chat.extractConversationKeys(keyChangeBlobs);
    byte[] raw = extracted.keys.get(extracted.latestVersion);
    byte[] one = chat.decryptConversationKey(encryptedBlob, senderIdentityPublicKey);
    ```
  </Tab>
</Tabs>
...
Pass **signing keys** so the SDK can verify senders. Map API public-key fields into `SigningKeyEntry`: `version` → `public_key_version`, `signing_public_key` → `public_key`, `public_key` → `identity_public_key`, plus `identity_public_key_signature` and `user_id`. Omit or pass an empty list to skip verification (not recommended in production).
...
<Tabs>
  <Tab title="Python">
    ...
    signing_keys = [{
        "user_id": uid,
        "public_key_version": row["version"],
...
- **PreparedConversationKeys** — output of `prepare_conversation_keys` (raw key + per-participant wraps).
