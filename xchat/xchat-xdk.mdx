...
Load keys, set the session once, decrypt a backlog, decrypt one live event, encrypt a message. Wire the send body to [`POST /2/chat/conversations/{id}/messages`](/x-api/chat/send-chat-message) as in [Getting Started](/xchat/getting-started).
...
<Tabs>
  <Tab title="Python">
    ...
    # Backlog: omitted signing keys fall back to the store
    ...
    # Live event: omitted arguments resolve from the key cache and store
  ...
  <Tab title="Rust">
    ...
    // Backlog: an empty slice falls back to the store
    ...
    // Live event: empty arguments resolve from the key cache and store
  ...
  <Tab title="Go">
    ...
    // Backlog: nil signing keys fall back to the store
    ...
    // Live event: nil arguments resolve from the key cache and store
  ...
  <Tab title="Java">
    ...
    try (Chat chat = new Chat()) {
        ...
        // Backlog: null signing keys fall back to the store
        ...
        // Live event: null arguments resolve from the key cache and store
...
Optional: signature verification is **on by default** (`reject_unverified = true`)—call `set_reject_unverified(false)` to disable it (not recommended); `set_key_version` sets just the registered key version when you don't want session identity defaults; `update_config` if backup realm config changes; `is_unlocked` / `has_identity_key` for UI state. Full field lists live in the [chat-xdk repo](https://github.com/xdevplatform/chat-xdk) stubs.
