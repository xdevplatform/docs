---
title: Introduction to XChat API
sidebarTitle: Introduction
description: Build end-to-end encrypted messaging applications with the XChat API
keywords: ["XChat API", "encrypted DM", "end-to-end encryption", "E2EE", "secure messaging", "chat API", "direct messages", "encrypted chat"]
---

import { Button } from '/snippets/button.mdx';

The XChat API enables developers to build applications that send and receive **end-to-end encrypted direct messages** on X. Unlike standard DMs, XChat messages are encrypted on the sender's device and can only be decrypted by the intended recipient — not even X can read the message contents.
...
XChat enables a wide range of secure messaging applications:
...
Building with XChat involves three main components working together:

```mermaid
flowchart LR
    subgraph Your App
        A[XChat XDK<br/>Encryption/Decryption]
...
|:----------|:--------|:----|
| **XChat XDK** | Handles all cryptographic operations — encryption, decryption, signing, verification | Rust, Python, JavaScript/WASM |
...
<Steps>
  ...
  <Step title="Encrypt outgoing messages">
    When sending a message, the XChat XDK encrypts the content using the conversation key and signs it with your signing key.
  </Step>
  ...
  <Step title="Decrypt and verify">
    The XChat XDK decrypts the message using the conversation key and verifies the sender's signature.
  </Step>
</Steps>
...
XChat uses **end-to-end encryption**, meaning messages are encrypted on your device before being sent to X's servers. Only the intended recipient(s) can decrypt the message — X cannot read the content.
...
These events deliver encrypted message payloads that you decrypt using the XChat XDK.

---

## API endpoints

XChat uses the `/2/chat/*` family of endpoints, plus public key endpoints under `/2/users/`:
...
XChat endpoints require **user context authentication**:
...
<CardGroup cols={3}>
  <Card title="XChat XDK" icon="lock" href="/xchat/xchat-xdk">
...
<CardGroup cols={2}>
  <Card title="Cryptography Primer" icon="key" href="/xchat/cryptography-primer">
    Understand the encryption concepts behind XChat
  </Card>
...
Before building with XChat, you'll need:
