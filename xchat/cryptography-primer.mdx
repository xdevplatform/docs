...
description: Understand the encryption concepts behind XChat without diving into implementation details
keywords: ["XChat cryptography", "E2EE primer", "encryption basics", "public key encryption", "ECDH", "digital signatures", "conversation keys"]
---

import { Button } from '/snippets/button.mdx';

This primer explains the cryptographic concepts behind XChat at a conceptual level. You don't need to understand the technical details to build with XChat — the SDKs handle all cryptographic operations for you — but understanding the concepts helps you make better architectural decisions.

<Note>
**You don't need to implement any of this yourself.** The XChat XDK handles all encryption, decryption, key management, and signatures. This document is for understanding, not implementation.
</Note>
...
XChat uses a layered encryption system where:
...
XChat uses three types of keys, each with a specific purpose:
...
<Steps>
  ...
  <Step title="Encrypt the message">
    The XChat XDK encrypts your message using the conversation key. The result is random-looking bytes that can only be decrypted with the same key.
  </Step>
  <Step title="Sign the message">
    The XChat XDK signs the encrypted message with your signing private key. This creates a digital signature that proves you authored this exact message.
  </Step>
...
<Steps>
  ...
  <Step title="Verify the signature">
    The XChat XDK verifies the signature using the sender's signing public key. This confirms the message came from the claimed sender and wasn't tampered with.
  </Step>
  <Step title="Decrypt the message">
    The XChat XDK decrypts the message using the conversation key. You can now read: "Hello, how are you?"
  </Step>
</Steps>
...
Your private keys (identity and signing) need to be stored securely. XChat uses **Juicebox** for this.
...
Every XChat message includes a **digital signature** that proves:
...
When you receive a message, the XChat XDK automatically:
...
### What XChat does NOT protect against
...
<CardGroup cols={2}>
  <Card title="Getting Started" icon="rocket" href="/xchat/getting-started">
    Build your first XChat application step-by-step
  </Card>
