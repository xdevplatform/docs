...
Your **private** identity and signing keys must be stored carefully. X Chat includes a **secure key backup** system so keys can be recovered with a passcode across devices without giving any single server the full secret.
...
| **Asymmetric encryption** | Different keys for encrypt vs decrypt (used to exchange conversation keys) |
...
| **ECDH / ECIES** | Algorithms used when exchanging conversation keys via identity keys |
