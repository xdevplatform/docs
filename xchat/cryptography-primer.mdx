...
<Note>
In practice you don't configure Juicebox by hand. The Chat XDK embeds the Juicebox client, and the realm configuration it needs is returned by the X API's public-key endpoints (the `juicebox_config` field). See [Getting Started](/xchat/getting-started#step-2-fetch-your-juicebox-config-and-initialize-the-chat-xdk).
</Note>
