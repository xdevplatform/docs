...
description: Learn the ECDH, public key encryption, and digital signature concepts behind X Chat end-to-end encryption without implementation details.
...
3. **Messages are signed** with the **signing key** so recipients can verify who sent them and that nothing was altered
...
Public halves are registered and discovered through the platform’s **public-key** APIs (see Encryption keys under API reference). Private halves stay in the Chat XDK (for example via [secure key backup](#secure-key-backup-distributed-key-storage) or a carefully protected key blob).
...
| **Shared among participants** | All participants who should read the conversation have a copy |
...
Your **private** identity and signing keys must be stored carefully. X Chat includes a **secure key backup** system (implemented with Juicebox) so keys can be recovered with a passcode across devices without giving any single server the full secret.
...
Secure key backup combines **secret sharing** with **passcode protection**:
...
4. Recovery requires your **passcode** and cooperation from **enough realms**
5. Wrong passcodes are **rate-limited** to slow guessing
...
You get recoverability (new device + passcode) without a single party holding the whole secret.

<Note>
You do not configure key backup servers by hand for the normal path. The Chat XDK includes the backup client; realm configuration comes from the X API as **`juicebox_config`** on your public-key record (the field is named for Juicebox, the underlying implementation). First-time passcode storage and later unlock are Chat XDK calls—see [initialize with existing keys](/xchat/getting-started#2-initialize-the-chat-xdk-with-existing-keys) and [create and register keys](/xchat/getting-started#3-create-and-register-keys-first-time-setup) in Getting Started. Some apps (especially servers and bots) use an exported key blob instead of secure key backup; protect that material like a password.
</Note>
...
| **Single-server key theft (with secure key backup)** | Shares are split across realms and passcode-gated |
...
| **Realm** | An independent secure key backup server holding one share of your key material |
