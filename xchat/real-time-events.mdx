...
When a user sends or receives an encrypted DM, X can deliver the event to your webhook endpoint within milliseconds. The event payload contains the encrypted message, which you decrypt using the [XChat XDK](/xchat/xchat-xdk).

```mermaid actions={false}
flowchart LR
    A["User sends<br/>encrypted DM"] --> B["X Activity API"] --> C["Your webhook"] --> D["XChat XDK<br/>decrypts message"]
```
...
When a chat event arrives, decrypt it using the XChat XDK:

<CodeGroup>
```python Python (Flask)
from xchat_xdk import XChatClient
...
```typescript TypeScript (Express)
import { XChatClient } from 'xchat-xdk';

const xchat = new XChatClient();
...
| `encrypted_payload` | Base64-encoded encrypted message (decrypt with XChat XDK) |
