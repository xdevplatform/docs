...
Set the session stores once—signing keys for all participants (`set_signing_keys`) and the opt-in conversation-key cache (`set_cache_keys`)—then decrypt each live event with `decrypt_event`; the omitted key arguments resolve from the stores. When a payload carries `conversation_key_change_event`, run it through `decrypt_events` first: that verifies the key change and, with caching on, retains its key for the `decrypt_event` call.
