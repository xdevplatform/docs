...
<Tabs>
  ...
  <Tab title="Go">
    ...
    _ = mime
    encrypted, err := chat.EncryptStream(plaintext, rawConvKey) // both raw []byte
    _ = dims
    _ = encrypted
    ```
  </Tab>
...
<Tabs>
  ...
  <Tab title="Go">
    ...
    payload, err := chat.EncryptMessage(chatxdk.EncryptMessageParams{
        ...
        ConversationKey: rawConvKey, Text: caption, // rawConvKey is raw []byte
...
<Tabs>
  ...
  <Tab title="Go">
    ...
    // read body into []byte → chat.DecryptStream(encryptedBlob, rawConvKey) — both raw []byte
    _ = resp
    _ = err
    ```
  </Tab>
