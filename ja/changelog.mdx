...
<Update label="2026 年 6 月 17 日" description="X API">
  ### 自動チャージのセーフガード

  [X API 従量課金プラン](/x-api/getting-started/pricing#auto-recharge)の自動チャージに、想定外の高額請求や暴走を防ぐための 2 つの組み込み制限が追加されました。

  - **5 分間に 1 回のチャージ。** 自動課金は 5 分ごとに最大 1 回までしかトリガーされないため、利用量が急増しても複数回のチャージが連続して積み重なることはありません。
  - **残高がゼロまたはマイナスの場合は一時停止。** アカウント残高がゼロまたはマイナスの間は自動チャージは実行されません。手動でクレジットを追加すると、自動チャージが再開されます。
