...
<Update label="2026 年 5 月 21 日" description="X API">
  ### 従来の Basic プランを Pay-Per-Use に移行

  本日、X API の従来の **Basic** プラン（月額および年額）を非推奨化し、**2026 年 6 月 1 日**より [Pay-Per-Use（PPU）](https://developer.x.com/en/portal/products) への自動移行を開始することを発表しました。切り替え日（現在の請求サイクルの終わり）を過ぎると、Basic サブスクリプションは終了し、月額固定料金なしで PPU が引き継がれます。

  - **Basic 月額サブスクライバー**: Stripe を通じて、お支払い方法に **\$200** の初期クレジットが請求されます。
  - **Basic 年額サブスクライバー**: 年額料金の残りの日割り額が PPU クレジットとして付与されます（切り替え時にカードへの請求はありません）。
  - **移行後の PPU デフォルト設定**: 残高が **\$10** を下回ったときに **\$200** で自動チャージが有効、サイクルあたりの利用上限は **\$400** です。[Developer Portal](https://developer.x.com/en/portal/dashboard) でいつでも変更できます。

  移行を希望しない場合は、2026 年 6 月 1 日より前に Developer Portal でオプトアウトできます。オプトアウトすると切り替え日以降は API にアクセスできなくなる点にご注意ください。詳しくは[こちら](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)をご覧ください。
</Update>

<Update label="2026 年 5 月 20 日" description="X API">
  ### 修正: 無料クレジットでも自動チャージが動作するように

  本日、自動チャージ機能の修正をリリースし、無料クレジット（バウチャーで取得したもの）が有料クレジットと同様に扱われるようにしました。これまで、無料クレジットの残高がゼロになっても、自動チャージは有効になっていても発動しませんでした。**2026 年 5 月 21 日**より、残高が枯渇し、[Developer Console](https://console.x.com) で自動チャージが有効になっている場合は、保存済みのお支払い方法を使用してアカウントが自動的にチャージされます。

  無料クレジットが尽きた後に課金されたくない場合は、Developer Console で自動チャージの設定を確認し、必要に応じて無効化してください。詳しくは[こちら](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)をご覧ください。
</Update>
