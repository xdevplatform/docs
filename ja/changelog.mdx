...
<Update label="2026 年 6 月 8 日" description="X API">
  ### Pay-Per-Use の料金管理機能と xAI クレジットの段階

  本日、[X API の料金ページ](/x-api/getting-started/pricing) を更新し、クレジットベースの課金の詳細と [Developer Console](https://console.x.com) の新しい管理機能を追加しました：

  - **自動チャージ** — クレジット残高が設定したしきい値を下回ったときに自動的にチャージします。
  - **支出上限** — 請求サイクルごとの支出上限を設定し、想定外の課金を防ぎます。
  - **重複排除の明確化** — 同じリソース (例: Post) を 24 時間 (UTC) 以内に複数回リクエストしても課金は 1 回のみです。
  - **xAI API クレジットの無料付与** — 請求サイクル内の X API の累積支出に応じて、[xAI API](https://docs.x.ai) クレジットを最大 **20% 還元**: \$200 以上で **10%**、\$500 以上で **15%**、\$1,000 以上で **20%**。Developer Console で xAI チームをリンクするとクレジットを受け取れます。

  エンドポイントごとの読み取り/書き込み料金と、\$0.001 の [Owned Reads](/x-api/getting-started/pricing#owned-reads) 料金も、単一のレート表にまとめて記載しました。
</Update>
