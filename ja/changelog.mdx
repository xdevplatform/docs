...
<Update label="2026 年 7 月 24 日" description="X API v2">
  ### X Activity API のイベントペイロードリファレンス

  本日、[X Activity API](/ja/x-api/activity/introduction) の新しい[イベントペイロード](/ja/x-api/activity/event-payloads)リファレンスを公開しました。Post、いいね、フォロー、プロフィール更新、チャット、DM、ミュート、ブロック、Spaces、ニュース、OAuth の失効など、サポートされているすべてのイベントタイプのサンプル JSON を掲載しています。

  サブスクリプションを設定する前に、実際にキャプチャしたペイロードを使って Webhook ハンドラーと JSON のデシリアライズをモデル化するのにご利用ください。
</Update>
