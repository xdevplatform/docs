...
<Update label="2026 年 5 月 21 日" description="X API">
  ### 従来の Basic プランを Pay-Per-Use へ移行

  本日、従来の **Basic** プラン（月額および年額）を廃止することを発表しました。**2026 年 6 月 1 日**以降、Basic のサブスクリプション利用者は、次回の切替日に [Pay-Per-Use (PPU)](https://docs.x.com/x-api/getting-started/about-x-api) へ自動的に移行されます。操作は不要です。

  - **Basic 月額**: 切替時に **$200** の初期クレジットが請求され、API リクエストに応じて消費されます。
  - **Basic 年額**: 残存期間の按分金額が PPU クレジットとして付与され、按分された月数の経過後に失効します（追加のカード請求はありません）。
  - **移行後の PPU 既定設定**: 残高が **$10** を下回ると **$200** の自動チャージが有効になり、請求サイクルごとのスペンドキャップは **$400** です。設定は [Developer Portal](https://developer.x.com/en/portal/dashboard) でいつでも変更できます。

  オプトアウトする場合は、2026 年 6 月 1 日までに Developer Portal の設定を更新してください。オプトアウトすると、切替日以降は API アクセスが停止し、変更は取り消せません。詳細は[こちら](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)をご覧ください。
</Update>

<Update label="2026 年 5 月 20 日" description="X API">
  ### 修正: 無料クレジットでも自動チャージが発動するように

  本日、バウチャー（無料クレジット）由来の残高がゼロになった際に自動チャージが発動しない問題を修正しました。**2026 年 5 月 21 日**以降、無料クレジットと有料クレジットは自動チャージにおいて同一に扱われます。残高が枯渇し自動チャージが有効になっている場合、保存済みの支払い方法で自動的にチャージされます。

  無料クレジットの消費後に課金されたくない場合は、[Developer Console](https://console.x.com) で自動チャージ設定を確認し、必要に応じて無効化してください。詳細は[こちら](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)をご覧ください。
</Update>
