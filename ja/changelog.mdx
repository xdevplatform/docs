...
<Update label="2026 年 5 月 21 日" description="X API">
  ...
  本日、レガシーの **Basic** プラン (月額および年額) の提供終了を発表しました。**2026 年 6 月 1 日** 以降、すべての Basic 加入者は現在の請求サイクルの終了時に [Pay-Per-Use (PPU)](https://docs.x.com/x-api/getting-started/about-x-api) に自動的に移行されます。お客様側での操作は不要です。月額加入者には **\$200** の初回クレジットが請求されます。年額加入者には残りの按分相当額が PPU クレジットとして付与されます。移行後の PPU 既定設定: 自動チャージは **\$200** で有効 (残高が **\$10** を下回ると発動)、サイクルごとの使用上限は **\$400** で、いずれも [Developer Portal](https://developer.x.com/en/portal/dashboard) でいつでも調整できます。オプトアウトする場合は、**2026 年 6 月 1 日** までに設定を更新してください。オプトアウトすると、切替日以降は API へのアクセスができなくなる点にご注意ください。詳細は [こちら](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305) をご覧ください。
</Update>

<Update label="2026 年 5 月 20 日" description="X API">
  ### 自動チャージが無料クレジットにも適用

  本日、無料 (バウチャー) クレジットがゼロに達した際に自動チャージが発動しない問題を修正しました。**2026 年 5 月 21 日** より、無料クレジットは有料クレジットと同じ扱いになります。残高が完全に枯渇し、[Developer Console](https://console.x.com) で自動チャージが有効になっている場合、登録済みの支払い方法を使用してアカウントが自動的にチャージされます。自動チャージを有効にしていて、無料クレジットの消費後に課金されたくない場合は、コンソールの設定を確認のうえ無効化してください。詳細は [こちら](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184) をご覧ください。
</Update>
