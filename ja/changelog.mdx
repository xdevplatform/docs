...
<Update label="2026 年 6 月 30 日" description="X API v2">
  ### X API および X ドキュメント向け MCP サーバー

  本日、Grok、Cursor、Claude などの AI ツールが X をネイティブに扱えるよう、2 つのホスト型 [Model Context Protocol](/tools/mcp) サーバーをリリースしました。

  - **X MCP** (`https://api.x.com/mcp`) — MCP 対応の任意のクライアントから X API エンドポイントを呼び出せます。Posts、フルアーカイブ Search、Users、Bookmarks、News & Trends、Articles (ドラフトおよび公開) を含む機能をサポートします。オープンソースの [`xurl mcp`](/tools/mcp#x-mcp-—-x-api) ブリッジ経由で接続でき、OAuth 2.0 のログインとトークン更新を自動で処理します。
  - **Docs MCP** (`https://docs.x.com/mcp`) — `search_x` および `get_page_x` ツールを介して、AI アシスタントが [X API ドキュメントを検索・閲覧](/tools/mcp#docs-mcp-—-documentation-search)できるようになります。

  両方のサーバーを同時に実行すれば、同じアシスタントからドキュメントを参照*しつつ* API を呼び出せます。クライアント設定 (Grok Build、Cursor、Claude、VS Code)、ヘッドレスセットアップ、トラブルシューティングについては [MCP ガイド](/tools/mcp)を参照してください。
</Update>
