...
<Update label="2026 年 7 月 1 日" description="開発者プラットフォーム">
  ...
  本日、[**Developer E𝕏hibit**](https://developer.x.com/exhibit) を公開しました。X API で構築されたアプリを紹介する公開ディレクトリです。[Developer Console](https://console.x.com) の **Apps** ページからアプリを送信し、簡単な認証手順を完了すると、あなたのリスティングが公開され、潜在的なユーザーや他の開発者がプロジェクトを見つけられるようになります。詳細は[こちら](https://devcommunity.x.com/t/introducing-the-developer-e-hibit-showcase-your-x-api-apps-and-get-discovered/269675)。
</Update>

<Update label="2026 年 6 月 30 日" description="開発者プラットフォーム">
  ### ホスト型 X MCP サーバー

  本日、`https://api.x.com/mcp` にてホスト型 [X MCP サーバー](/tools/mcp)を公開しました。MCP 対応の AI ツール（Grok Build、Cursor、Claude、VS Code など）が、あなた自身のアカウント権限で X API を呼び出せるようになります。ローカルサーバーを構築・実行する必要はありません。オープンソースの [`xurl mcp`](https://github.com/xdevplatform/xurl) ブリッジ経由で接続すると、OAuth 2.0 PKCE ログインとトークンの自動更新を代わりに処理します。

  対応機能には、フルアーカイブの Post 検索、ユーザー検索、ニュース検索、タイムラインとメンション、ブックマーク管理、トレンド、Articles の下書き作成・公開が含まれます。詳細は[こちら](https://devcommunity.x.com/t/announcing-the-hosted-x-mcp/269558)。
</Update>
