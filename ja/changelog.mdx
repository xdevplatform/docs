...
<Update label="2026 年 5 月 21 日" description="X API">
  ### レガシー Basic プランを Pay-Per-Use へ移行

  本日、レガシー **Basic** の月額・年額プランの提供終了を発表しました。**2026 年 6 月 1 日**より、すべての Basic 加入者は現在の請求サイクルの終了時に [Pay-Per-Use (PPU)](https://docs.x.com/x-api/getting-started/pricing) へ自動的に移行されます。お客様による操作は不要です。月額加入者には Stripe を通じて **\$200** の初回クレジットが請求されます。年額加入者には、年額料金の残りの按分金額が PPU クレジットとして付与されます。移行後は、オートリチャージがデフォルトで **\$200** (残高が **\$10** を下回ったときにトリガー) で有効になり、請求サイクルあたりの上限は **\$400** に設定されます。これらの設定はすべて [Developer Portal](https://developer.x.com/en/portal/dashboard) で調整できます。移行を希望されない方は、2026 年 6 月 1 日までにオプトアウトできます。オプトアウトした場合、切り替え日に API アクセスが終了します。詳細は[こちら](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)をご覧ください。
</Update>

<Update label="2026 年 5 月 20 日" description="X API">
  ### 無料クレジット枯渇後にオートリチャージが作動するように

  本日、引き換え済みクレジットバウチャー (無料クレジット) の残高がゼロに達した際にオートリチャージ機能が作動しなかった問題を修正しました。**2026 年 5 月 21 日**より、無料クレジットはオートリチャージにおいて有料クレジットと同様に扱われます。残高が枯渇しオートリチャージが有効になっている場合は、登録済みの支払い方法で自動的に請求されます。無料クレジットが使い切られた後の課金を避けたい場合は、[Developer Console](https://console.x.com) でオートリチャージの設定を確認し、必要に応じて無効にしてください。詳細は[こちら](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)をご覧ください。
</Update>
