...
<Update label="2026 年 6 月 19 日" description="X Ads API">
  ### Ads API Analytics: 未対応のプレースメント、メトリックグループ、エンティティタイプを削除

  本日、現在本番環境でサポートされている内容を反映するため、[Ads API Analytics](/x-ads-api/analytics) エンドポイントを更新しました。お客様の連携で以下の値を参照している場合は、リクエストを適宜更新してください。

  - **プレースメント:** `PUBLISHER_NETWORK` はアナリティクスリクエストで有効なプレースメントではなくなりました（Active Entities のレスポンス内でのみ引き続き表示されます）。
  - **メトリックグループ:** `MEDIA` メトリックグループが削除され、`ENGAGEMENT` および `FOLLOWERS` の Media View Rate 派生メトリックも併せて削除されました。
  - **エンティティタイプ:** `MEDIA_CREATIVE` および `ORGANIC_TWEET` は、アナリティクスおよび同期アナリティクスでサポートされるエンティティタイプではなくなりました。
  - **目的:** `APP_ENGAGEMENTS` および `LEAD_GENERATION` の目的セクション（Metrics by Objective と Derived Metrics の両方）が削除されました。
  - **セグメンテーション:** セグメンテーションは `AGE`、`GENDER`、`METROS`、`PLATFORMS` のみサポートされます。
</Update>
