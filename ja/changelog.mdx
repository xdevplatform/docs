...
<Update label="2026 年 5 月 21 日" description="X API">
  ### 既存の Basic プランを Pay-Per-Use へ移行

  本日、X API の既存の **Basic** プラン (月額および年額) を廃止し、**2026 年 6 月 1 日**より **Pay-Per-Use (PPU)** へ自動移行することを発表しました。PPU では固定の月額料金が廃止され、実際に行ったリクエスト分のみを支払います。Basic Monthly のサブスクライバーは切替日に初期クレジット **\$200** が課金されます。Basic Annual のサブスクライバーは残りの按分相当額が PPU クレジットとして付与され、新たなカード課金はありません。移行後、自動チャージは残高が **\$10** を下回ったときに **\$200** をチャージするのが既定で、1 サイクルあたりの支出上限は **\$400** です。これらは [Developer Portal](https://developer.x.com/en/portal/dashboard) でいつでも変更できます。6 月 1 日より前にオプトアウトすることもできますが、その場合は切替日に API アクセスが終了します。詳細は[こちら](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)をご覧ください。
</Update>

<Update label="2026 年 5 月 20 日" description="X API">
  ### 自動チャージが無料クレジットでも動作するようになりました

  **2026 年 5 月 21 日**より、引き換えたバウチャーの無料クレジットは、自動チャージの対象として有料クレジットと同等に扱われます。これまでは、バウチャー残高がゼロに達しても自動チャージが有効になっていてもトリガーされませんでした。今回の修正により、自動チャージがオンで残高が枯渇した場合は、保存済みの支払い方法で自動的に課金されます。これは有料クレジットの従来の動作と一致します。無料クレジット消費後に課金されないようにするには、[Developer Console](https://console.x.com) で自動チャージを確認し、必要に応じて無効化してください。詳細は[こちら](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)をご覧ください。
</Update>
