...
<Update label="2026 年 6 月 30 日" description="X Developer Platform">
  ### X API および X ドキュメント向けのホスト型 MCP サーバー

  本日、Grok、Cursor、Claude、VS Code などの AI ツールを X に直接接続できる、2 つのホスト型 [Model Context Protocol](/tools/mcp) サーバーをリリースしました。

  - **X MCP**（`https://api.x.com/mcp`）— MCP 対応の任意のクライアントから X API エンドポイントを呼び出し、Post の検索、ユーザーのルックアップ、ブックマークの管理、トレンドやニュースの取得、Articles の下書き作成や公開を、すべてご自身の X アカウントの権限で行えます。接続にはオープンソースの [`xurl mcp`](https://github.com/xdevplatform/xurl) ブリッジを使用し、OAuth 2.0 PKCE ログインの処理とトークンの自動更新を行います。
  - **Docs MCP**（`https://docs.x.com/mcp`）— AI ツールから X API ドキュメントを検索・閲覧できます。

  Grok Build、Cursor、Claude、VS Code 向けのセットアップガイド、およびヘッドレス／リモートマシンでのログイン手順は、[MCP サーバー](/tools/mcp) のドキュメントに記載されています。
</Update>
