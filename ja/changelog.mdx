...
<Update label="2026 年 6 月 30 日" description="開発者ツール">
  ### X API とドキュメント向けのホスト型 MCP サーバー

  本日、Grok、Cursor、Claude Desktop、VS Code などの AI ツールが X と直接やり取りできるよう、2 つのホスト型 [Model Context Protocol サーバー](/tools/mcp)を公開しました。

  - **X MCP** (`https://api.x.com/mcp`) — MCP 対応の任意のクライアントから X API のエンドポイントを呼び出せます。モデルはフルアーカイブ検索、ユーザー検索、bookmarks の管理、trends やニュースの取得、Articles のドラフト作成や公開を、ご自身のアカウントの権限で実行できます。オープンソースの `xurl mcp` ブリッジ経由で接続し、OAuth 2.0 とトークンの自動更新を任せられます。
  - **Docs MCP** (`https://docs.x.com/mcp`) — エディタを離れることなく、AI アシスタントから X API ドキュメントを検索・閲覧できます。

  Grok Build、Cursor、Claude Desktop、VS Code、その他の MCP クライアントごとの設定方法は、[MCP セットアップガイド](/tools/mcp)をご覧ください。
</Update>
