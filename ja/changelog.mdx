...
<Update label="2026 年 6 月 18 日" description="X API">
  ### 料金ページの更新: 自動チャージのセーフガード、Webhook イベントの料金、xAI クレジット階層の明確化

  本日、[X API の料金ページ](/x-api/getting-started/pricing)をユーザー向けに以下の点で更新しました。

  - **自動チャージのセーフガード** — 自動チャージは 5 分間に最大 1 回までしか実行されず、残高がゼロまたはマイナスの場合は一時停止されるため、想定外の課金から保護されます。
  - **Webhook イベントの料金** — [X Activity API](/x-api/activity/introduction) のイベント（`post.create`、`follow.follow`、`dm.received`、`spaces.start` など）について、イベントごとの料金表を追加しました。他のリソースと同じ 24 時間 UTC ウィンドウ内で重複排除されます。
  - **xAI API の無料クレジット階層** — 累計支出に応じた還元レート（&dollar;200 以上で 10%、&dollar;500 以上で 15%、&dollar;1,000 以上で 20%）を、各購入で実際に受け取る金額がわかる具体例とともに明確化しました。
  - **Enterprise との比較** — [Enterprise の料金ページ](/enterprise-api/getting-started/pricing)で、従量課金プランには月間 200 万件の Post 読み取り上限が含まれる一方、Enterprise プランはカスタムまたは無制限のボリュームを提供することを明記しました。
</Update>
