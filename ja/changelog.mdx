...
<Update label="2026 年 7 月 21 日" description="X API v2">
  ### Account Activity API の非推奨化

  本日、[Account Activity API (AAA)](/x-api/account-activity/introduction) の非推奨化を発表しました。今後、リアルタイムのユーザーアクティビティ配信には [X Activity API (XAA)](/x-api/activity/introduction) をご利用ください。XAA は同じイベントタイプ（Posts、Direct Messages、Likes、Follows、Spaces など）を、より低レイテンシーのパイプラインでカバーし、ユーザー ID ごとに単一のサブスクリプションモデルを提供します。

  既存の AAA インテグレーションは非推奨期間中は引き続き動作します。中断を避けるため、XAA への移行を計画してください。
</Update>
