...
<Update label="2026 年 7 月 21 日" description="開発者プラットフォーム">
  ### developer.x.com/status に新しいライブステータスページを公開

  X Developer Platform（X API v2、エンタープライズ API、Ads API、ストリーミングエンドポイント、および関連サービスを含む）のライブステータスは、[developer.x.com/status](https://developer.x.com/status) でホストされるようになりました。新しいページをブックマークして、稼働状況や現在発生中のインシデントを確認してください。過去のインシデントは [インシデント](/incidents) ページに保存されています。
</Update>
