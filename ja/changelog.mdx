---
title: 変更履歴
description: "更新情報の通知を受け取るには、@API の 「通知をオンにする」 を有効にしてください。 本日、X API v2 の検索エンドポイント (/2/tweets/search/recent、/2/tweets/search/all、/2/tweets/counts/recent、/2/tweets/counts/all."
sidebarTitle: 概要
...
<Update label="2026 年 5 月 21 日" description="X API">
  ### レガシー Basic プランが Pay-Per-Use へ移行

  本日、レガシーの X API **Basic** プラン (月額および年額) を非推奨化し、新しい主力プランである **Pay-Per-Use (PPU)** へ自動移行することを発表しました。**2026 年 6 月 1 日**以降、現在の Basic サブスクリプションは現行の請求サイクル終了時 (切替日) に終了し、PPU に移行されます。PPU では固定の月額料金はなく、実際に行った API リクエストに対してのみお支払いいただきます。切替時、月額サブスクライバーには Stripe 経由で **\$200** の初期クレジットが課金されます。年額サブスクライバーには、年額料金の残額が日割りで PPU クレジットとして付与されます。移行後、オートリチャージは残高が **\$10** を下回ったときに **\$200** で発動するようデフォルト設定され、請求サイクルごとの上限は **\$400** に設定されます。いずれも[Developer Portal](https://developer.x.com/en/portal/dashboard) でいつでも調整可能です。PPU への移行を希望しない場合は、2026 年 6 月 1 日までに Developer Portal でオプトアウトできますが、その場合、切替日以降は API アクセスが失われます。詳細は[こちら](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305)をご覧ください。
</Update>

<Update label="2026 年 5 月 20 日" description="X API">
  ### オートリチャージが無料クレジットでも発動するように

  クレジットバウチャー (無料クレジット) の残高がゼロになったときにオートリチャージが発動しなかった問題を修正しました。**2026 年 5 月 21 日**より、オートリチャージにおいて無料クレジットは有料クレジットと同様に扱われます。残高が枯渇し、オートリチャージが有効になっている場合、登録済みの支払い方法を使用して自動的にチャージされます。無料クレジット消化後の課金を避けたい場合は、[Developer Console](https://console.x.com) で設定を確認し、必要に応じてオートリチャージを無効にしてください。詳細は[こちら](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184)をご覧ください。
</Update>

<Update label="2026 年 5 月 13 日" description="X Ads API">
  ### X Ads API フィードバックプログラム

  **X Ads API** の改善に向けて、開発者および広告主の皆さまから直接フィードバックを募集しています。バグ報告、エンドポイントの課題、希望機能、ドキュメントのギャップなど、ぜひお聞かせください。[フィードバックフォーム](https://docs.google.com/forms/d/e/1FAIpQLSeDn5s1T0ze9-7K0XAw18MD3bEzGrIle3ko3uyjN8EP5dJhYQ/viewform)から、または[告知スレッド](https://devcommunity.x.com/t/x-ads-api-share-your-feedback/265247)へのご返信でご共有ください。
</Update>
