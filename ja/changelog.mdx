...
<Update label="2026 年 6 月 30 日" description="開発者プラットフォーム">
  ...
  本日、Grok、Cursor、Claude、VS Code などの AI ツールから X を直接利用できる、ホスト型の [Model Context Protocol](/tools/mcp) サーバーを 2 つ公開しました。

  - **X MCP**（`https://api.x.com/mcp`）— X API のエンドポイント（Post やユーザーの検索・取得、全期間検索、Bookmarks、Trends、News、Articles のドラフト作成と公開など）を、ご自身の X アカウントの権限で呼び出せます。OAuth とトークンの自動更新を代行するオープンソースのブリッジ [`xurl mcp`](https://github.com/xdevplatform/xurl) 経由で接続します。
  - **Docs MCP**（`https://docs.x.com/mcp`）— 認証不要で、AI ツール内から X API のドキュメントを検索・閲覧できます。

  クライアント設定（Grok Build、Cursor、Claude、VS Code）、ヘッドレスでのログイン、トラブルシューティングについては [MCP セットアップガイド](/tools/mcp) を参照してください。
</Update>
