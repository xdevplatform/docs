...
<Update label="2026 年 6 月 28 日" description="X Ads API">
  ### キャンペーンの予算最適化を `LINE_ITEM` のみに簡素化

  [Campaign Management API](/x-ads-api/campaign-management/reference) のキャンペーンは、X Ads Manager の動作に合わせて、予算最適化として `LINE_ITEM` のみをサポートするようになりました。

  - キャンペーンの `POST` および `PUT` リクエストで、`budget_optimization` は `LINE_ITEM` のみを受け付け、デフォルト値は `LINE_ITEM` になりました（以前は `CAMPAIGN`）。
  - `standard_delivery` はキャンペーンレベルでは非推奨になりました。標準配信と高速配信の切り替えは、ライン項目の [`standard_delivery`](/x-ads-api/campaign-management/reference) パラメーターで設定してください。
</Update>
