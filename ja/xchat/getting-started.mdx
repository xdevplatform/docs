...
description: Chat XDK と X API を使ってエンドツーエンド暗号化された X Chat メッセージングを構築する
...
| コンポーネント | 役割 |
|:----------|:-----|
| **[Chat XDK](/xchat/xchat-xdk)** | 暗号化、復号、署名、および秘密鍵の保存（Juicebox または鍵 blob） |
...
**Juicebox**（PIN + API からの `juicebox_config`）または**鍵 blob**（`import_keys`）を使って秘密鍵をロードします。次に、登録済みの公開鍵バージョン（レコード上の `public_key_version`）を設定します。
...
サーバーおよびボットのサンプルでは、通常**鍵 blob**（`export_keys` / `import_keys`）を使用します。クライアントアプリでは通常 **Juicebox**（PIN を使った `setup` / `unlock`）を使用します。両方のパスについては [Chat XDK](/xchat/xchat-xdk) リファレンスを参照してください。

---

## 3. 公開鍵を登録する（1 回のみ）

キーペアを生成し、**公開**鍵を X に登録し、**秘密**鍵を保存します（PIN で `setup`、または `export_keys`）。
...
<Warning>
Juicebox には強力な PIN を使用してください。PIN を紛失したり、保護されていない鍵 blob を失うと、過去のメッセージの復号ができなくなる可能性があります。
</Warning>
...
- 本番環境で PIN、秘密鍵、メッセージ平文をログに残さない
- Web アプリでは、OAuth トークン（および Juicebox realm トークン発行）をサーバー側で保持し、秘密鍵はクライアントの Chat XDK 内でのみ保持することが望ましい
