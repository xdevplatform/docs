---
title: "X Chat で画像やファイルを送る"
sidebarTitle: メディア
description: "暗号化されたチャットで画像やファイルを共有します。Chat XDK でメディアを暗号化、アップロード、送信、ダウンロード、復号できます。"
keywords: ["X Chat メディア", "暗号化画像", "添付ファイル", "encrypt_stream", "メディアアップロード", "メディアダウンロード"]
---
...
このパスは Posts のメディアモデル(`expansions=attachments.media_keys`、`media.fields=variants` など)では**ありません**。それらのパラメーターは **Posts** に適用されます。暗号化された X Chat ブロブは **`media_hash_key`** と X Chat メディアダウンロードでアドレッシングされます。
