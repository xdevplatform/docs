---
title: "Chat XDK: X Chat の暗号化ツールキット"
sidebarTitle: Chat XDK
description: "鍵管理、暗号化、復号、署名を代わりに処理してくれるライブラリ、Chat XDK を紹介します。対応する全言語のリファレンス付き。"
keywords: ["Chat XDK", "chat-xdk", "暗号化SDK", "鍵管理", "署名", "SDK リファレンス"]
---
...
<Warning>
**ラップする前に取得した鍵を検証してください。** prepare メソッドは渡された任意の公開鍵に対して新しい会話鍵を暗号化します。渡す前に、取得した各レコードに対して `verify_key_binding(identity, signing, signature)` を呼び出し(public-keys API からの `public_key`、`signing_public_key`、`identity_public_key_signature` の各フィールドで)、差し替えられたアイデンティティ鍵が会話鍵を受け取れないようにします。
</Warning>
...
<CardGroup cols={2}>
  <Card title="はじめに" icon="/icons/xds/icon-rocket.svg" href="/ja/xchat/getting-started">
    Chat XDK を Chat API に配線
  </Card>
  <Card title="メディア" icon="/icons/xds/icon-photo.svg" href="/ja/xchat/media">
    ストリーム暗号化とメディア REST
  </Card>
