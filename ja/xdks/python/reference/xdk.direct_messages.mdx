...
description: "X API SDK の direct_messages Python パッケージのリファレンス。 X API v2 の direct messages エンドポイント向けのクライアントと Pydantic モデルをまとめています。"
---
...
このモジュールは、X API の direct messages エンドポイントへのアクセスを提供します
direct messages-related のすべての機能の主要なエントリポイントとして機能します。
