---
title: X API v2 の News ルックアップエンドポイント
description: "X API v2 の news エンドポイントを使用して、X 上のブレイキングニュースストーリーやヘッドラインをルックアップします。サマリー、フック、コンテキスト、ティッカー、クラスタリングされた Post 結果を含みます。"
sidebarTitle: はじめに
keywords: ["news", "news API", "headlines", "breaking news", "news lookup", "news stories", "news search"]
---

News ルックアップエンドポイントを使用すると、開発者は X 上のニュースや速報ヘッドラインを取得できます。

このエンドポイントは OAuth1 および OAuth2 PKCE の app-auth と user-auth 認証をサポートします。

## はじめに

このエンドポイントを使用するには、[Developer Console](https://developer.x.com/en/portal/dashboard) から取得した [Bearer Token](/fundamentals/authentication/oauth-2-0/application-only) が必要です。

Bearer Token を取得したら、以下のように news API を呼び出すことができます:
