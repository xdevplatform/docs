...
<Update label="Sep 2, 2026" description="X API v2">
  ...
  We improved delivery latency on the X API v2 streaming endpoints that prioritize data hydration, including [Filtered Stream](/x-api/posts/filtered-stream/introduction) and the volume streams. P99 latency is now approximately **4-5 seconds**, down from ~6-7 seconds.

  You receive fully hydrated Post data with all metadata faster, with no changes required on your side. Retry behavior and delivery guarantees are unchanged. See [consuming streaming data](/x-api/fundamentals/consuming-streaming-data) for details.

  If you need the lowest possible latency, [Powerstream](/x-api/powerstream/introduction) remains the fastest option.
</Update>
