...
<Update label="Jun 30, 2026" description="X Developer Platform">
  ### Hosted MCP servers for the X API and X docs

  Today, we launched two hosted [Model Context Protocol](/tools/mcp) servers so you can connect AI tools like Grok, Cursor, Claude, and VS Code directly to X:

  - **X MCP** at `https://api.x.com/mcp` — call X API endpoints from any MCP‑compatible client to search posts, look up users, manage bookmarks, fetch trends and news, and draft or publish Articles, all with your own X account's permissions. Connect through the open‑source [`xurl mcp`](https://github.com/xdevplatform/xurl) bridge, which handles OAuth 2.0 PKCE login and auto‑refreshes your token.
  - **Docs MCP** at `https://docs.x.com/mcp` — search and read the X API documentation from inside your AI tool.

  Setup guides for Grok Build, Cursor, Claude, and VS Code, plus headless/remote login instructions, are in the [MCP servers](/tools/mcp) docs.
</Update>
