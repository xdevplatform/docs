...
<Update label="Jun 8, 2026" description="X API">
  ### Pay-Per-Use pricing controls and xAI credit tiers

  Today, we expanded the [X API pricing page](/x-api/getting-started/pricing) with more detail on credit-based billing and new controls in the [Developer Console](https://console.x.com):

  - **Auto-recharge** — automatically top up your credit balance when it falls below a threshold you set.
  - **Spending limits** — cap how much you can spend per billing cycle to prevent unexpected charges.
  - **Deduplication clarified** — the same resource (e.g., a Post) requested more than once in a 24-hour UTC window is only charged once.
  - **Free xAI API credit tiers** — earn up to **20% back** in [xAI API](https://docs.x.ai) credits based on cumulative X API spend per billing cycle: **10%** at \$200+, **15%** at \$500+, **20%** at \$1,000+. Link your xAI team in the Developer Console to receive credits.

  Per-endpoint read and write rates, as well as \$0.001 [Owned Reads](/x-api/getting-started/pricing#owned-reads) pricing, are also now documented in a single rate table.
</Update>
