...
<Update label="Aug 21, 2026" description="X API v2">
  ...
  Today, we added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). You can now subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast.

  Both are public events and can be subscribed to with either app-only or user-context authentication. See [Event payloads](/x-api/activity/event-payloads) for sample JSON.
</Update>

<Update label="Aug 14, 2026" description="X API v2">
  ### `tweet_count` removed from Trends by WOEID

  The `GET /2/trends/by/woeid/{woeid}` endpoint no longer returns the `tweet_count` field, and `tweet_count` is no longer a valid value for the `trend.fields` parameter. The endpoint returns `trend_name` only. See [Trends by WOEID](/x-api/trends/trends-by-woeid/introduction).

  The `GET /2/users/personalized_trends` endpoint is unaffected and continues to return `tweet_count`.
</Update>
