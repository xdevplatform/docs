...
<Update label="Sep 5, 2026" description="X API v2">
  ...
  We improved delivery latency on the [streaming endpoints](/x-api/fundamentals/consuming-streaming-data) that prioritize data hydration, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction) and the volume streams. P99 latency is now approximately **4-5 seconds**, down from ~6-7 seconds. No changes are required on your end. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction).

  ### More authentication options for webhook and X Activity API management

  We expanded the authentication methods accepted by the [webhook](/x-api/webhooks/introduction) and [X Activity API](/x-api/activity/introduction) management endpoints:

  - `GET /2/webhooks`, `PUT /2/activity/subscriptions/{subscription_id}`, and `DELETE /2/activity/subscriptions/{subscription_id}` now accept OAuth 2.0 user context and OAuth 1.0a user context, in addition to app-only Bearer Token.
  - `POST /2/webhooks`, `PUT /2/webhooks/{webhook_id}`, and `DELETE /2/webhooks/{webhook_id}` now accept OAuth 2.0 user context, in addition to OAuth 1.0a user context and app-only Bearer Token.
</Update>
