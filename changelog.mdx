...
<Update label="Sep 2, 2026" description="X API v2">
  ### Broadcast start and end events for X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). You can now subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast.

  These are public events: you can subscribe with either app-only or user-context authentication. Payloads include the host's profile, the `broadcast_id`, title, language, broadcast state, timing fields, and a thumbnail URL. See [event payloads](/x-api/activity/event-payloads) for full examples.

  ### Filter qualifiers for X Activity API subscriptions

  We added an optional `filter.qualifiers` map to [X Activity API subscriptions](/x-api/activity/introduction#filter-qualifiers). Qualifiers are event-specific predicates that narrow which events a subscription delivers:

  - `conversation_type` (`direct`, `group`) on `chat.received`
  - `direction` (`inbound`, `outbound`) on `follow.follow`, `follow.unfollow`, `like.create`

  A subscription accepts up to 8 qualifier entries. Unsupported keys or values return **400**. The create, list, and update subscription responses echo `filter.qualifiers` back.

  ### Usage credits endpoint

  You can now check your Pay-per-use credit balance programmatically. [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) returns your remaining credit balance in USD, alongside the existing [`GET /2/usage/tweets`](/x-api/usage/get-usage) endpoint for Post consumption. See the [Usage introduction](/x-api/usage/introduction).

  ### Pay-per-use monthly Post cap raised to 3 million

  We raised the monthly Post read cap on Pay-per-use plans from 2 million to 3 million reads per billing cycle. If you need higher volume, consider an [Enterprise plan](/enterprise-api/introduction). See [pricing](/x-api/getting-started/pricing) and [Post cap](/x-api/fundamentals/post-cap).

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload in the [X Activity API](/x-api/activity/event-payloads) now includes an `is_subscriber` boolean alongside the `broadcast_id`, message fields, and author profile.
</Update>
