...
<Update label="Sep 5, 2026" description="X API v2">
  ...
  We improved delivery latency on the hydrated streaming endpoints, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction). These streams now deliver Posts with a **P99 latency of approximately 4-5 seconds**, down from the previous ~6-7 seconds. No changes are required on your side. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction).

  ### OAuth 2.0 user context for webhook and X Activity subscription management

  The [webhook management endpoints](/x-api/webhooks/introduction) (`GET`/`POST /2/webhooks`, `PUT`/`DELETE /2/webhooks/{webhook_id}`) and the [X Activity API](/x-api/activity/introduction) subscription management endpoints (`PUT`/`DELETE /2/activity/subscriptions/{subscription_id}`) now accept OAuth 2.0 user-context and OAuth 1.0a user-context authentication in addition to the app-only Bearer Token. You can now manage webhooks and Activity subscriptions with the same user-context credentials your app already uses for other endpoints.
</Update>
