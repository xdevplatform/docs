...
<Update label="Sep 8, 2026" description="X API v2">
  ### Faster streaming delivery

  We improved delivery latency across X API streaming endpoints, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction) and [PowerStream](/x-api/powerstream/introduction). P99 latency for hydrated Post delivery is now approximately **4-5 seconds**, down from ~6-7 seconds. No changes are required on your side. See [consuming streaming data](/x-api/fundamentals/consuming-streaming-data) for delivery guarantees.

  ### More authentication options for webhook and subscription management

  [Webhook management endpoints](/x-api/webhooks/introduction) (`GET /2/webhooks`, `POST /2/webhooks`, `PUT /2/webhooks/{webhook_id}`, `DELETE /2/webhooks/{webhook_id}`) now accept OAuth 2.0 user-context authentication in addition to OAuth 1.0a and app-only bearer tokens.

  The X Activity API [update subscription](/x-api/activity/update-x-activity-subscription) and [delete subscription](/x-api/activity/deletes-x-activity-subscription) endpoints (`PUT` / `DELETE /2/activity/subscriptions/{subscription_id}`) now accept OAuth 2.0 user-context and OAuth 1.0a authentication in addition to app-only bearer tokens. You can now manage webhooks and subscriptions with the same user-context tokens you use elsewhere.
</Update>
