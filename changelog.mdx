...
<Update label="Sep 18, 2026" description="Weekly recap">
  ### This week on the X developer platform

  #### New features

  - **Conversion API Google Tag Manager server-side template (X Ads API).** Advertisers can send website conversions through the official **X Ads Conversion API (Official)** tag in a Google Tag Manager server container, without writing Conversion API request code. See [Set up the Conversion API with Google Tag Manager](/x-ads-api/measurement/gtm-server-side).
  - **Lead Generation API (X Ads API).** Create and manage native lead forms, attach a form to a card with the `LEAD_FORM` destination, and export submitted leads as JSON, CSV, or XLSX. The Leads API requires the Campaign user permission; lead exports are restricted to account administrators. See the [Lead Generation API](/x-ads-api/lead-generation).

  #### Updates

  - **`REGIONS` segmentation for Analytics (X Ads API).** Asynchronous analytics jobs now support `REGIONS` as a `segmentation_type`. The `country` parameter is required when requesting a regional breakdown. See [Metrics and Segmentation](/x-ads-api/analytics#metrics-and-segmentation).
</Update>
