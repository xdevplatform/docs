...
<Update label="Aug 30, 2026" description="X API">
  ### Filter qualifiers for X Activity API subscriptions

  We added an optional `filter.qualifiers` map to [X Activity API](/x-api/activity/introduction#filter-qualifiers) subscriptions. Qualifiers narrow which events match, so you only receive the events you care about. You can filter `chat.received` events by `conversation_type` (`direct` or `group`), and `follow.follow`, `follow.unfollow`, and `like.create` events by `direction` (`inbound` or `outbound`). A subscription supports up to 8 qualifiers, and responses echo them back under `filter.qualifiers`.

  ### Broadcast start and end events for X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast. Both are public events and can be subscribed to with either app-only or user-context authentication. See [event payloads](/x-api/activity/event-payloads) for sample payloads.

  ### New usage credits endpoint

  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), which returns your Pay-per-use (PPU) credit balance in USD. Use it to monitor your remaining credits programmatically. See the [Usage introduction](/x-api/usage/introduction) for all usage endpoints.

  ### Pay-per-use monthly Post cap raised to 3 million

  We raised the monthly [Post read cap](/x-api/fundamentals/post-cap) on Pay-per-use plans from 2 million to 3 million. If you need higher volume, consider an [Enterprise plan](/forms/enterprise-api-interest).

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload in the X Activity API now includes an `is_subscriber` boolean, indicating whether the chat author is a subscriber of the broadcaster. See [event payloads](/x-api/activity/event-payloads) for an updated sample.
</Update>
