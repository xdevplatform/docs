...
<Update label="Sep 26, 2026" description="X API v2">
  ...
  - **Subscription expiration for X Activity API:** Pass an optional `expires_at` (RFC 3339) when creating a subscription, and X deletes it automatically at that time. To change the expiration, send the same subscription again with a new value. See [Subscription expiration](/x-api/activity/introduction#subscription-expiration).
  - **OAuth 2.0 webhook signatures:** Webhook POSTs now include an `X-Twitter-Webhooks-Signature-OAuth2` header signed with your app's OAuth 2.0 client secret. You can also generate CRC responses with the client secret. The legacy `X-Twitter-Webhooks-Signature` header still works. See [Signature headers](/x-api/webhooks/introduction#signature-headers).
  - **`is_moderator` in `broadcast.chat` events:** Broadcast chat payloads now tell you whether the message author is a moderator. The broadcast owner shows as `false`. See [`broadcast.chat`](/x-api/activity/event-payloads).

  ### Updates

  - **Broadcast scopes in token exchange:** When you [exchange OAuth 1.0a tokens for OAuth 2.0](/fundamentals/authentication/oauth-2-0/oauth-1-0a-token-exchange), the new tokens now include `broadcast.read`. Tokens for read-and-write users also include `broadcast.write`.
  - **Approving Chat join requests:** When you [add members to a Chat group](/x-api/chat/add-members-to-a-chat-group-conversation), adding a user with a pending join request approves that request.
  - **Livestream API access:** The Livestream API is open to approved apps only. You no longer need an Enterprise agreement. Request access with the [Livestream API Access Form](/forms/livestream-api-access).
  - **Batch compliance upload limit:** Batch compliance uploads accept files up to **128 MB** (about 6.5 million IDs). Larger uploads return `413`. For faster processing, split large datasets into jobs that each cover a narrower time period. See [Batch compliance](/x-api/compliance/batch-compliance/integrate).
  - **`source` removed from `tweet.fields`:** The deprecated `source` Post field is no longer available. Remove it from your `tweet.fields` requests.
</Update>
