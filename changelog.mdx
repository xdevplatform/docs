...
<Update label="Sep 6, 2026" description="X API v2">
  ...
  We improved delivery latency on the X API v2 streaming endpoints. Streams that prioritize data hydration and delivery, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction), now have a P99 latency of approximately **4-5 seconds**, down from ~6-7 seconds. No changes are required on your side. If you need the lowest possible latency, use [Powerstream](/x-api/powerstream/introduction).

  ### OAuth 2.0 user context for webhook and subscription management

  The [webhook management endpoints](/x-api/webhooks/introduction) (`GET` and `POST /2/webhooks`, `PUT` and `DELETE /2/webhooks/{webhook_id}`) and the [X Activity API](/x-api/activity/introduction) subscription endpoints (`PUT` and `DELETE /2/activity/subscriptions/{subscription_id}`) now accept OAuth 2.0 user-context and OAuth 1.0a user-context authentication, in addition to app-only Bearer Tokens. You can now manage webhooks and subscriptions using the same user-context credentials as the rest of your integration.
</Update>
