...
<Update label="Jun 28, 2026" description="X Ads API">
  ### Campaign budget optimization simplified to `LINE_ITEM`

  Campaigns in the [Campaign Management API](/x-ads-api/campaign-management/reference) now support only `LINE_ITEM` budget optimization, matching the behavior of X Ads Manager.

  - `budget_optimization` accepts only `LINE_ITEM` on `POST` and `PUT` campaign requests, and defaults to `LINE_ITEM` (previously `CAMPAIGN`).
  - `standard_delivery` is deprecated at the campaign level. Configure standard versus accelerated delivery on the line item via its [`standard_delivery`](/x-ads-api/campaign-management/reference#put-accounts-account_id-line_items-line_item_id) parameter instead.
</Update>
