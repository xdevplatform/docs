...
<Update label="Aug 27, 2026" description="X API v2">
  ### This week: new usage credits endpoint, X Activity API additions, and a higher PPU post cap

  **New features**

  - **Check your Pay-Per-Use credit balance**: Added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), a new endpoint that returns your Pay-Per-Use (PPU) credit balance in USD. You can now monitor your remaining credits programmatically instead of checking the developer portal. See the [Usage API docs](/x-api/usage/introduction).
  - **Broadcast start and end events for X Activity API**: Added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast. Both are public events, so you can subscribe with either app-only or user-context authentication. Sample payloads are in the [event payloads reference](/x-api/activity/event-payloads).
  - **Filter qualifiers for X Activity API subscriptions**: Subscriptions now accept an optional `filter.qualifiers` map (up to 8 entries) to narrow which events match. Use `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Unsupported keys or values return a 400 error. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).

  **Updates**

  - **Pay-Per-Use monthly post cap raised to 3 million**: The monthly Post read cap on Pay-Per-Use plans increased from 2 million to 3 million. See [Post cap](/x-api/fundamentals/post-cap) and [pricing](/x-api/getting-started/pricing).
  - **`is_subscriber` field on `broadcast.chat` events**: The `broadcast.chat` event payload in the X Activity API now includes an `is_subscriber` boolean, indicating whether the chat message author is a subscriber of the broadcaster. See the [event payloads reference](/x-api/activity/event-payloads).
</Update>
