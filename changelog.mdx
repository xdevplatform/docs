...
<Update label="Jul 21, 2026" description="X API v2">
  ### Account Activity API deprecation

  Today, we announced the deprecation of the [Account Activity API (AAA)](/x-api/account-activity/introduction). Going forward, use the [X Activity API (XAA)](/x-api/activity/introduction) for real-time user activity delivery — it covers the same event types (Posts, Direct Messages, Likes, Follows, Spaces, and more) over a lower-latency pipeline, with a single subscription model per User ID.

  Existing AAA integrations will continue to work during the deprecation window. Plan to migrate to XAA to avoid disruption.
</Update>
