...
<Update label="Jun 19, 2026" description="X Ads API">
  ### Ads API Analytics: removal of unsupported placements, metric groups, and entity types

  Today, we updated the [Ads API Analytics](/x-ads-api/analytics) endpoint to reflect what is currently supported in production. If your integration references any of the values below, update your requests accordingly:

  - **Placements:** `PUBLISHER_NETWORK` is no longer a valid placement for analytics requests (it remains visible only in Active Entities responses).
  - **Metric groups:** the `MEDIA` metric group has been removed, along with the Media View Rate derived metrics under `ENGAGEMENT` and `FOLLOWERS`.
  - **Entity types:** `MEDIA_CREATIVE` and `ORGANIC_TWEET` are no longer supported entity types for analytics or sync analytics.
  - **Objectives:** the `APP_ENGAGEMENTS` and `LEAD_GENERATION` objective sections (both Metrics by Objective and Derived Metrics) have been removed.
  - **Segmentation:** segmentation is supported only for `AGE`, `GENDER`, `METROS`, and `PLATFORMS`.
</Update>
