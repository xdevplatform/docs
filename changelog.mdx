...
<Update label="Sep 9, 2026" description="X API v2">
  ### Lower streaming latency and new authentication options

  **Streaming latency improved.** Volume and filtered stream endpoints now deliver hydrated Post data with a **P99 latency of approximately 4-5 seconds**, down from the previous ~6-7 seconds. No changes are required on your side. See [consuming streaming data](/x-api/fundamentals/consuming-streaming-data) and the [Powerstream introduction](/x-api/powerstream/introduction).

  **Broadcast scopes documented.** The `broadcast.read` and `broadcast.write` OAuth 2.0 scopes are now listed in the [scopes table](/fundamentals/authentication/oauth-2-0/authorization-code). Use them to authorize the [Broadcasts endpoints](/x-api/broadcasts/list-broadcasts) for viewing and managing live broadcasts and their chat.

  **More authentication options for webhooks and X Activity subscriptions.** [Webhook management endpoints](/x-api/webhooks/introduction) (`GET/POST /2/webhooks`, `DELETE/PUT /2/webhooks/:webhook_id`) and [X Activity subscription endpoints](/x-api/activity/introduction) (`PUT/DELETE /2/activity/subscriptions/:subscription_id`) now accept OAuth 2.0 user context, and OAuth 1.0a user context where it was not previously supported, in addition to App-only Bearer Token.
</Update>
