...
<Update label="Aug 20, 2026" description="X API v2">
  ...
  We corrected the reference for [`GET /2/trends/by/woeid/{woeid}`](/x-api/trends/trends-by-woeid/introduction) to reflect actual API behavior. The `tweet_count` field is not returned by this endpoint, and `tweet_count` is no longer a valid value for the `trend.fields` parameter. Requests continue to return `trend_name`.

  This does not affect [`GET /2/users/personalized_trends`](/x-api/trends/personalized-trends/introduction), which still returns `tweet_count`.
</Update>
