...
<Update label="Sep 4, 2026" description="X API v2">
  ...
  You can now use OAuth 2.0 user-context authentication to manage webhooks and [X Activity API](/x-api/activity/introduction) subscriptions:

  - [Webhook endpoints](/x-api/webhooks/introduction) (`GET`/`POST /2/webhooks`, `PUT`/`DELETE /2/webhooks/{webhook_id}`) now accept OAuth 2.0 user context. `GET /2/webhooks` also now accepts OAuth 1.0a.
  - X Activity API subscription endpoints (`PUT`/`DELETE /2/activity/subscriptions/{subscription_id}`) now accept OAuth 2.0 user context and OAuth 1.0a, in addition to app-only Bearer tokens.

  This means you can manage webhooks and subscriptions with the same fine-grained OAuth 2.0 user tokens you use elsewhere, instead of switching to an app-only Bearer token.

  ### Lower streaming latency: P99 now ~4-5 seconds

  We improved delivery latency on [volume streaming endpoints](/x-api/fundamentals/consuming-streaming-data). Hydrated streams, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction), now deliver Posts with a P99 latency of approximately **4-5 seconds**, down from ~6-7 seconds. No changes are required on your end. If you need the lowest possible latency, see [Powerstream](/x-api/powerstream/introduction).
</Update>
