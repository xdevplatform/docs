...
<Update label="Jun 30, 2026" description="X API v2">
  ### MCP servers for the X API and X docs

  Today, we shipped two hosted [Model Context Protocol](/tools/mcp) servers so AI tools like Grok, Cursor, and Claude can work with X natively:

  - **X MCP** (`https://api.x.com/mcp`) — call X API endpoints from any MCP‑compatible client. Capabilities span Posts, full‑archive Search, Users, Bookmarks, News & Trends, and Articles (draft and publish). Connect via the open‑source [`xurl mcp`](/tools/mcp#x-mcp-—-x-api) bridge, which handles OAuth 2.0 login and token refresh for you.
  - **Docs MCP** (`https://docs.x.com/mcp`) — let your AI assistant [search and read X API docs](/tools/mcp#docs-mcp-—-documentation-search) inline via `search_x` and `get_page_x` tools.

  You can run both servers together to look up documentation *and* call the API from the same assistant. See the [MCP guide](/tools/mcp) for client configs (Grok Build, Cursor, Claude, VS Code), headless setup, and troubleshooting.
</Update>
