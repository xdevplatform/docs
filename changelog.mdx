...
<Update label="Sep 22, 2026" description="X API v2">
  ### `is_moderator` on `broadcast.chat` events

  The [`broadcast.chat`](/x-api/activity/event-payloads#broadcast-chat) event delivered by the X Activity API now includes an `is_moderator` boolean alongside `is_subscriber`. Use it to identify chat messages sent by broadcast moderators. The broadcast owner is reported as `is_moderator: false` in their own broadcast.

  See [Event payloads](/x-api/activity/event-payloads#broadcast-chat) for the full `broadcast.chat` schema.
</Update>
