...
<Update label="Aug 29, 2026" description="X API v2">
  ...
  We added an optional `filter.qualifiers` map to [X Activity API](/x-api/activity/introduction) subscriptions. Qualifiers let you narrow which events match, so you only receive the events you need. Two qualifiers are supported today: `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Each subscription accepts up to 8 qualifiers, and unsupported keys or values return a `400` error. Learn more in [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).

  ### Broadcast lifecycle events in the X Activity API

  We added two public event types to the X Activity API: `broadcast.start` and `broadcast.end`. Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. Both events work with app-only or user-context authentication. See sample payloads in [Event payloads](/x-api/activity/event-payloads).

  ### New endpoint to check your Pay-Per-Use credit balance

  We added `GET /2/usage/credits`, which returns your Pay-Per-Use (PPU) credit balance in USD. You can now monitor your remaining credits programmatically. See the [API reference](/x-api/usage/get-usage-credits).

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload now includes an `is_subscriber` boolean alongside the chat message and author details, so you can tell whether a chat message came from a subscriber. See [Event payloads](/x-api/activity/event-payloads).
</Update>
