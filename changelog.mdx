...
<Update label="Aug 29, 2026" description="X API v2">
  ### This week: X Activity API events and qualifiers, PPU credit balance

  **New features**

  - **Broadcast lifecycle events in the X Activity API**: We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast. Both are public events, so you can subscribe with either app-only or user-context authentication.
  - **Filter qualifiers for X Activity API subscriptions**: Subscriptions now accept an optional `filter.qualifiers` map of event-specific predicates that narrow which events match, with up to 8 entries per subscription. Use `conversation_type` (`direct`, `group`) on `chat.received`, or `direction` (`inbound`, `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers) for details and examples.
  - **Credit balance endpoint**: The new [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) endpoint returns your Pay-Per-Use credit balance in USD, so you can monitor remaining credits programmatically. See the [Usage docs](/x-api/usage/introduction) for all usage endpoints.

  **Updates**

  - **`is_subscriber` on `broadcast.chat` events**: The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the chat message author is a subscriber of the broadcaster. See [Event payloads](/x-api/activity/event-payloads) for the full sample payload.
</Update>
