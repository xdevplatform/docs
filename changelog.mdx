...
<Update label="Sep 3, 2026" description="X API v2">
  ...
  We reduced the delivery latency of the X API v2 volume streaming endpoints. Streams that prioritize data hydration and delivery, such as the [Filtered Stream](/x-api/posts/filtered-stream/introduction), now have a P99 latency of approximately **4-5 seconds**, down from approximately 6-7 seconds. No integration changes are required.

  If you need the lowest possible latency, use [Powerstream](/x-api/powerstream/introduction). Learn more about delivery guarantees in [Consuming streaming data](/x-api/fundamentals/consuming-streaming-data).
</Update>
