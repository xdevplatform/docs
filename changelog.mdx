...
<Update label="Aug 31, 2026" description="X API v2">
  ### Broadcast lifecycle events for X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. Both are public events: you can subscribe with either app-only or user-context authentication.

  ### Filter qualifiers for X Activity API subscriptions

  We added an optional `qualifiers` map to X Activity API subscription filters, so you can narrow which events a subscription delivers. Supported qualifiers include `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Unsupported keys or values return a 400 error. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers) for details.

  ### New endpoint to check your credit balance

  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), which returns your Pay-Per-Use credit balance in USD. Use it to monitor remaining credits programmatically alongside the existing [usage endpoints](/x-api/usage/introduction).

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload in the X Activity API now includes an `is_subscriber` boolean, indicating whether the chat message author is a subscriber of the broadcaster. See [event payloads](/x-api/activity/event-payloads) for a sample.
</Update>
