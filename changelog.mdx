...
<Update label="Sep 1, 2026" description="X API v2">
  ### New: Bots endpoints for the Chat API

  We added six new `/2/bots` endpoints for creating and managing programmatic bot accounts owned by your project. You can create, list, update, and delete bots, and rotate or revoke their bearer tokens. Bot accounts carry an "Automated by @owner" label. See the [Bots guide](/xchat/bots) and the [API reference](/x-api/bots/get-bots).

  ### New: Delete Chat messages endpoint

  We added `POST /2/chat/conversations/{id}/messages/delete` to the [Chat API](/x-api/chat/delete-chat-messages). Delete one or more messages from a conversation, either for every participant (messages you sent, or any message in groups you administer) or from your own view only.

  ### New: Usage credits endpoint

  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), which returns your developer-platform credit balance in USD. Use it to monitor your prepaid, free, and total balances programmatically.

  ### New: Broadcast events for the X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. Both are public events, so you can subscribe with either app-only or user-context authentication.

  ### Updated: Filter qualifiers for X Activity API subscriptions

  X Activity API subscriptions now accept an optional `filter.qualifiers` map (up to 8 entries) to narrow which events match. Use `conversation_type` (`direct` or `group`) on `chat.received` events, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create` events. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).

  ### Updated: `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the message author is a subscriber of the broadcaster. See [Event payloads](/x-api/activity/event-payloads).
</Update>
