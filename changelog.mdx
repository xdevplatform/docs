...
<Update label="Aug 27, 2026" description="X API v2">
  ### This week: new Activity API events, filter qualifiers, and a credit balance endpoint

  #### New features

  - **Filter qualifiers for X Activity API subscriptions.** Subscriptions now accept an optional `filter.qualifiers` map to narrow which events match. Scope `chat.received` events by `conversation_type` (`direct` or `group`), and `follow.follow`, `follow.unfollow`, and `like.create` events by `direction` (`inbound` or `outbound`). Each subscription supports up to 8 qualifiers, and unsupported keys or values return a 400 error. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).
  - **Broadcast lifecycle events in the X Activity API.** Subscribe by User ID to the new public `broadcast.start` and `broadcast.end` events to be notified in real time when a user starts or ends a live broadcast. Both work with app-only or user-context authentication. See [event payloads](/x-api/activity/event-payloads).
  - **Credit balance endpoint.** The new `GET /2/usage/credits` endpoint returns your Pay-per-use (PPU) credit balance in USD, so you can monitor your remaining credits programmatically. See the [Usage docs](/x-api/usage/introduction).

  #### Updates

  - **`is_subscriber` on `broadcast.chat` events.** The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the chat sender is a subscriber of the broadcast owner.
  - **Pay-per-use monthly Post cap raised to 3 million.** Pay-per-use plans can now read up to 3 million Posts per month, up from 2 million. See [Post cap](/x-api/fundamentals/post-cap).
</Update>
