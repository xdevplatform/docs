...
<Update label="May 5, 2026" description="X API" >
### Self-serve replaces Basic and Pro tiers
Self-serve, pay-per-usage [pricing](/x-api/getting-started/pricing) is now the standard plan for the X API. References to the legacy Basic and Pro subscription tiers have been removed across pricing, enterprise, and billing support pages. If you previously used Basic or Pro, you can opt in to pay-per-use directly from the [Developer Console](https://console.x.com). Customers who need more than self-serve provides can still [apply for Enterprise access](/x-api/enterprise-gnip-2.0/enterprise-gnip).
</Update>
