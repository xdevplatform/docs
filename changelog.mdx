...
<Update label="Sep 9, 2026" description="X API v2">
  ### New OAuth 2.0 scopes for live broadcasts

  Today, we added two new scopes to [OAuth 2.0 Authorization Code Flow with PKCE](/fundamentals/authentication/oauth-2-0/authorization-code):
  ...
  Your app can now request broadcast access during user authorization. For example, `broadcast.read` is required to subscribe to the private `broadcast.chat` event in the [X Activity API](/x-api/activity/introduction).
</Update>
