...
<Update label="Jun 30, 2026" description="Developer tools">
  ### Hosted MCP servers for the X API and docs

  Today, we launched two hosted [Model Context Protocol servers](/tools/mcp) so AI tools like Grok, Cursor, Claude Desktop, and VS Code can talk to X directly:

  - **X MCP** (`https://api.x.com/mcp`) — call X API endpoints from any MCP-compatible client. The model can search the full archive, look up users, manage bookmarks, fetch trends and news, and draft or publish Articles using your own account's permissions. Connect via the open-source `xurl mcp` bridge, which handles OAuth 2.0 and token refresh for you.
  - **Docs MCP** (`https://docs.x.com/mcp`) — let your AI assistant search and read the X API documentation without leaving your editor.

  See the [MCP setup guide](/tools/mcp) for client-specific configuration for Grok Build, Cursor, Claude Desktop, VS Code, and any other MCP client.
</Update>
