...
<Update label="Sep 26, 2026" description="X API v2">
  ## New features

  ### Automatic expiration for X Activity API subscriptions

  You can now pass an optional `expires_at` timestamp (RFC 3339) when you create an X Activity API subscription. X deletes the subscription automatically when that time is reached. To change the expiration, `POST` the same subscription again with a new `expires_at`. See [Subscription expiration](/x-api/activity/introduction#subscription-expiration).

  ### Webhook signatures with your OAuth 2.0 client secret

  Webhook POSTs can now include an `X-Twitter-Webhooks-Signature-OAuth2` header, signed with your app's OAuth 2.0 client secret. We recommend verifying this header when present. You can also generate the CRC `response_token` with the OAuth 2.0 client secret. The legacy `X-Twitter-Webhooks-Signature` header (OAuth 1.0 consumer secret) is unchanged. See [Signature headers](/x-api/webhooks/introduction#signature-headers).

  ### `is_moderator` in `broadcast.chat` events
  ...
  - **OAuth 1.0a token exchange:** Exchanged tokens now include `broadcast.read` (Read) and `broadcast.write` (Read and write). See [permission mapping](/fundamentals/authentication/oauth-2-0/oauth-1-0a-token-exchange#what-permissions-do-the-new-tokens-get).
  - **Livestream API access:** Access is limited to whitelisted apps and no longer requires an Enterprise plan. Request access through the [Livestream API Access Form](/forms/livestream-api-access).
  - **XChat group members:** [Adding a member](/x-api/chat/add-members-to-a-chat-group-conversation) who has a pending join request now approves that request. Approval uses the same key-rotation body as adding any member.
  - **`source` Post field:** `source` is no longer a supported value for `tweet.fields` in X API v2. See [Post lookup](/x-api/posts/lookup/integrate).

  ## Bug fixes

  - **Batch compliance upload limit:** The upload limit is 128 MB per job (about 6.5 million IDs), not 1 GiB as previously stated. Larger uploads return `413`. Split large datasets across jobs, and group IDs from similar time periods for faster processing. See [Uploads](/x-api/compliance/batch-compliance/integrate#uploads).
</Update>
