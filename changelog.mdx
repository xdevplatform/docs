...
<Update label="Sep 2, 2026" description="X API v2">
  ...
  Today, we added bulk support to the [create Bookmark endpoint](/x-api/users/create-bookmark) (`POST /2/users/{id}/bookmarks`). You can now pass `tweet_ids` (up to 25 Post IDs) to bookmark multiple Posts in a single request. Passing `tweet_id` for a single Post continues to work.

  - An optional `folder_id` adds every Post in the request to the same Bookmark folder.
  - Bulk responses return the bookmarked Posts in request order, plus per-ID `errors` for any failures. The remaining IDs are still bookmarked.
  - Billing and rate limits apply per HTTP request.
</Update>
