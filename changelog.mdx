...
<Update label="Sep 6, 2026" description="X API v2">
  ### User-context authentication for webhooks and X Activity API subscriptions

  You can now authenticate [webhook management endpoints](/x-api/webhooks/introduction) (`GET`/`POST /2/webhooks`, `PUT`/`DELETE /2/webhooks/{webhook_id}`) with OAuth 2.0 user context, in addition to the existing app-only Bearer Token and OAuth 1.0a options.

  The [X Activity API](/x-api/activity/introduction) subscription update and delete endpoints (`PUT`/`DELETE /2/activity/subscriptions/{subscription_id}`) now also accept OAuth 1.0a and OAuth 2.0 user context, in addition to app-only Bearer Tokens.

  ### Lower streaming latency

  We reduced the P99 latency of the X API v2 [streaming endpoints](/x-api/fundamentals/consuming-streaming-data), including the [Filtered Stream](/x-api/posts/filtered-stream/introduction), from approximately 6-7 seconds to approximately **4-5 seconds**. You receive fully hydrated Posts faster, with no changes required on your side. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction).
</Update>
