...
<Update label="Sep 9, 2026" description="X API">
  ### Block and unblock endpoints for Enterprise

  We added [`POST /2/users/{id}/blocking`](/x-api/users/block-user) and [`DELETE /2/users/{source_user_id}/blocking/{target_user_id}`](/x-api/users/unblock-user) to the X API v2. You can now block and unblock users on behalf of an authenticated user. These endpoints are only available on the [Enterprise plan](/enterprise-api/introduction) and are not available on self-serve (Pay-Per-Use) tiers.

  ### Subscription events for X Activity API

  We added `subscriptions.subscribe` and `subscriptions.unsubscribe` events to the [X Activity API](/x-api/activity/introduction). You can now subscribe by User ID to receive real-time notifications when a user subscribes to, or unsubscribes from, another user via the paid subscriptions button on a profile. Both events support the `direction` filter (`inbound`, `outbound`), which is also now supported on `follow.follow` and `follow.unfollow`.

  These are private events: they require user-context (OAuth 2.0) authentication with the `follows.read` scope. Note that `subscriptions.unsubscribe` is emitted when the subscription actually expires, not upon cancellation.

  ### Broadcast scopes for OAuth 2.0
