...
<Update label="Jul 6, 2026" description="X API v2">
  ### X Activity API: new private events and higher subscription limits

  We expanded the [X Activity API](/x-api/activity/introduction) with new private events and raised subscription limits across all tiers.

  **New private events** (require OAuth 2.0 user authorization):

  - `chat.received`, `chat.sent` — encrypted chat messages sent and received
  - `dm.received`, `dm.sent`, `dm.read`, `dm.indicate_typing` — legacy DM activity, including read receipts and typing indicators
  - `post.mention.create` — @mentions of the filtered user
