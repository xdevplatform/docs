...
<Update label="Sep 24, 2026" description="X API v2">
  ### Batch compliance upload limit is 128 MB

  Batch compliance uploads accept up to **128 MB** per job (about 6.5 million IDs). We previously listed the limit as 1 GiB. Uploads over 128 MB return `413`.

  - For larger datasets, split IDs across multiple sequential jobs.
  - For faster processing, group IDs from a similar time period into the same job. Jobs whose IDs span many years take much longer to process.

  See the [batch compliance integration guide](/x-api/compliance/batch-compliance/integrate#uploads) for upload and error-handling details.
</Update>
