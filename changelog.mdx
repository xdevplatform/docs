...
<Update label="Sep 17, 2026" description="This week on the X Ads API">
  ### Week of Sep 15–17, 2026

  A quick recap of what shipped this week on the X Ads API.

  **New features**

  - **Lead Generation API.** Create and manage native lead forms, attach them to cards with the `LEAD_FORM` destination, and export submitted leads as JSON, CSV, or XLSX. Requires the Campaign user permission; lead exports are restricted to account administrators. See [Lead Generation API](/x-ads-api/lead-generation).
  - **Conversion API template for Google Tag Manager (server-side).** Send website conversions through the official **X Ads Conversion API (Official)** tag in a GTM server container, without writing Conversion API request code. See [Set up the Conversion API with Google Tag Manager](/x-ads-api/measurement/gtm-server-side).

  **Updates**

  - **`REGIONS` segmentation for Analytics.** Asynchronous analytics jobs now accept `REGIONS` as a `segmentation_type`. The `country` parameter is required when requesting a regional breakdown. See [Metrics and Segmentation](/x-ads-api/analytics#metrics-and-segmentation).
</Update>
