...
<Update label="Sep 3, 2026" description="X API v2">
  ...
  We improved delivery latency on the X API v2 [streaming endpoints](/x-api/fundamentals/consuming-streaming-data). Volume streams and the [Filtered Stream](/x-api/posts/filtered-stream/introduction) now deliver hydrated Post data with a **P99 latency of approximately 4-5 seconds**, down from approximately 6-7 seconds.

  No changes are required on your end. If you need the lowest possible latency, [Powerstream](/x-api/powerstream/introduction) remains the fastest option.
</Update>
