...
<Update label="Aug 28, 2026" description="X API v2">
  ### Weekly roundup

  #### New features

  - **Chat API bot endpoints**: You can now create and manage programmatic bot accounts for your project via six new `/2/bots` endpoints: create, list, update, and delete bots, plus rotate and revoke bot tokens. See the [Bots guide](/xchat/bots) and [API reference](/x-api/bots/create-a-bot).
  - **Broadcast events for X Activity API**: Subscribe by User ID to two new public events, `broadcast.start` and `broadcast.end`, to be notified in real time when a user starts or ends a live broadcast. See the [event payloads](/x-api/activity/event-payloads).
  - **Filter qualifiers for X Activity API subscriptions**: Subscriptions now accept an optional `filter.qualifiers` map (up to 8 entries) to narrow which events match, such as `conversation_type` on `chat.received` and `direction` on `follow.follow`, `follow.unfollow`, and `like.create`. Unsupported keys or values return a 400 error. Learn more in the [introduction](/x-api/activity/introduction#filter-qualifiers).
  - **Credit balance endpoint**: `GET /2/usage/credits` returns your Pay-Per-Use credit balance in USD, including total, prepaid, and free balances plus unexpired free grants. See the [API reference](/x-api/usage/get-usage-credits).
  - **Delete Chat messages endpoint**: `POST /2/chat/conversations/{id}/messages/delete` deletes one or more messages from a Chat conversation. See the [API reference](/x-api/chat/delete-chat-messages).

  #### Updates

  - **Pay-Per-Use monthly Post cap raised**: The monthly cap for Pay-Per-Use plans increased from 2 million to 3 million Post reads per billing cycle. See [Post caps](/x-api/fundamentals/post-cap).
  - **`is_subscriber` on `broadcast.chat` events**: The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the message author subscribes to the broadcaster.
</Update>
