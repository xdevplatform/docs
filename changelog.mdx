...
<Update label="Aug 14, 2026" description="X Ads API">
  ### New `WEB_CONVERSION` metrics

  We added eight new metrics to the `WEB_CONVERSION` metric group on the [X Ads API Analytics endpoints](/x-ads-api/analytics). You can now request the following metrics alongside the existing web conversion set:
  ...
  Each returns the number of conversions for that event type along with the corresponding sale amount and order quantity, and supports the `PLATFORMS` and `CONVERSION_TAGS` segmentations.
</Update>
