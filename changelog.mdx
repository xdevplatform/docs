...
<Update label="Jul 24, 2026" description="X API v2">
  ### Event payload reference for the X Activity API

  Today, we published a new [Event payloads](/x-api/activity/event-payloads) reference for the [X Activity API](/x-api/activity/introduction) with sample JSON for every supported event type — posts, likes, follows, profile updates, chat, DMs, mutes, blocks, Spaces, news, and OAuth revokes.

  Use it to model your webhook handlers and JSON deserialization against real, live-captured payloads before wiring up a subscription.
</Update>
