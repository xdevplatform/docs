...
<Update label="Sep 9, 2026" description="X API v2">
  ...
  We improved P99 latency on the X API v2 [streaming endpoints](/x-api/fundamentals/consuming-streaming-data) from approximately 6-7 seconds to approximately **4-5 seconds**. This applies to streams that prioritize data hydration and delivery, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction). You now receive fully hydrated Posts faster, with no changes required on your end. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction).

  ### User context authentication for webhooks and Activity subscriptions

  [Webhook](/x-api/webhooks/introduction) management endpoints (`GET`/`POST /2/webhooks`, `PUT`/`DELETE /2/webhooks/{webhook_id}`) now accept OAuth 2.0 user context authentication. Updating or deleting an [X Activity API](/x-api/activity/introduction) subscription (`PUT`/`DELETE /2/activity/subscriptions/{subscription_id}`) now accepts OAuth 1.0a and OAuth 2.0 user context as well. App-only Bearer Token authentication continues to work on all of these endpoints.
</Update>
