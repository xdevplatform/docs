...
<Update label="Aug 24, 2026" description="X API v2">
  ...
  Today, we added the [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) endpoint. You can now programmatically check your Pay-Per-Use (PPU) credit balance in USD, making it easier to monitor spend and avoid interruptions. See the [Usage docs](/x-api/usage/introduction) for details.
</Update>

<Update label="Aug 21, 2026" description="X API v2">
  ### Broadcast events for X Activity API

  Today, we added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). You can now subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast. Both are public events and can be subscribed to with either app-only or user-context authentication.
</Update>

<Update label="Aug 13, 2026" description="X API">
  ### Pay-Per-Use monthly Post read cap raised to 3 million

  We increased the monthly Post read cap on Pay-Per-Use plans from 2 million to 3 million. If you need higher volume, consider an [Enterprise plan](/forms/enterprise-api-interest). Learn more about [Post caps](/x-api/fundamentals/post-cap).
</Update>
