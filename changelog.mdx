...
<Update label="Sep 17, 2026" description="X API v2">
  ### Download DM media endpoint

  The [`GET /2/dm_conversations/media/{dm_id}/{media_id}/{resource_id}`](/x-api/direct-messages/download-dm-media) endpoint for downloading media attached to a Direct Message is now listed under **Direct Messages > Lookup** in the sidebar, making it easier to find.
</Update>

<Update label="Sep 17, 2026" description="X API v2">
  ### `liking_user_id` on Account Activity like events

  The `like_events` payload delivered by the [Account Activity API](/x-api/activity/event-payloads) now documents the `liking_user_id` field, which identifies the user who liked the Post. The corresponding user object is also included in the `users` array of the event.
</Update>
