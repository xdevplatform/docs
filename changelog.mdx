...
<Update label="Sep 4, 2026" description="X API v2">
  ### More authentication options for webhooks and X Activity subscriptions

  We expanded the accepted authentication methods on the [webhook management endpoints](/x-api/webhooks/introduction) and the [X Activity API](/x-api/activity/introduction) subscription endpoints:

  - `POST`, `PUT`, and `DELETE /2/webhooks` now accept OAuth 2.0 user context in addition to OAuth 1.0a user context and app-only Bearer Token authentication.
  - `GET /2/webhooks` and the X Activity subscription update and delete endpoints (`PUT` / `DELETE /2/activity/subscriptions/{subscription_id}`) previously required an app-only Bearer Token. They now also accept OAuth 1.0a and OAuth 2.0 user context.
...
<Update label="Sep 2, 2026" description="X API v2">
  ...
  We improved delivery latency for the X API v2 [streaming endpoints](/x-api/fundamentals/consuming-streaming-data). Streams that prioritize full data hydration, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction), now deliver with a **P99 latency of approximately 4-5 seconds**, down from approximately 6-7 seconds. No changes are required on your side.

  If you need the lowest possible latency, use [Powerstream](/x-api/powerstream/introduction).
</Update>
