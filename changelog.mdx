...
<Update label="Sep 22, 2026" description="X API v2">
  ...
  You can now pass an optional `expires_at` (RFC 3339 timestamp) when creating an [X Activity API](/x-api/activity/introduction) subscription. When that time is reached, XAA automatically deletes the subscription so you don't have to clean it up.

  - Omit `expires_at` and the subscription stays active until you delete it, the app is deleted, or the user revokes access.
  - To extend or shorten expiration, POST the same subscription again with a new `expires_at`. XAA refreshes the existing subscription and updates the value.
  - `PUT /2/activity/subscriptions/:id` updates `tag` and `webhook_id` only. Use re-creation to change `expires_at`.
  - Create, list, and update responses echo `expires_at` back when it is set.

  See [Subscription expiration](/x-api/activity/introduction#subscription-expiration) for details.
</Update>
