...
<Update label="Sep 7, 2026" description="X API v2">
  ...
  - **Faster streaming delivery**: [X API v2 streaming endpoints](/x-api/fundamentals/consuming-streaming-data) now deliver hydrated Post data with a P99 latency of approximately **4-5 seconds**, down from 6-7 seconds. This includes [filtered stream](/x-api/posts/filtered-stream/introduction). If you need the lowest possible latency, use [Powerstream](/x-api/powerstream/introduction).
  - **OAuth 2.0 user context for webhooks**: The [webhooks management endpoints](/x-api/webhooks/introduction) (`GET` and `POST /2/webhooks`, `PUT` and `DELETE /2/webhooks/:webhook_id`) now accept OAuth 2.0 user context authentication. `GET /2/webhooks` also now accepts OAuth 1.0a user context.
  - **More authentication options for X Activity API subscriptions**: `PUT` and `DELETE /2/activity/subscriptions/:subscription_id` now accept OAuth 1.0a and OAuth 2.0 user context in addition to the app-only Bearer Token. See the [X Activity API](/x-api/activity/introduction).
</Update>
