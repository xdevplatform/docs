...
<Update label="Sep 15, 2026" description="Livestream API">
  ### Broadcast chat moderator endpoints

  You can now manage chat moderators for your broadcasts through the Livestream API. Moderators are scoped to the authenticated user and take effect across every broadcast that user hosts.

  - [GET /2/broadcasts/chat/moderators](/livestream-api/chat/list-moderators) lists the authenticated user's chat moderators
  - [POST /2/broadcasts/chat/moderators](/livestream-api/chat/add-moderator) adds a user as a chat moderator
  - [DELETE /2/broadcasts/chat/moderators/{id}](/livestream-api/chat/remove-moderator) removes a chat moderator

  Each user can have up to 20 moderators; adding a 21st returns `400 Bad Request`. All three endpoints support OAuth 2.0 user context and OAuth 1.0a.
</Update>
