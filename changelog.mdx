...
<Update label="Aug 26, 2026" description="X API v2">
  ...
  Today, we added an optional `filter.qualifiers` map to [X Activity API](/x-api/activity/introduction) subscriptions. Qualifiers are event-specific predicates that narrow which events match, so you only receive the activity you care about. You can set `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. A subscription supports up to 8 qualifiers, and unsupported keys or values return a 400 error. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers) for details.

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload now includes an `is_subscriber` boolean, so you can tell whether the chat message author is a subscriber of the broadcaster. See the [event payloads reference](/x-api/activity/event-payloads) for a sample payload.
</Update>

<Update label="Aug 24, 2026" description="X API v2">
  ### New endpoint to check your PPU credit balance

  Today, we added `GET /2/usage/credits` to the [Usage endpoints](/x-api/usage/introduction). It returns your current Pay-Per-Use (PPU) credit balance in USD, so you can monitor spend and trigger top-ups programmatically instead of checking the Developer Console.
</Update>

<Update label="Aug 21, 2026" description="X API v2">
  ### Broadcast events for X Activity API

  Today, we added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. These are public events and work with both app-only and user-context authentication.
</Update>

<Update label="Aug 14, 2026" description="X API">
  ### Legacy Pro plans moving to Pay-Per-Use

  Today, we announced that legacy X API Pro plans (monthly and annual) are deprecated and all Pro subscribers will be automatically migrated to [Pay-Per-Use (PPU)](/x-api/getting-started/pricing). Starting September 1, 2026, each Pro subscription ends at the close of its current billing cycle. No action is required. Pro Monthly subscribers receive a \$5000 initial PPU credit at cutover; Pro Annual subscribers receive their remaining prorated annual value as PPU credits with no charge at cutover. Migrated accounts default to auto-recharge of \$5000 when the balance falls below \$250 and a \$10000 spend cap per billing cycle. Both settings are adjustable in the Developer Portal. Learn more [here](https://devcommunity.x.com/t/important-update-legacy-x-api-pro-plans-are-moving-to-pay-per-use-ppu/273255).
</Update>

<Update label="Aug 13, 2026" description="X API">
  ### Pay-Per-Use monthly Post cap raised to 3 million

  We raised the monthly Post read cap on Pay-Per-Use plans from 2 million to 3 million Posts. See [Post cap](/x-api/fundamentals/post-cap) for details.
</Update>
