...
<Update label="Aug 26, 2026" description="X API v2">
  ...
  **Check your Pay-Per-Use credit balance**
  ...
  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) to the [Usage endpoints](/x-api/usage/introduction). It returns your developer platform credit balance in USD, so you can monitor remaining credits programmatically and top up before you run out. It supports both app-only and user-context (OAuth 2.0) authentication.
  ...
  **Pay-Per-Use monthly Post cap raised to 3 million**
  ...
  We raised the monthly [Post cap](/x-api/fundamentals/post-cap) on Pay-Per-Use plans to 3 million Post reads per billing cycle. If you need higher volume, consider an [Enterprise plan](/forms/enterprise-api-interest).

  #### Bug fixes

  **Trends by WOEID response corrected**

  `GET /2/trends/by/woeid/{woeid}` returns `trend_name` only; it does not return `tweet_count`. We corrected the [endpoint reference](/x-api/trends/trends-by-woeid/introduction) to match this behavior. [Personalized Trends](/x-api/trends/personalized-trends/introduction) is unaffected and still returns `tweet_count`.
</Update>
