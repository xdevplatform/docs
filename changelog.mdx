...
<Update label="Sep 8, 2026" description="X API v2">
  ### Lower latency on streaming endpoints

  X API v2 streaming endpoints that prioritize data hydration and delivery, including [Filtered Stream](/x-api/posts/filtered-stream/introduction) and [Volume Streams](/x-api/posts/volume-streams/introduction), now deliver hydrated Post data with a P99 latency of approximately **4-5 seconds**, down from 6-7 seconds. You receive matching Posts faster with no changes required on your end. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction).

  ### User context authentication for webhook and subscription management

  The [webhook management endpoints](/x-api/webhooks/introduction) (`GET`/`POST /2/webhooks` and `DELETE`/`PUT /2/webhooks/{webhook_id}`) now accept OAuth 2.0 user context authentication. The [X Activity API](/x-api/activity/introduction) endpoints `PUT` and `DELETE /2/activity/subscriptions/{subscription_id}` now accept OAuth 2.0 and OAuth 1.0a user context. You can now manage webhooks and Activity API subscriptions with user-context tokens instead of only app-only Bearer Tokens.
</Update>
