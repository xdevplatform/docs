...
<Update label="Jun 18, 2026" description="X API">
  ### Pricing page updates: auto-recharge safeguards, webhook event pricing, and clearer xAI credit tiers

  Today, we refreshed the [X API pricing page](/x-api/getting-started/pricing) with several user-facing clarifications:

  - **Auto-recharge safeguards** — auto-recharge now triggers at most once every 5 minutes and is paused while your balance is zero or negative, protecting against runaway charges.
  - **Webhook event pricing** — added a per-event price table for [X Activity API](/x-api/activity/introduction) events (e.g., `post.create`, `follow.follow`, `dm.received`, `spaces.start`), deduplicated within the same 24-hour UTC window as other resources.
  - **Free xAI API credit tiers** — clarified the cumulative-spend reward tiers (10% at &dollar;200+, 15% at &dollar;500+, 20% at &dollar;1,000+) with a worked example so you can see exactly what you'll receive per purchase.
  - **Enterprise comparison** — the [Enterprise pricing page](/enterprise-api/getting-started/pricing) now calls out that pay-per-use plans include a 2 million monthly post read cap, while Enterprise plans offer custom or unlimited volumes.
</Update>
