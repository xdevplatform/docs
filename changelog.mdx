...
<Update label="Aug 31, 2026" description="X API v2">
  ...
  We added an optional `filter.qualifiers` map to [X Activity API](/x-api/activity/introduction) subscriptions. Qualifiers narrow which events match a subscription: use `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Each subscription supports up to 8 qualifiers, and responses echo them back. Learn more in [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).

  ### Broadcast start and end events for X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). You can now subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast. Both are public events, so app-only or user-context authentication works. See the [sample payloads](/x-api/activity/event-payloads).

  ### Usage credits endpoint

  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), which returns your Pay-Per-Use credit balance in USD. Use it to monitor your remaining credits programmatically. See the [Usage endpoints](/x-api/usage/introduction) for details.

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the chat message author is a subscriber. See the [event payloads reference](/x-api/activity/event-payloads).
</Update>
