...
<Update label="Sep 3, 2026" description="X API v2">
  ### Lower latency for streaming endpoints

  We improved delivery latency for the X API v2 streaming endpoints that prioritize data hydration, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction). P99 latency is now approximately **4-5 seconds**, down from approximately 6-7 seconds. You still receive fully hydrated Posts with all metadata, just faster. No changes are required on your side.

  See [consuming streaming data](/x-api/fundamentals/consuming-streaming-data) for delivery guarantees. If you need the lowest possible latency, use [Powerstream](/x-api/powerstream/introduction).
</Update>
