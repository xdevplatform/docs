...
<Update label="Jun 30, 2026" description="Developer platform">
  ...
  Today, we launched two hosted [Model Context Protocol](/tools/mcp) servers so AI tools like Grok, Cursor, Claude, and VS Code can work with X directly:

  - **X MCP** at `https://api.x.com/mcp` — call X API endpoints (post and user lookup, full-archive search, bookmarks, trends, news, drafting and publishing Articles) using your own X account's permissions. Connect via the open-source [`xurl mcp`](https://github.com/xdevplatform/xurl) bridge, which handles OAuth and token refresh for you.
  - **Docs MCP** at `https://docs.x.com/mcp` — search and read X API documentation from inside your AI tool, no auth required.

  See the [MCP setup guide](/tools/mcp) for client configs (Grok Build, Cursor, Claude, VS Code), headless login, and troubleshooting.
</Update>
