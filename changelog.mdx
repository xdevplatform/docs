...
<Update label="Sep 2, 2026" description="X API v2">
  ...
  [X Activity API](/x-api/activity/introduction) subscriptions now accept an optional `filter.qualifiers` map of event-specific predicates that narrow which events match, with up to 8 entries per subscription. Supported today:

  - `conversation_type` (`direct`, `group`) on `chat.received`
  - `direction` (`inbound`, `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`

  Responses echo `filter.qualifiers` back, and unsupported keys or values return a **400** with a descriptive message. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers) for details.

  ### Broadcast start and end events for X Activity API

  We added `broadcast.start` and `broadcast.end` event types to the X Activity API. Subscribe by User ID to receive real-time notifications when a user starts or ends a live broadcast. Both are public events, so you can subscribe with either app-only or user-context authentication. Sample payloads are in [Event payloads](/x-api/activity/event-payloads).

  ### Pay-Per-Use credit balance endpoint

  The new [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) endpoint returns your Pay-Per-Use (PPU) credit balance in USD, so you can monitor remaining credits programmatically alongside the existing [Usage](/x-api/usage/introduction) endpoint.

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the chat message author is a subscriber. See the updated sample in [Event payloads](/x-api/activity/event-payloads#broadcast-chat).
</Update>
