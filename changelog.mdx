...
<Update label="Sep 15, 2026" description="X API v2">
  ### `post_id` qualifier for `like.create` on X Activity API

  We added a `post_id` [filter qualifier](/x-api/activity/introduction#filter-qualifiers) to `like.create` events on the [X Activity API](/x-api/activity/introduction). Set `filter.qualifiers.post_id` on a subscription to receive `like.create` events only for likes on a specific Post, so you can track engagement on individual Posts without filtering the full user-level like stream on your side.
</Update>
