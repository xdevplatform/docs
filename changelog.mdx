...
<Update label="May 21, 2026" description="X API">
  ### Legacy Basic plans migrating to Pay-Per-Use

  Today, we announced that the legacy X API **Basic** plans (monthly and annual) are being deprecated, with automatic migration to [Pay-Per-Use (PPU)](https://developer.x.com/en/portal/products) starting **June 1, 2026**. After your cutover date (the end of your current billing cycle), your Basic subscription ends and PPU takes over with no fixed monthly fee.

  - **Basic Monthly subscribers**: a **\$200** initial credit will be charged via Stripe.
  - **Basic Annual subscribers**: the remaining prorated value is granted as PPU credits (no card charge at cutover).
  - **Default PPU settings**: auto-recharge enabled at **\$200** when balance falls below **\$10**, with a **\$400** per-cycle spend cap. Adjust anytime in the [Developer Portal](https://developer.x.com/en/portal/dashboard).

  If you don't want to migrate, you can opt out in the Developer Portal before June 1, 2026 — note that opting out ends API access after your cutover date. Learn more [here](https://devcommunity.x.com/t/important-update-legacy-x-api-basic-plans-are-moving-to-pay-per-use-ppu/266305).
</Update>

<Update label="May 20, 2026" description="X API">
  ### Fix: auto-recharge now triggers for free credits

  Today, we shipped a fix to the auto-recharge feature so that free credits (from redeemed vouchers) are treated the same as paid credits. Previously, when a free credit balance reached zero, auto-recharge would not trigger even if it was enabled. Effective **May 21, 2026**, if your balance is depleted and auto-recharge is enabled in the [Developer Console](https://console.x.com), your account will recharge automatically using your saved payment method.

  If you don't want to incur charges after your free credits run out, review your auto-recharge setting in the Developer Console and disable it if needed. Learn more [here](https://devcommunity.x.com/t/update-to-auto-recharge-behavior-for-free-credits/266184).
</Update>
