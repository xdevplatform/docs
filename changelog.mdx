...
<Update label="Aug 30, 2026" description="X API v2">
  ### New features
  ...
  - **Filter qualifiers for X Activity API subscriptions**: Subscriptions now accept an optional `filter.qualifiers` map (up to 8 entries) to narrow which events match. Use `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).
  - **Usage credits endpoint**: The new [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) endpoint returns your Pay-per-use (PPU) credit balance in USD, so you can monitor remaining credits programmatically. See the [Usage endpoints](/x-api/usage/introduction).

  ### Updates

  - **`is_subscriber` on `broadcast.chat` events**: The `broadcast.chat` event payload now includes an `is_subscriber` boolean indicating whether the chat author is a subscriber. See [event payloads](/x-api/activity/event-payloads).
  - **Higher Pay-per-use Post cap**: The monthly Post read cap on Pay-per-use plans increased from 2 million to 3 million. See [Post cap](/x-api/fundamentals/post-cap).
</Update>
