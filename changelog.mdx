...
<Update label="Sep 1, 2026" description="X API">
  ### Bots endpoints for programmatic accounts

  We launched the [Bots endpoints](/xchat/bots), six new `/2/bots` endpoints for creating and managing programmatic bot accounts that belong to your project. Create a bot, update its identity, and mint, rotate, or revoke its bearer token using OAuth 2.0 app-only authentication. Bot accounts carry an "Automated by @owner" label. See the [API reference](/x-api/bots/create-a-bot).

  ### X Ads MCP server

  We launched the [X Ads MCP](/x-ads-api/mcp), a remote Model Context Protocol server built into X's API gateway. Any MCP-capable client (Grok, Claude Code, or a custom agent) can connect and use 23 Ads tools for campaign reads, analytics, targeting search, and writes. Authentication uses the user's own OAuth 2.0 token, and campaigns and line items are always created `PAUSED`, so nothing spends money until explicitly activated.

  ### Broadcast events for X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. Both are public events, so app-only or user-context authentication works.

  ### Usage credits endpoint

  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), which returns your developer-platform credit balance in USD. Pay-Per-Use developers can now track their balance programmatically alongside the existing [usage endpoint](/x-api/usage/introduction).

  ### Filter qualifiers for X Activity API subscriptions

  Subscriptions now accept an optional `filter.qualifiers` map (up to 8 entries) of event-specific predicates that narrow which events match. Supported qualifiers include `conversation_type` (`direct`, `group`) for `chat.received` and `direction` (`inbound`, `outbound`) for follow and like events. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).

  ### Pay-Per-Use monthly Post cap raised

  The monthly cap on Post reads for Pay-Per-Use plans increased from 2 million to 3 million. Learn more about the [monthly Post cap](/x-api/fundamentals/post-cap).
</Update>
