...
<Update label="Sep 7, 2026" description="X API v2">
  ### Faster streaming delivery

  We reduced the P99 latency of our hydrated streaming endpoints, including [Filtered Stream](/x-api/posts/filtered-stream/introduction), from approximately 6-7 seconds to approximately 4-5 seconds. Matching Posts now reach your stream sooner, with no changes required on your side. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction). Learn more about [consuming streaming data](/x-api/fundamentals/consuming-streaming-data).

  ### User-context authentication for webhook and subscription management

  The [webhook management endpoints](/x-api/webhooks/introduction) (`GET` and `POST /2/webhooks`, `PUT` and `DELETE /2/webhooks/{webhook_id}`) and the X Activity API subscription [update](/x-api/activity/update-x-activity-subscription) and [delete](/x-api/activity/deletes-x-activity-subscription) endpoints now accept OAuth 2.0 user context and OAuth 1.0a user context authentication, in addition to app-only Bearer tokens. You can now manage webhooks and subscriptions with the same user-context credentials your app already uses.
</Update>
