...
<Update label="Sep 23, 2026" description="X API v2">
  ### `broadcast` scopes added to OAuth 1.0a → OAuth 2.0 token exchange

  When you exchange an OAuth 1.0a user token for an OAuth 2.0 token, the granted scopes now include `broadcast.read` for **Read** permissions and `broadcast.write` for **Read and write** permissions. Existing users with these OAuth 1.0a permissions can call the broadcast endpoints under OAuth 2.0 without a new consent screen.

  See the [permission-to-scope mapping](/fundamentals/authentication/oauth-2-0/oauth-1-0a-token-exchange) on the OAuth 1.0a token exchange page.
</Update>
