...
<Update label="Aug 29, 2026" description="X API v2">
  ### Broadcast lifecycle events in the X Activity API
  ...
  X Activity API subscriptions now accept an optional `filter.qualifiers` map to narrow which events match, with up to 8 entries per subscription. Currently supported: `conversation_type` (`direct`, `group`) on `chat.received`, and `direction` (`inbound`, `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Unsupported keys or values return a 400 error. Learn more in the [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers) section.

  ### Credit balance endpoint

  We added [`GET /2/usage/credits`](/x-api/usage/get-usage-credits), which returns your Pay-Per-Use credit balance in USD. Use it to monitor remaining credits programmatically.

  ### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload in the [X Activity API](/x-api/activity/event-payloads) now includes an `is_subscriber` boolean indicating whether the message author is a subscriber of the broadcaster.

  ### Trends by WOEID: `tweet_count` removed

  `GET /2/trends/by/woeid/{woeid}` does not return `tweet_count`, and the field has been removed from the `trend.fields` parameter and response schema. The endpoint returns `trend_name` only. The `/2/users/personalized_trends` endpoint is unaffected and still returns `tweet_count`.
</Update>
