...
**Marketplace handle search returns product tier**
The marketplace handle search response now includes a `product_tier` field, indicating the product tier of the handle (for example, `premium`). Use this to filter and surface handles by tier in your integrations.

**Removed legacy chat conversation endpoint**
The `GET /2/chat/conversations/{id}` endpoint has been retired. Use the remaining [Chat endpoints](/x-api/chat/get-chat-conversations) to retrieve conversations and messages.
</Update>
