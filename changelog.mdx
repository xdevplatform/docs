...
<Update label="Sep 6, 2026" description="X API v2">
  ### OAuth 2.0 user context for webhook and X Activity subscription management

  We added OAuth 2.0 user context as a supported authentication method on the [webhook management endpoints](/x-api/webhooks/introduction) (`GET /2/webhooks`, `POST /2/webhooks`, `PUT /2/webhooks/{webhook_id}`, `DELETE /2/webhooks/{webhook_id}`) and on the [X Activity API](/x-api/activity/introduction) subscription endpoints `PUT /2/activity/subscriptions/{subscription_id}` and `DELETE /2/activity/subscriptions/{subscription_id}`.

  You can now manage webhooks and update or delete Activity subscriptions with OAuth 2.0 user tokens, in addition to the previously supported methods. No changes are required to existing integrations.
</Update>

<Update label="Sep 2, 2026" description="X API v2">
  ### Lower streaming latency: P99 now ~4-5 seconds

  We improved delivery latency on the [high-volume streaming endpoints](/x-api/fundamentals/consuming-streaming-data), including the [Filtered Stream](/x-api/posts/filtered-stream/introduction). P99 latency is now approximately **4-5 seconds**, down from ~6-7 seconds.

  No action is needed. If you need the lowest possible latency, see [Powerstream](/x-api/powerstream/introduction).
</Update>
