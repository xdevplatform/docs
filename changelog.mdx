...
<Update label="Sep 4, 2026" description="X API v2">
  ...
  We improved delivery latency on the X API v2 streaming endpoints. [Filtered Stream](/x-api/posts/filtered-stream/introduction) and the volume streams now deliver hydrated Posts with a **P99 latency of ~4-5 seconds**, down from ~6-7 seconds. No changes are required on your end. See [consuming streaming data](/x-api/fundamentals/consuming-streaming-data) for delivery guarantees, and [Powerstream](/x-api/powerstream/introduction) if you need the lowest possible latency.

  ### More authentication options for webhook and subscription management

  The [webhook management endpoints](/x-api/webhooks/introduction) (`GET`/`POST /2/webhooks`, `PUT`/`DELETE /2/webhooks/{webhook_id}`) now accept OAuth 2.0 user context authentication, in addition to the existing options. The [X Activity API](/x-api/activity/introduction) subscription update and delete endpoints (`PUT`/`DELETE /2/activity/subscriptions/{subscription_id}`) now accept OAuth 2.0 and OAuth 1.0a user context, in addition to the app-only Bearer token.
</Update>
