...
<Update label="Aug 28, 2026" description="X API v2">
  ...
  [X Activity API](/x-api/activity/introduction) subscriptions now accept an optional `filter.qualifiers` map of event-specific predicates, so you receive only the events you care about. Use `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Each subscription supports up to 8 qualifiers. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers) for details.

  #### Broadcast lifecycle events for X Activity API

  We added `broadcast.start` and `broadcast.end` events to the [X Activity API](/x-api/activity/introduction). Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. Both are public events, so you can subscribe with either app-only or user-context authentication. Sample payloads are available in the [event payloads reference](/x-api/activity/event-payloads).

  #### New `GET /2/usage/credits` endpoint

  You can now check your Pay-Per-Use credit balance programmatically. The new [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) endpoint returns your PPU credit balance in USD, making it easier to monitor spend and avoid interruptions.

  ### Updates

  #### `is_subscriber` field on `broadcast.chat` events

  The `broadcast.chat` event payload now includes an `is_subscriber` boolean, so you can tell whether a chat message came from a subscriber. See the [event payloads reference](/x-api/activity/event-payloads).

  #### Pay-Per-Use monthly Post cap raised to 3 million

  The monthly [Post read cap](/x-api/fundamentals/post-cap) on Pay-Per-Use plans increased from 2 million to 3 million Posts. If you need higher volume, consider an Enterprise plan.
</Update>
