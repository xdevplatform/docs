...
<Update label="Sep 1, 2026" description="X API v2">
  ### This week: Chat API bots, broadcast events, filter qualifiers, and more

  #### New features

  - **Bot accounts for the Chat API.** You can now create and manage programmatic bot accounts for your project with the new `/2/bots` endpoints: create, list, update, and delete bots, plus rotate and revoke their bearer tokens. Bots are real X accounts that authenticate with a dedicated token and carry an "Automated by" label. See the [Bots guide](/xchat/bots) and [API reference](/x-api/bots/create-a-bot).
  - **Broadcast lifecycle events in the X Activity API.** We added `broadcast.start` and `broadcast.end` public events. Subscribe by User ID to be notified in real time when a user starts or ends a live broadcast. See [event payloads](/x-api/activity/event-payloads#broadcast-events).
  - **Filter qualifiers for X Activity API subscriptions.** Subscriptions now accept an optional `filter.qualifiers` map to narrow which events match, for example `conversation_type` (`direct` or `group`) on `chat.received`, and `direction` (`inbound` or `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. See [Filter qualifiers](/x-api/activity/introduction#filter-qualifiers).
  - **Credit balance endpoint.** The new [`GET /2/usage/credits`](/x-api/usage/get-usage-credits) endpoint returns your Pay-per-use (PPU) credit balance in USD, so you can monitor spend programmatically.

  #### Updates

  - **Bulk bookmarking.** `POST /2/users/{id}/bookmarks` now accepts a `tweet_ids` array (up to 25 Post IDs) to bookmark several Posts in one request, with an optional `folder_id` applied to all of them. The response reports per-ID results and failures. See [Create Bookmark](/x-api/bookmarks/create-bookmark).
  - **`is_subscriber` on `broadcast.chat` events.** The `broadcast.chat` event payload in the X Activity API now includes an `is_subscriber` boolean indicating whether the chat author subscribes to the broadcaster.
</Update>
