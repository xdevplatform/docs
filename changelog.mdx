...
<Update label="Sep 1, 2026" description="X API v2">
  ### This week: Chat API bots, X Activity API filters and events, usage credits

  **New features**

  - **Bots for the Chat API**: You can now create and manage programmatic bot accounts for your project with the new `/2/bots` endpoints: create, list, update, and delete bots, plus rotate and revoke bot tokens. Bots authenticate with app-only bearer tokens. See the [Bots guide](/xchat/bots) and [API reference](/x-api/bots/create-a-bot).
  - **Filter qualifiers for the X Activity API**: Subscriptions now accept an optional `filter.qualifiers` map (up to 8 entries) to narrow which events match. Use `conversation_type` (`direct`, `group`) on `chat.received`, and `direction` (`inbound`, `outbound`) on `follow.follow`, `follow.unfollow`, and `like.create`. Unsupported keys or values return a 400 error. [Learn more](/x-api/activity/introduction#filter-qualifiers).
  - **Broadcast events for the X Activity API**: Subscribe by User ID to the new public `broadcast.start` and `broadcast.end` events to receive real-time notifications when a user starts or ends a live broadcast. Both work with app-only or user-context authentication. See [event payloads](/x-api/activity/event-payloads).
  - **Usage credits endpoint**: The new `GET /2/usage/credits` endpoint returns your Pay-Per-Use credit balance in USD. See the [API reference](/x-api/usage/get-usage-credits).

  **Updates**

  - **`is_subscriber` on `broadcast.chat`**: The `broadcast.chat` event payload now includes an `is_subscriber` boolean alongside the message and author details. See [event payloads](/x-api/activity/event-payloads#broadcast-chat).
</Update>
