...
<Update label="Sep 5, 2026" description="X API v2">
  ### Lower streaming latency: P99 now ~4-5 seconds

  We improved delivery latency across the X API v2 volume streaming endpoints, including the [Filtered Stream](/x-api/posts/filtered-stream/introduction). P99 latency is now approximately **4-5 seconds**, down from ~6-7 seconds. No changes are required on your side. See [consuming streaming data](/x-api/fundamentals/consuming-streaming-data) for delivery guarantees, or [Powerstream](/x-api/powerstream/introduction) if you need the lowest possible latency.

  ### More authentication options for webhooks and X Activity API subscriptions

  We expanded the authentication methods accepted by the [webhooks](/x-api/webhooks/introduction) and [X Activity API](/x-api/activity/introduction) management endpoints:

  - `POST /2/webhooks`, `PUT /2/webhooks/{webhook_id}`, and `DELETE /2/webhooks/{webhook_id}` now accept OAuth 2.0 user context in addition to OAuth 1.0a and app-only Bearer Token.
  - `GET /2/webhooks` now accepts OAuth 2.0 and OAuth 1.0a user context. It was previously app-only.
  - `PUT /2/activity/subscriptions/{subscription_id}` and `DELETE /2/activity/subscriptions/{subscription_id}` now accept OAuth 2.0 and OAuth 1.0a user context in addition to app-only Bearer Token.

  You can now manage webhooks and X Activity API subscriptions with the same user-context credentials your app already uses elsewhere.
</Update>
