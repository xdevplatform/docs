...
<Update label="Sep 7, 2026" description="X API v2">
  ...
  We improved delivery latency across the X API v2 streaming endpoints that prioritize data hydration, including [Filtered Stream](/x-api/posts/filtered-stream/introduction). P99 latency is now **approximately 4-5 seconds**, down from approximately 6-7 seconds. No changes are required on your end. For the lowest-latency option, see [Powerstream](/x-api/powerstream/introduction). Learn more about [consuming streaming data](/x-api/fundamentals/consuming-streaming-data).

  ### OAuth 2.0 user context for webhook and subscription management

  [Webhook management](/x-api/webhooks/introduction) endpoints (`GET`/`POST /2/webhooks`, `PUT`/`DELETE /2/webhooks/{webhook_id}`) now accept OAuth 2.0 user context and OAuth 1.0a user context authentication, in addition to app-only Bearer tokens. The [X Activity API](/x-api/activity/introduction) subscription endpoints `PUT`/`DELETE /2/activity/subscriptions/{subscription_id}` also accept user-context authentication. You can now manage webhooks and subscriptions with the same user-context tokens your app already uses.
</Update>
