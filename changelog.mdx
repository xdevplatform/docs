...
<Update label="Week of Sep 15, 2026" description="This week on the X Ads API">
  ### New features

  - **Conversion API Google Tag Manager server-side template.** Advertisers can send website conversions to X through the official **X Ads Conversion API (Official)** tag in a Google Tag Manager server container, without writing Conversion API request code. See [Set up the Conversion API with Google Tag Manager](/x-ads-api/measurement/gtm-server-side). The template is available in the [GTM Community Template Gallery](https://tagmanager.google.com/gallery/#/owners/twitter/templates/x-ads-conversion-api-gtm-template).
  - **Lead Generation API.** You can now create and manage native [lead forms](/x-ads-api/lead-generation), attach a form to a card with the `LEAD_FORM` destination, and export submitted leads as JSON, CSV, or XLSX. The Leads API requires the Campaign user permission; lead exports are restricted to account administrators.

  ### Updates

  - **`REGIONS` segmentation for asynchronous Analytics jobs.** Analytics jobs now support `REGIONS` as a `segmentation_type`, with `country` required when requesting a regional breakdown. See [Metrics and Segmentation](/x-ads-api/analytics#metrics-and-segmentation).
</Update>
