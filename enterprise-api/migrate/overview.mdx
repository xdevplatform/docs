...
<Button href="/enterprise-api/migrate/x-api-endpoint-map">
    X API Endpoint Map
  </Button>
...
- [Spaces endpoints](/enterprise-api/spaces/introduction) to help people get more out of X Spaces, and to allow developers to help shape the future of audio conversations.
- [Hide replies](/enterprise-api/posts/hide-replies/introduction), which allows you to build tools that help limit the impact of abusive, distracting, or misleading replies at scale.
- New Lists endpoints that allow you to [pin and unpin Lists](/enterprise-api/lists/pinned-lists/introduction), or look up someone’s pinned Lists.
- New [batch compliance endpoints](/enterprise-api/compliance/batch-compliance/introduction) that allow you to ensure your stored user and Tweet data is in compliance.
...
- [A consistent design across endpoints](/enterprise-api/fundamentals/consistency)
- [The ability to specify which fields and objects return in the response payload](/enterprise-api/fundamentals/data-dictionary#how-to-use-fields-and-expansions)
- [New and more detailed data objects](/enterprise-api/fundamentals/data-dictionary)
- [Receive and filter data with new contextual information powered by Tweet annotations](/enterprise-api/fundamentals/post-annotations)
- [Access to new metrics](/enterprise-api/fundamentals/metrics)
- [Easily identify and filter for conversations that belong to a reply thread](/enterprise-api/fundamentals/conversation-id)
...
- [Recovery and redundancy functionality for streaming endpoints](/enterprise-api/posts/filtered-stream/integrate/recovery-and-redundancy-features)
- [Easily return counts of Tweets that match a query](/enterprise-api/posts/counts/introduction)
- [Support for Edit Tweets](/enterprise-api/fundamentals/edit-posts)
...
| Object | Description |
|:--------|:-------------|
| [Tweet](/enterprise-api/fundamentals/data-dictionary#tweet) | The Tweet object has a long list of root-level fields, such as `id`, `text`, and `created_at`. Tweet objects are also the parent object to several child objects including `user`, `media`, `poll`, and `place`. |
| [User](/enterprise-api/fundamentals/data-dictionary#user) | The user object contains X user account metadata describing the referenced user. |
| [Spaces](/enterprise-api/fundamentals/data-dictionary#space) | The Space object consists of fields such as `state`, `host_id`, `is_ticketed`, and even `lang`. |
| [Lists](/enterprise-api/fundamentals/data-dictionary#list) | The List object contains basic information about the requested list including `description`, `member_count`, and `owner_id`. |
| [Media](/enterprise-api/fundamentals/data-dictionary#media) | If a Tweet contains media (such as images), then the media object can be requested using the `media.fields` parameter and includes fields such as the `media_key`, `type`, `url`, `preview_image_url`, and more. |
| [Poll](/enterprise-api/fundamentals/data-dictionary#poll) | A poll included in a Tweet is not a primary object on any endpoint, but can be found and expanded in the Tweet object. |
| [Place](/enterprise-api/fundamentals/data-dictionary#place) | The place object consists of fields such as `place_id`, `geo` object, `country_code`, and more. This information can be used to identify Tweets and study Tweets by location. |

Learn more about [how to use fields and expansions](/enterprise-api/fundamentals/data-dictionary#how-to-use-fields-and-expansions).
...
Learn more about the available [metrics](/enterprise-api/fundamentals/metrics).
...
The X API v2 endpoints provide edited Tweet metadata. The _Edit Tweet_ feature was first introduced for testing among X employees on September 1, 2022. Starting on that date, eligible Tweets are editable for 30 minutes and up to 5 times. Learn more about [Edit Tweets](/enterprise-api/fundamentals/edit-posts).
...
A new Tweet field helps identify which conversation thread a Tweet belongs to. A conversation ID is the Tweet ID of the Tweet that started the conversation. Learn more about [conversation tracking](/enterprise-api/fundamentals/conversation-id).
...
- We have a Twitter [Postman collection](https://www.postman.com/xapidevelopers/x-api-public-workspace/collection/34902927-2efc5689-99c6-4ab6-8091-996f35c2fd80) that allows you to use the Postman client to make requests of and connect to the individual endpoints. This is a low-friction way to test authentication and experiment with the endpoints.
- We’ve also provided a list of both Twitter-supported and third-party libraries in Ruby, Python, Node, Java, and many more. For additional context, take a look at our [tools and libraries page](/tools-and-libraries).
...
- **Tweets**
  - [Tweets lookup](/enterprise-api/posts/lookup/integrate)
  - [Manage Tweets](/enterprise-api/posts/manage-tweets/migrate/overview)
  - [Timelines](/enterprise-api/posts/timelines/migrate/overview)
  - [Search Tweets](/enterprise-api/posts/search/migrate/overview)
  - [Tweet counts](/enterprise-api/posts/counts/migrate/overview)
  - [Filtered stream](/enterprise-api/posts/filtered-stream/migrate/overview)
  - Sampled stream
  - [Retweets](/enterprise-api/posts/retweets/migrate/overview)
  - [Likes](/enterprise-api/posts/likes/migrate/likes-lookup-standard-to-twitter-api-v2)
  - [Hide replies](/enterprise-api/posts/hide-replies/migrate)
- **Users**
  - [Users lookup](/enterprise-api/users/lookup/migrate/overview)
  - [Follows](/enterprise-api/users/follows/migrate/standard-to-twitter-api-v2)
  - [Blocks](/enterprise-api/users/blocks/migrate)
  - [Mutes](/enterprise-api/users/mutes/migrate/manage-mutes-standard-to-twitter-api-v2)
- **Lists**
  - [List lookup](/enterprise-api/lists/list-lookup/migrate/overview)
  - [Manage Lists](/enterprise-api/lists/manage-lists/migrate/overview)
  - [List Tweet lookup](/enterprise-api/lists/list-tweets/migrate/overview)
  - [List members](/enterprise-api/lists/list-members/migrate/overview)
...
You can learn more by visiting our [data formats migration](/enterprise-api/migrate/data-format-migration) section of this migration hub, or by visiting our specific data format guides:
...
Those of you who have used the platform for some time will notice that many of the new endpoints are aligned with existing [standard v1.1](https://developer.x.com/en/docs/twitter-api/v1) and [enterprise](/enterprise-api/enterprise-gnip-2.0/enterprise-gnip) endpoints. Indeed, we intend for these to replace all three versions in the future.

We’ve put together a table to help you understand how the [X API endpoints map](/enterprise-api/migrate/x-api-endpoint-map) to previous versions.
