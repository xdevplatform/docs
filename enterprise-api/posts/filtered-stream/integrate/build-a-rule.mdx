...
Multiple rules can be applied using the [POST /tweets/search/stream/rules](/enterprise-api/stream/update-stream-rules) endpoint. Once you've added rules and connected using [GET /tweets/search/stream](/enterprise-api/stream/get-stream-rules), only Posts matching your rules will be delivered. You do not need to disconnect to add or remove rules.

---

## Rule limitations

Limits on the number of rules depend on your [access level](/enterprise-api/getting-started/about-x-api). See the [filtered stream introduction](/enterprise-api/posts/filtered-stream/introduction) for specific limits.
...
When [building search queries](/enterprise-api/posts/search/integrate/build-a-query), keywords with accents match Posts both with and without the accents. For example, `Diacrítica` matches both _Diacrítica_ and _Diacritica_.
</Note>
...
<Note>
[Search Posts](/enterprise-api/posts/search/introduction) behaves differently—it only matches on the Quote Tweet's content, not the original Post.
</Note>
...
Use [POST /2/tweets/search/stream/rules](/enterprise-api/stream/update-stream-rules) to add or remove rules.
...
First, use [Post lookup](/enterprise-api/posts/lookup/introduction) with `tweet.fields=context_annotations` to identify domain.entity IDs:
...
<CardGroup cols={2}>
  <Card title="Operators reference" icon="list" href="/enterprise-api/posts/filtered-stream/integrate/operators">
    Complete list of available operators
  </Card>
  <Card title="Filtered stream quickstart" icon="rocket" href="/enterprise-api/posts/filtered-stream/quickstart">
