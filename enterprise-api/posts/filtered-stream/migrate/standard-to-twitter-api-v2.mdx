...
If you have been working with the v1.1 [statuses/filter](https://developer.x.com/en/docs/enterprise-api/v1/tweets/filter-realtime/api-reference/post-statuses-filter) endpoint, this guide can help you understand the similarities and differences between the standard and X API v2 filtered stream endpoints.
...
Both versions provide metadata that describes any edit history. Check out the [filtered stream API References](/enterprise-api/posts/filtered-stream/introduction) and the [Post edits fundamentals page](/enterprise-api/fundamentals/edit-posts) for more details. 
...
In addition to the changes in how you request certain fields, X API v2 is also introducing new JSON designs for the objects returned by the APIs, including Post and [user](/enterprise-api/fundamentals/data-dictionary#user) objects.
...
Both of these features are only available via [Academic Research access](/enterprise-api/getting-started/about-x-api). You can learn more about this functionality via our [recovery and redundancy features](/enterprise-api/fundamentals/recovery-and-redundancy) integration guide. 
