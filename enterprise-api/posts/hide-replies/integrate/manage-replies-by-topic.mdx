...
keywords: ["manage replies by topic", "topic-based moderation", "reply moderation by topic", "hide replies by topic", "topic filtering"]
---
...
Through the hide replies endpoint, you can build integrations to help people and brands keep their conversation on topic. This page shows how to manage a conversation using the hide replies and [recent search](/enterprise-api/posts/recent-search) endpoints.

Recent search has functionality to a conversation and its replies, and the Post payload returns [Post annotations](/enterprise-api/fundamentals/post-annotations) to help you understand the context and topic of each Post, regardless of language.
...
5.  For longer conversations, it will provide controls to [paginate through search results](/enterprise-api/posts/search/integrate/paginate).
     
...
2.  Request [user and Post fields](/enterprise-api/fundamentals/fields) in the same request to avoid making separate requests. This approach can also improve your app's performance.
