...
| **Post [JSON format](/enterprise-api/fundamentals/data-dictionary)** | Standard v1.1 format | [X API v2 format](/enterprise-api/fundamentals/data-dictionary), determined by `fields` and `expansions` parameters (not backward-compatible with v1.1) |
| **Supports selecting specific [fields](/enterprise-api/fundamentals/data-dictionary)** |  | ✔ |
| **Supports the [annotations](/enterprise-api/fundamentals/post-annotations) fields** |  | ✔ |
| **Supports new [metrics](/enterprise-api/fundamentals/metrics) fields** |  | ✔ |
...
You may also be interested in our [visual data format migration tool](/enterprise-api/migrate/data-format-migration) to help you quickly see the differences between the [X API v1.1 data format](/enterprise-api/fundamentals/data-dictionary) and the [X API v2 format](/enterprise-api/fundamentals/data-dictionary).
...
Both versions provide metadata that describes any edit history. Check out the Post lookup API References and the [Edit Posts fundamentals page](/enterprise-api/fundamentals/edit-posts) for more details.
...
X API v2, however, only delivers the Post `id` and `text` fields by default. Additional fields and objects require the use of [fields](/enterprise-api/fundamentals/fields) and [expansions](/enterprise-api/fundamentals/expansions) parameters. The expanded fields return in an `includes` object within the response, which can be matched to the primary Post object by matching IDs.

For more on using fields and expansions, see the [guide on how to use fields and expansions](/enterprise-api/fundamentals/data-dictionary). A [data format migration guide](/enterprise-api/fundamentals/fields) also maps standard v1.1 fields to the newer v2 fields.

Additionally, X API v2 introduces new JSON designs for objects, including the Post and [user](/enterprise-api/fundamentals/fields) objects:
...
- `conversation_id`
- Two new [annotations](/enterprise-api/fundamentals/post-annotations) fields (`context` and `entities`)
- New [metrics](/enterprise-api/fundamentals/metrics) fields
...
The response payloads from v1.1 will differ from v2. With v2, you can request different fields with the [fields](/enterprise-api/fundamentals/fields) and [expansions](/enterprise-api/fundamentals/expansions) parameters.
