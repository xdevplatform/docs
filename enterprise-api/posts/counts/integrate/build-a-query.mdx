...
Your queries will be limited depending on which [access level](/enterprise-api/getting-started/about-x-api) you are using. 
...
While most operators are available to any developer, there are several that are reserved for those that have been approved for Enterprise access. We list which access level each operator is available to in the [list of operators](/enterprise-api/posts/search/integrate/build-a-query) table using the following labels:
...
However, please note that [filtered stream](/enterprise-api/posts/filtered-stream) will match on both the content from the original Post that was quoted and the Quote Tweet's content.
 
...
As you build a query, it is important for you to periodically test it out using one of the [Search Post](/enterprise-api/posts/search/introduction) endpoints to ensure that the Posts that are matching your query are relevant to your use case.
...
This rule was built to filter for original Posts that included an image of a pet that is not a cat, where the language identified in the Post is Japanese. To do this, we used the context: operator to take advantage of the[Post annotation](/enterprise-api/fundamentals/post-annotations) functionality. We first used the[Post lookup](/enterprise-api/posts/lookup/introduction) endpoint and the tweet.fields=context_annotations fields parameter to identify which domain.entity IDs we need to use in our query:
