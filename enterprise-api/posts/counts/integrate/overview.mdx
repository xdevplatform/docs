...
Every day, many thousands of developers make requests to the X API. To help manage the volume, [rate limits](/enterprise-api/fundamentals/rate-limits) are placed on each endpoint that limits the number of requests that every developer can make on behalf of an app or on behalf of an authenticated user.
...
You can use our guide on [how to build a query](/enterprise-api/posts/counts/integrate/build-a-query) to learn more.
