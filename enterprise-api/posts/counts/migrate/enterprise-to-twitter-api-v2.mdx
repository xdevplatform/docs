...
However, the X API v2 does not offer a 30 day time period endpoint like the enterprise API does. Instead it offers the aforementioned full-archive, or a 7 day time period, which align with the [v2 Search Posts endpoints](/enterprise-api/posts/search/introduction).
 
...
To see a full table of the operators that are available for X API v2, enterprise, and even premium, please visit the [Post counts migration landing page](/enterprise-api/posts/counts/migrate/overview).
...
|     |     |
| :--- | :--- |
| **Receive a count of Posts that match a query in the last 7 days** | `[GET /2/tweets/counts/recent](/enterprise-api/posts/tweet-counts#api-reference-index/get-tweets-counts-recent)` |

### Full-archive Post counts

Only available to those with [Self-serve and Enterprise access](/enterprise-api/getting-started/about-x-api)

|     |     |
| :--- | :--- |
| **Receive a count of Posts that match a query** | `[GET /2/tweets/counts/all](/enterprise-api/posts/tweet-counts#api-reference-index/get-tweets-counts-all)` |
