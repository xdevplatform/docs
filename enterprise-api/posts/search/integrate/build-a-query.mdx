...
See the complete [list of operators](/enterprise-api/posts/search/integrate/operators) for availability details.
...
When [building filtered stream rules](/enterprise-api/posts/filtered-stream/integrate/build-a-rule), keywords with accents only match Posts that also include the accent. For example, `Diacrítica` only matches _Diacrítica_, not _Diacritica_.
</Note>
...
<Note>
[Filtered stream](/enterprise-api/posts/filtered-stream/introduction) behaves differently—it matches on both the Quote Tweet and the original Post's content.
</Note>
...
<CardGroup cols={2}>
  <Card title="Operators reference" icon="list" href="/enterprise-api/posts/search/integrate/operators">
    Complete list of available operators
  </Card>
  <Card title="Search quickstart" icon="rocket" href="/enterprise-api/posts/search/quickstart/recent-search">
