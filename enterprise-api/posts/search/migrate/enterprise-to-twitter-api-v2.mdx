...
However, the X API v2 does not offer a 30 day time period endpoint like the enterprise API does. Instead it offers the aforementioned full-archive, or a 7 day time period, which align with the Native Enriched to v2 and [Activity Streams to v2](/enterprise-api/migrate/data-format-migration#migrating-from-activity-streams-data-format-to-v2) which can help you map enterprise fields to the newer v2 fields. This guide will also provide you the specific expansion and field parameter that you will need to pass with your v2 request to return specific fields.
 

**Response data format**

One of the biggest differences between the [enterprise response format](https://developer.x.com/enterprise-api/enterprise-gnip-2.0/fundamentals/data-dictionary) and[X API v2’s format](/enterprise-api/fundamentals/data-dictionary) is how you select which fields return in your payload.
...
To see a full table of the operators that are available for X API v2, enterprise, and even premium and standard, please visit the [Search Posts migration landing page](/enterprise-api/posts/search/migrate/overview).

**Next steps**

[Check out our quick start guide for X API v2 full-archive search](/enterprise-api/posts/search/quickstart/full-archive-search)

[Review the API reference for full-archive search](/enterprise-api/posts/full-archive-search)
