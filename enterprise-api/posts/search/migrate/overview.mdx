...
The v2 Search Tweets endpoint will eventually replace the [standard v1.1 search/posts](/enterprise-api/posts/search/introduction) endpoint and [enterprise Search API](/enterprise-api/enterprise-gnip-2.0/fundamentals/search-api). If you have code, apps, or tools that use an older version of a X search endpoint and are considering migrating to the newer X API v2 endpoints, then this guide is for you. 
...
| Post JSON format | Standard v1.1 format | [X API v2 format](/enterprise-api/fundamentals/data-dictionary) (determined by fields and expansions request parameters, not backward-compatible with v1.1 formats)<br/><br/>To learn more about how to migrate from the Standard v1.1 format to the X API v2 format, please visit our [data formats migration guide](/enterprise-api/migrate/data-format-migration). |
| Supports selecting which [fields](/enterprise-api/fundamentals/fields) return in the payload |     | ✔   |
| Supports requesting and receiving [annotations](/enterprise-api/fundamentals/post-annotations) |     | ✔   |
| Supports requesting specific [metrics](/enterprise-api/fundamentals/metrics) within Post object |     | ✔   |
| Supports the [conversation_id](/enterprise-api/fundamentals/conversation-id) operator and field |     | ✔   |
...
| Post JSON format | [Native Enriched or Activity Streams](/enterprise-api/enterprise-gnip-2.0/fundamentals/data-dictionary) format | [X API v2](/enterprise-api/fundamentals/data-dictionary) format (determined by fields and expansions request parameters) |
| Supports selecting which [fields](/enterprise-api/fundamentals/fields) return in the payload |     | ✔   |
| Supports requesting and receiving [annotations](/enterprise-api/fundamentals/post-annotations) |     | ✔   |
| Supports requesting specific [metrics](/enterprise-api/fundamentals/metrics) within Post object |     | ✔   |
| Supports the [conversation_id](/enterprise-api/fundamentals/conversation-id) operator and field |     | ✔   |
...
* [Enterprise operators](/enterprise-api/enterprise-gnip-2.0/fundamentals/search-api#available-operators)
* [X API v2 operators](/enterprise-api/posts/search/integrate/build-a-query#operators)
...
[Search Posts: Standard v1.1 to X API v2](/enterprise-api/posts/search/migrate/standard-to-twitter-api-v2)

[Search Posts: Enterprise to X API v2](/enterprise-api/posts/search/migrate/enterprise-to-twitter-api-v2)
