...
**`POST /2/webhooks`** — [API Reference](/enterprise-api/webhooks/create-webhook)
...
**`GET /2/webhooks`** — [API Reference](/enterprise-api/webhooks/get-webhook)
...
**`DELETE /2/webhooks/:webhook_id`** — [API Reference](/enterprise-api/webhooks/delete-webhook)
...
**`PUT /2/webhooks/:webhook_id`** — [API Reference](/enterprise-api/webhooks/validate-webhook)
...
<Warning>
- **All incoming Direct Messages** will be delivered via webhooks. DMs sent via [POST /2/dm_conversations/with/:participant_id/messages](/enterprise-api/direct-messages/send-a-new-message-to-a-user) will also be delivered, so your app can track DMs sent from other clients.
...
- X sends events as **POST requests** with JSON payloads. See the [Account Activity data object structure](/enterprise-api/account-activity/introduction#account-activity-data-object-structure) for example payloads.
</Warning>
