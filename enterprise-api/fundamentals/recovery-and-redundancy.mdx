...
Learn how to maximize connection time and recover missed data when using X streaming endpoints including [Filtered Stream](/x-api/posts/filtered-stream/introduction), [Volume Streams](/x-api/posts/volume-streams/introduction), [Firehose Streams](/x-api/stream/stream-all-posts), [Likes Firehose](/x-api/stream/stream-all-likes), [Powerstream](/x-api/powerstream/introduction), and [Compliance Streams](/x-api/compliance/streams/introduction).
...
| Firehose Streams | `backfill_minutes` | `?backfill_minutes=5` |
| Likes Firehose | `backfill_minutes` | `?backfill_minutes=5` |
...
```bash
curl 'https://api.x.com/2/tweets/firehose/stream?partition=1&start_time=2022-07-12T15:10:00Z&end_time=2022-07-12T15:20:00Z' \
  -H "Authorization: Bearer $ACCESS_TOKEN"
```
...
```bash
curl 'https://api.x.com/2/likes/firehose/stream?partition=1&start_time=2022-07-12T15:10:00Z&end_time=2022-07-12T15:20:00Z' \
  -H "Authorization: Bearer $ACCESS_TOKEN"
```
