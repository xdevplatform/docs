...
There are a few important concepts to understand when using the [`POST /2/media/upload`](/enterprise-api/media/media-upload) endpoint. Uploading media with OAuth can be a bit tricky, so we’ve outlined some things to keep in mind as well as a working sample of how to use this endpoint here.
...
In order to process larger GIFs, use the [chunked upload](/enterprise-api/media/quickstart/media-upload-chunked) endpoint with the `media_category` parameter. This allows the server to process the GIF file asynchronously, which is a requirement for processing larger files. Pass `media_category=tweet_gif` to enable async upload behavior for Posts with an animated GIF.
...
For an example of how to upload media, please see the [chunked media upload documentation](/enterprise-api/media/quickstart/media-upload-chunked).

### Troubleshooting

For issues with the Media APIs, browse the [Media API category](https://devcommunity.x.com/c/enterprise-api/media-apis) in the developer forums for an answer.