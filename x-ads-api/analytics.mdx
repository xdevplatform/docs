---
title: Analytics
keywords: ["advertising analytics", "ads analytics", "campaign analytics", "ad metrics", "advertising metrics", "campaign performance"]
---
...
End time is exclusive. For example, a request with `start_time=2025-01-01T00:00:00Z` and `end_time=2025-01-02T00:00:00Z` will return a single day's worth of analytics metrics (not two) as this time range covers only a 24 hour period.
...
- Make sure you've requested data for all placements: `ALL_ON_TWITTER`, `PUBLISHER_NETWORK`, `SPOTLIGHT`, and `TREND`.
...
- We support the following placement values in analytics: `ALL_ON_TWITTER`, `PUBLISHER_NETWORK`, `SPOTLIGHT`, and `TREND` (i.e., the [X Audience Platform](/x-ads-api/audiences))
...
| :--- | :--- | :--- | :--- |
| Metric | Description | Segmentation Available | Data Type |
| `media_views` | Total number of views (autoplay and click) of media across Videos, GIFs, and Images. | ✔   | Array of ints |
| `media_engagements` | Total number of clicks of media across Videos, GIFs, and Images. | ✔   | Array of ints |
...
`twurl -H ads-api.x.com "/12/stats/accounts/18ce54d4x5t/active_entities?entity=PROMOTED_TWEET&start_time=2025-03-05T00:00:00Z&end_time=2025-03-06T00:00:00Z"`
...
```json
    {
      ...
      "data": [
        {
          "entity_id": "2r0wxw",
          "activity_start_time": "2025-03-04T20:55:20Z",
          "activity_end_time": "2025-03-05T03:43:56Z",
          "placements": [
            "ALL_ON_TWITTER"
          ]
        },
        {
          "entity_id": "2r30fn",
          "activity_start_time": "2025-03-05T08:11:08Z",
          "activity_end_time": "2025-03-05T14:40:59Z",
          "placements": [
            "ALL_ON_TWITTER",
            "PUBLISHER_NETWORK"
          ]
...
| 2025-03-04T20:55:20Z  <br/>  <br/>2025-03-05T14:40:59Z | 2025-03-04T00:00:00Z  <br/>  <br/>2025-03-06T00:00:00Z |

**Note**: It's important to include the timestamps with hours, minutes, and seconds set to zero. Otherwise, if only the date is passed in, we will assume you're requesting analytics starting and ending at midnight in the ads account's timezone, which may not be desirable. For example, if the minimum activity start time is 2025-02-28T01:30:07Z and the timestamp is omitted for an ads account with an offset of -08:00:00, the analytics request will miss changes that happened between 01:30 and 08:00.

Alternatively, if you would prefer to request analytics for just the returned activity time window without expanding to full days, you can. Using this approach, the derived start and end times would be 2025-03-04T20:00:00Z and 2025-03-05T15:00:00Z, respectively. (Note that ranges like these are not accepted if you specify `DAY` granularity in the analytics request.)
...
```
`twurl -H ads-api.x.com "/12/stats/accounts/18ce54d4x5t/active_entities?entity=LINE_ITEM&start_time=2025-02-11T02:00:00Z&end_time=2025-02-11T03:00:00Z"`
```
...
```json
    {
      ...
      "data": [
        {
          "entity_id": "dvcz7",
          "activity_start_time": "2025-02-11T02:02:55Z",
          "activity_end_time": "2025-02-11T02:58:12Z",
          "placements": [
            "ALL_ON_TWITTER"
          ]
        }
...
Based on these activity start and end times and using the approach described above, the analytics `start_time` and `end_time` values are set to 2025-02-11T00:00:00Z and 2025-02-12T00:00:00Z, respectively. We see that the third element in each of the metrics arrays below are non-zero, as we expected based on the active entities information.

```
`twurl -H ads-api.x.com "/12/stats/accounts/18ce54d4x5t?entity=LINE_ITEM&entity_ids=dvcz7&start_time=2025-02-11T00:00:00Z&end_time=2025-02-12T00:00:00Z&granularity=HOUR&metric_groups=ENGAGEMENT,VIDEO&placement=ALL_ON_TWITTER"`
```
...
```
`twurl -H ads-api.x.com "/12/stats/accounts/18ce54d4x5t/active_entities?entity=LINE_ITEM&start_time=2025-02-11T03:00:00Z&end_time=2025-02-11T04:00:00Z"`
```
...
```json
    {
      ...
      "data": [
        {
          "entity_id": "dvcz7",
          "activity_start_time": "2025-02-11T02:07:17Z",
          "activity_end_time": "2025-02-11T03:49:22Z",
          "placements": [
            "ALL_ON_TWITTER"
          ]
        }
...
```
`twurl -H ads-api.x.com "/12/stats/accounts/18ce54d4x5t/active_entities?entity=LINE_ITEM&start_time=2025-02-11T04:00:00Z&end_time=2025-02-11T05:00:00Z"`
```
...
```json
    {
      ...
      "data": [
        {
          "entity_id": "dvcz7",
          "activity_start_time": "2025-02-11T03:42:39Z",
          "activity_end_time": "2025-02-11T03:48:48Z",
          "placements": [
            "ALL_ON_TWITTER"
          ]
        }
...
```
`twurl -H ads-api.x.com "/12/stats/accounts/18ce54d4x5t/active_entities?entity=LINE_ITEM&start_time=2025-02-11T05:00:00Z&end_time=2025-02-11T06:00:00Z"`
```
...
```
$ twurl -X POST -H ads-api.x.com "/12/stats/jobs/accounts/18ce54d4x5t?entity=LINE_ITEM&entity_ids=el32n&start_time=2025-03-12T00:00:00Z&end_time=2025-03-20T00:00:00Z&granularity=TOTAL&placement=ALL_ON_TWITTER&metric_groups=ENGAGEMENT"
```
...
```json
    {
      ...
      "data": {
        ...
        "granularity": "TOTAL",
        "entity": "LINE_ITEM",
        "created_at": "2025-04-01T23:19:46Z",
        "platform": null,
        "updated_at": "2025-04-01T23:19:46Z",
        "metric_groups": [
          "ENGAGEMENT"
        ]
      }
      ...
      "data": [
        {
          ...
          "placement": "ALL_ON_TWITTER",
          "id": 1120829647711653888,
          "expires_at": "2025-04-03T23:19:48Z",
          "account_id": "18ce54d4x5t",
          "status": "SUCCESS",
          "granularity": "TOTAL",
          "entity": "LINE_ITEM",
          "created_at": "2025-04-01T23:19:46Z",
          "platform": null,
          "updated_at": "2025-04-01T23:19:48Z",
          "metric_groups": [
            "ENGAGEMENT"
          ]
        }
...
| end_time  <br/>_required_ | Scopes the retrieved data to the specified end time, expressed in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601).<br/><br/>**Note**: Must be expressed in whole hours (0 minutes and 0 seconds).<br/><br/>Type: string<br/><br/>Example: `2025-05-26T07:00:00Z` |
| start_time  <br/>_required_ | Scopes the retrieved data to the specified start time, expressed in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601).<br/><br/>**Note**: Must be expressed in whole hours (0 minutes and 0 seconds).<br/><br/>Type: string<br/><br/>Example: `2025-05-19T07:00:00Z` |

### Example Request[](#example-request "Permalink to this headline")

`GET https://ads-api.x.com/12/stats/accounts/18ce54d4x5t/reach/campaigns?campaign_ids=8fgzf&start_time=2025-05-19&end_time=2025-05-26`
...
```
GET https://ads-api.x.com/12/stats/accounts/18ce54d4x5t/reach/funding_instruments?funding_instrument_ids=lygyi&start_time=2025-05-19&end_time=2025-05-26
```
...
```
GET https://ads-api.x.com/12/stats/accounts/18ce54d4x5t?entity=LINE_ITEM&entity_ids=8u94t&start_time=2025-05-19&end_time=2025-05-26&granularity=TOTAL&placement=ALL_ON_TWITTER&metric_groups=ENGAGEMENT
```
...
```json
    {
      ...
      "request": {
        "params": {
          "start_time": "2025-05-19T07:00:00Z",
          "segmentation_type": null,
          "entity_ids": [
            "8u94t"
          ],
...
`GET https://ads-api.x.com/12/stats/accounts/18ce54d4x5t/active_entities?entity=PROMOTED_TWEET&start_time=2025-02-28&end_time=2025-03-01`
...
```json
    {
      "request": {
        "params": {
          ...
          "end_time": "2025-03-01T08:00:00Z"
        }
      },
      "data": [
        {
          "entity_id": "2mvb28",
          "activity_start_time": "2025-02-28T01:30:07Z",
          "activity_end_time": "2025-03-01T07:42:55Z",
          "placements": [
            "ALL_ON_TWITTER"
          ]
        },
        {
          "entity_id": "2mvb29",
          "activity_start_time": "2025-02-27T11:30:07Z",
          "activity_end_time": "2025-03-01T07:42:50Z",
          "placements": [
            "ALL_ON_TWITTER",
            "PUBLISHER_NETWORK"
          ]
        ...
        {
          "entity_id": "2mvfan",
          "activity_start_time": "2025-02-27T09:00:05Z",
          "activity_end_time": "2025-03-01T06:06:36Z",
          "placements": [
            "PUBLISHER_NETWORK"
          ]
        },
        {
          "entity_id": "2n17dx",
          "activity_start_time": "2025-02-28T02:02:26Z",
          "activity_end_time": "2025-03-01T07:52:44Z",
          "placements": [
            "ALL_ON_TWITTER",
            "PUBLISHER_NETWORK"
          ]
