...
description: "Glossary of X Ads API hierarchy and terminology covering accounts, campaigns, line items, creatives, cards, funding instruments, and audience concepts."
...
A collection of creatives that have been uploaded to an ads account as [account media](/x-ads-api/creatives/reference#account-media) that can include promoted video [pre-roll assets](/x-ads-api/campaign-management/reference#video-views-preroll-objective), also referred to as in-stream ads, and images that can be promoted on the [X Audience Platform](https://developer.x.com/content/developer-twitter/en/docs/ads/measurement/overview/twitter-audience-platform).
...
[Lead forms](/x-ads-api/lead-generation) collect contact information in a native form on X. Attach a form to a card with a `LEAD_FORM` destination, then export submitted leads for a form and time window. Lead form identifiers are UUIDs.
