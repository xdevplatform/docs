...
<script>
  // Preserve backward compatibility for old deep anchor links
  // (e.g. /campaign-management#get-accounts-account-id-campaigns)
  // These anchors now live on the /reference page.
  (function () {
    try {
      var hash = window.location.hash;
      if (hash && /^#(get|post|put|delete|patch|batch)-/i.test(hash)) {
        var target = '/x-ads-api/campaign-management/reference' + hash;
        // Use replace so the browser history stays clean
