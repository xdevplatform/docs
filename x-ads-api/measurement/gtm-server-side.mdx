...
<Note>
You can keep using your existing GA4 event names. If you leave the X tag’s **Event ID** empty, the template sends `event_name` as `gtm_` plus the incoming name, lowercased (for example `purchase` → `gtm_purchase`).
</Note>
