...
Destinations are where advertisers intend to take users. They are always required within `DETAILS` or `BUTTON` components. Destination types are `WEBSITE`, `APP`, and `LEAD_FORM`.

**Note**: Website destinations can only be used with `DETAILS` components and app destinations can only be used with `BUTTON` components. The `LEAD_FORM` destination can be used anywhere a destination is accepted, including buttons in a `BUTTON_GROUP` component. See [Lead Generation](/x-ads-api/lead-generation).
...
Opens a native lead form when the component is tapped. Invalid lead form ids are rejected with one error per invalid id. See [Attach a lead form to a card](/x-ads-api/lead-generation/reference#attach-a-lead-form-to-a-card).

| Name | Description |
| :--- | :--- |
| type   *required* | The destination type, which determines its schema.  Type: enum  Possible values: `LEAD_FORM` |
| id   *required* | Id of the lead form to open. Must be a valid lead form UUID.  Type: string  Example: `3f2b8c1a-8f4e-4a0b-9df2-0123456789ab` |
